...
4. From the project root, open `/qfai-migration-spec-to-story` and read its
   `references/migration-guide.md`. The skill is under
   `.qfai/assistant/skill/qfai-migration-spec-to-story/`. There is no
...
host integration links and the managed `.gitignore` block. Read the skill
guide's write-boundary table before approving a dry run.
...
After step 10, run the locally installed `qfai validate --profile full
...
complete run, rerun the ten steps and confirm that they change no files.
