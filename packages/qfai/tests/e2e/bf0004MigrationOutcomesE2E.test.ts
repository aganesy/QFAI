...
describe("BF-0004 migration outcomes", () => {
  ...
  it("preserves user ignore lines and makes the decision directory visible to Git", async () => {
    ...
    const initialized = run(freshRoot, process.execPath, [
      cli,
      "init",
      "--dir",
      freshRoot,
    ...
    const fresh = await text(freshRoot, ".gitignore");
    const marker = "# ── QFAI managed (generated by qfai init) ──";
    expect(migrated.slice(migrated.indexOf(marker)).trimEnd()).toBe(
      fresh.slice(fresh.indexOf(marker)).trimEnd(),
    );
