...
describe.skip("spec-0004 saas-package profile CHG-006 (test-first, pending /qfai-implement)", () => {
  it("QFAI:BF-0001 — saas-package validate PASSes with honest D-SAAS-PACKAGE-VERIFY-SKIPPED skip findings (normal)", async () => {
  ...
  it("QFAI:BF-0001 — saas-package validate does NOT PASS when the DCON-005 attestation is missing (error)", async () => {
...
describe.skip("spec-0004 primary_tasks dual-shape CHG-006 (test-first, pending /qfai-implement)", () => {
  it("QFAI:BF-0001 — auditProfile accepts both string-only and structured primary_tasks (normal)", async () => {
  ...
  it("QFAI:BF-0001 — QFAI-AUD-020 warning names the 3..7 count band and the closed structured schema rejects an item missing acceptance (error/boundary)", async () => {
...
describe.skip("spec-0004 pack-location CI lane CHG-006 (test-first, pending /qfai-implement)", () => {
  it("QFAI:BF-0002 — check-pack-locations rejects a misplaced root review pack with R-PACK-LOCATION-DRIFT (error)", async () => {
  ...
  it("QFAI:BF-0002 — lane passes silently for packs placed under allowed roots (normal)", async () => {
