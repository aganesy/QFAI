...
describe("E2E: force update (US-0003-0003)", { timeout: 60000 }, () => {
  it("--force overwrites skills but preserves skills.local", async () => {
    ...
    try {
      ...
      const skillsLocalDir = path.join(tmpDir, ".qfai", "assistant", "skills.local");
      await mkdir(skillsLocalDir, { recursive: true });
      await writeFile(path.join(skillsLocalDir, "custom.md"), "custom content");

      await captureStdout(() => runInit({ dir: tmpDir, force: true, dryRun: false, yes: true }));

      const customContent = await readFile(path.join(skillsLocalDir, "custom.md"), "utf-8");
...
describe("E2E: gitignore managed block auto-append (US-0003-0015)", { timeout: 60000 }, () => {
  ...
  it("init migrates legacy managed block (strips review-*/ negations) on re-run", async () => {
    ...
    try {
      ...
      expect(content).not.toContain("!.qfai/review/review-*/**");
      const markerCount =
        content.split("# ── QFAI managed (generated by qfai init) ──").length - 1;
