 ...
 *
 * `runInit` once, shared across the file: eight inits of a full asset tree is eight times the same
...
const delivered = deliveredWorkflowTree("qfai-e2e-spec0003-");
const { workflowsDir, workflowText, jobsOf } = delivered;
  ...
  () => {
    it("delivers every runner selector through a repository variable", async () => {
      ...
      for (const [key, job] of Object.entries(jobs)) {
        ...
        selectors += 1;
        // The form is `shippedWorkflowFixtures`'s, shared with the declared shape and the
        // runner row. What this row adds is the subject: the tree `qfai init` wrote, rather
        // than the asset tree the other two read.
        const selector = parseRunnerSelector(String(runsOn));
        expect(selector, `${key} hard-codes its runner: ${String(runsOn)}`).not.toBeNull();
        ...
        expect(selector?.fallback ?? "", `${key} defaults to a non-public label`).toMatch(
