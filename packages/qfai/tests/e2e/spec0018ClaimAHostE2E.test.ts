/**
 * E2E: a host is claimed as supported only with its eval record (spec-0018).
 *
 * On a temp tree holding both READMEs, an eval record for the current version is written the way
 * the runner writes it; the claim check passes for the recorded host, then fails on a host the
 * READMEs claim with no record.
 */
...
async function passingRecord(host: string, version: string) {
  ...
  const typed = Object.fromEntries(
    Object.entries(Object(vocabulary)).map(([k, v]) => [k, String(v)]),
  );
...
// QFAI:SPEC-0018:US-0018-0010
it("US-0018-0010 (TDD-0464): an eval record is written, the claim check passes, then fails on a claim with no record", async () => {
