...
/**
 * `red-provenance.md` and the stage-handover topic file it points at.
 *
 * The handover protocol moved out when the file crossed the shipped-asset line
 * ceiling — `red-provenance.md` answers what a RED observation IS, and
 * `stage-handover.md` answers who takes the row through it. This guard's
 * subject spans both, so it reads both; the pointer under the original heading
 * keeps `#which-stage-hands-a-row-over` resolving for the shipped citation.
 */
const readProvenance = async (tree: string): Promise<string> => {
  const [provenance, handover] = await Promise.all([
    read(tree, PROVENANCE),
    read(tree, STAGE_HANDOVER),
  ]);
  return `${provenance}
${handover}`;
};
...
describe.each(TREES)("%s", (tree) => {
  ...
  it("requires the branch and its evidence in the stage evidence file", async () => {
    ...
    expect(atdd).toContain("## Ledger rows advanced");
    expect(flat(await readProvenance(tree))).toContain(
...
describe.each(TREES)("%s (reachability and sequencing)", (tree) => {
  ...
  it("hands branch-1 rows over before the gates that require a green tree", async () => {
    ...
    expect(flat(await readProvenance(tree))).toContain("return with the tree green");
  });
...
describe.each(TREES)("%s (executability of the handed-over row)", (tree) => {
  ...
  it("hands the P1c round-trip a named row list", async () => {
    ...
    expect(flat(await readProvenance(tree))).toContain("Name the row in the handoff");
  });
...
describe.each(TREES)("%s (gates only what has a payload)", (tree) => {
  it("does not block a branch-2-only run on evidence P1b defers", async () => {
    ...
    expect(flat(await readProvenance(tree))).toContain("its rows are gated when the trio lands");
  });
...
describe.each(TREES)("%s (the handoff survives ledger order and time)", (tree) => {
  ...
  it("re-checks the branch against the tree at each row's handoff", async () => {
    ...
    expect(flat(await readProvenance(tree))).toContain("**The choice is provisional");
    expect(flat(await readProvenance(tree))).toContain(
...
describe.each(TREES)("%s (each gate reads what the step before it produced)", (tree) => {
  it("orders Phase Red 3a, 3b, 3c", async () => {
    ...
    const implement = await read(tree, IMPLEMENT);
    const a = implement.indexOf("#### Red 3a ");
    const b = implement.indexOf("#### Red 3b ");
    const c = implement.indexOf("#### Red 3c ");
...
describe.each(TREES)("%s (the two sides of each contract agree)", (tree) => {
  ...
  it("names a sibling reference as a sibling", async () => {
    ...
    const provenance = await readProvenance(tree);
  ...
  it("lets one pack cover a whole T1 coherent group", async () => {
    ...
    expect(layout).toContain(
      "Every member row's `Review pack seal` at gate item 10 is therefore the same seal over the same `review-<timestamp>/` directory **for that round**.",
    );
  ...
  it("sends the Integration checkpoint evidence to the same file item 12 rechecks", async () => {
    ...
    expect(checkpoint).toContain(
      "row is `E2E` / `API` / `Integration`, where the implement file was never created",
    );
  });
  ...
  it("sends the Integration cross-spec obligation to the layer-owned file too", async () => {
    ...
    expect(crossSpec).toContain("Naming only two of them splits one row's record across two files");
  });
  ...
  it("keeps a marked legacy row's cross-spec obligation where the prohibition reads", async () => {
    ...
    expect(crossSpec).toContain("Read the marker first, then the `Layer`");
  });
  ...
  it("requires the sibling on an Integration row the ATDD stage never handed over", async () => {
    ...
    expect(notObservable).toContain(
      "**`Integration` sits with `E2E` and `API`, not with `Unit` and `Component`**",
    );
  });
  ...
  it("sends a marked legacy row's review-fix round to the file its evidence lives in", async () => {
    ...
    expect(reviewFix).toContain(
      "A `#### Round N` block **nested inside the row's own `### TDD-NNNN` section** under " +
        "`## Ledger rows advanced` in `.qfai/evidence/atdd-<spec-id>.md`",
    );
    expect(reviewFix).toContain("round `N+1` where `N` is the round the reviewer ruled on");
