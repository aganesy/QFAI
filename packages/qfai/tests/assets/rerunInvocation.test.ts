...
describe.each(QFAI_TREES)("%s", (tree) => {
  ...
  it("gives qfai-sdd the contract-scoped target the protocol names", async () => {
    ...
    expect(sdd).toContain(
      "Stage 0 + Phase 0 (Contracts-first) + Phase 2c (Obligation reconciliation, limited to the existing `BR` / `AC` of the specs that reference any contract this run touched) + Phase 4 (Delta update) only",
    );
  });
