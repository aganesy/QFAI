...
describe("the ATDD estimator table's Signal column has a definition", () => {
  for (const tree of QFAI_TREES) {
    ...
    it(`${tree}: a DB contract obligation lands in the Integration numerator`, async () => {
      ...
      const skill = await read(tree, SKILL);
      // Qualified the same way `volume-signals.md` states it in the two
      // assertions above: `QFAI-ATDD-115` fires on the active contracts only,
      // so a skill line reading "the `CON-DB-*` this spec references" sizes the
      // row for tests the slice must not write.
      expectPhrase(skill, "plus the **active** `CON-DB-*` this spec references");
      // `#TC` is the whole numerator, the contracts included, so the Raw count
      // column names it alone. Adding them beside it counted each one twice.
      expect(flat(skill)).toContain("| Integration | #TC | INT_s |");
    ...
    it(`${tree}: a deferred contract is excluded from every numerator`, async () => {
      ...
      expectPhrase(
        skill,
        "a contract deferred with `x-qfai-status: planned` — `-- x-qfai-status: planned` for a `CON-DB-*` — owes no test, so exclude it from the count and name it in `Notes`",
      );
    });
