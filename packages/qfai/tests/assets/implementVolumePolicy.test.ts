...
describe("qfai-implement scales its ceremony to ledger volume", () => {
  for (const relativePath of SKILLS) {
    it(`${relativePath}: defines risk tiers with a stated default`, async () => {
    ...
    it(`${relativePath}: permits batched review with a bounded unit`, async () => {
      const section = volumePolicy(await readFile(path.join(repoRoot, relativePath), "utf-8"));
    ...
    it(`${relativePath}: allows a sequential multi-spec queue without enabling parallelism`, async () => {
      const content = await readFile(path.join(repoRoot, relativePath), "utf-8");
      const section = volumePolicy(content);
    ...
    it(`${relativePath}: makes the gate-cycle cost visible before processing`, async () => {
      const section = volumePolicy(await readFile(path.join(repoRoot, relativePath), "utf-8"));
