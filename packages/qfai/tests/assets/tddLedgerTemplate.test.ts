...
const deletedTcRow = (tddId: string, status: string): string =>
  `| ${tddId} | TC-0009 | Unit | T1 | tests/deleted.test.ts | deleted | ${status} | - | - | - | - | - | - | - |`;
...
describe("tdd/test-list.md has a shipped template and a named producer", () => {
  for (const tree of QFAI_TREES) {
    ...
    it(`${tree}: the column's requiredness matches the schema SSOT`, async () => {
      ...
      // Requiredness is asserted in the SSOT alone now: main's template stopped
      // restating the column table ("a second copy has nothing keeping it
      // honest"), so the template carries the DERIVATION — what Phase 2b does —
      // and `spec-traceability-rules.md` carries what the column IS.
      const template = (await read(tree, TEMPLATE)).replace(/\s*\n\s*/g, " ");
      expect(template).toContain("the T1 review-group key");
      expect(template).toContain("keep the **lowest-numbered** `BR-*`");

      const ssot = (
        await read(tree, "assistant/skills/qfai-sdd/references/spec-traceability-rules.md")
      ).replace(/\s*\n\s*/g, " ");
      // The optional set grew on main, so `BR-Ref` joins its end rather than
      // sitting next to `Blocked-By`.
      expect(ssot).toContain("`Tier`, `BR-Ref`");
    ...
    it(`${tree}: the shared falsifiability exception covers an Integration row`, async () => {
      ...
      // The layer alone is only the usual proxy for the condition: two kinds of
      // `Integration` row are never handed over, and both still require a
      // sibling `TDD-NNNN` in `Satisfied-by`.
    ...
    it(`${tree}: the template routes to the schema instead of restating it`, async () => {
      // The retirement paragraph needs a list of legal statuses to say which
      // one `retired` is not. A second copy of that list in the template has
      // nothing keeping it honest — it drifts away from the rules
      // `validateTddList` enforces, and the author filling the ledger reads the
      // copy. So the template names the one SSOT and holds no table.
      const template = await read(tree, TEMPLATE);
      expect(template).toContain(
      ...
      expect(template).not.toMatch(/^\| Status +\|/m);

      // And the statuses the SSOT lists are the ones the validator accepts.
      const rules = await read(
        tree,
        "assistant/skills/qfai-sdd/references/spec-traceability-rules.md",
      );
      for (const status of [
        ...
        expect(rules, `the schema SSOT no longer lists \`${status}\``).toContain(`\`${status}\``);
        ...
        expect(
          ...
          `the schema documents \`${status}\` but validateTddList rejects it`,
        ).toHaveLength(0);
      }
    });
    ...
    it(`${tree}: the retirement record follows the approval, not a fixed artifact`, async () => {
      ...
      // is not.
      expect(unwrap(skill)).toContain("the deletion is recorded in whatever authorised it");
    });
    ...
    it(`${tree}: the deleted row's Evidence pointer still resolves after the row goes`, async () => {
      ...
      // managed block ignores `.qfai/evidence/*` and then re-includes
      // `implement-<spec-id>.md` and `atdd-<spec-id>.md` by name, so the file
      // the `Evidence` cell points at is committed. The record therefore
      // carries the pointer, and the body stays where it was written.
      ...
      expect(template).toContain("re-includes both by name");
      ...
      // The CR is one of those records, so its enumeration and its
    ...
    it(`${tree}: the record cites the evidence rather than copying it`, async () => {
      ...
      // managed block commits. Copying the section under the record would
      // duplicate an audit trail that already survives the row, and on the
      // cross-spec path the copy is what the shared-scope scan rejects.
      const template = await read(tree, TEMPLATE);
      expect(template).toContain("The body stays where it was written");
      expect(template).not.toContain("the body of the `### TDD-NNNN` section it points at");

      const checklists = await read(
        tree,
        "assistant/skills/qfai-sdd/references/sdd-phase-checklists.md",
      );
      expect(checklists).toContain("re-includes both by name, so it still resolves");
      expect(checklists).not.toContain(
        "and with it the body of the `### TDD-NNNN` section that cell anchors to",
      );

      const cr = await read(tree, "assistant/skills/qfai-sdd/templates/change-request.md");
      expect(cr).toContain("Copy the `Evidence` cell in verbatim");
      expect(cr).not.toContain("paste the body of the `### TDD-NNNN`");
    });
    ...
    it(`${tree}: a cross-spec retirement keeps the evidence body out of _policies`, async () => {
      ...
      // `CON-API-ref` mandatory. The `Rationale` cell carries the pointer; the
      // body is never copied there.
      const template = await read(tree, TEMPLATE);
      expect(template).toContain(
        "**Nothing beyond the pointer goes into `_policies/10_delta.md`.**",
      );
      expect(template).toContain("`QFAI-LAYER-100`");
      expect(template).toContain("`TRACE_SHARED_SCOPE_VIOLATION`");
      expect(template).toContain("leave the section it names in\nthe evidence file");

      const checklists = await read(
        tree,
        "assistant/skills/qfai-sdd/references/sdd-phase-checklists.md",
      );
      expect(checklists).toContain("Never paste an evidence body into `_policies/10_delta.md`");
    ...
    it(`${tree}: every Triage row the guidance retires under is one Triage approves`, async () => {
      ...
      expect(unwrap(approvalPass)).toContain("deleting a ledger row is operator-approved");
    ...
    it(`${tree}: a row retired before it ran records "no evidence" instead of inventing one`, async () => {
      ...
      const template = await read(tree, TEMPLATE);
      expect(unwrap(template)).toContain(
        "**A row that never ran has no such section — read the cell to know.**",
      );
      expect(unwrap(template)).toContain(
        "no evidence — retired at Status = <status>, never executed",
      );
      expect(unwrap(template)).toContain("Never compose a section so the record has something");
      ...
      expect(unwrap(template)).toContain("**The status alone does not settle it.**");
      expect(unwrap(template)).toContain("its `Blocked-By` names the status it left from");

      const checklists = await read(
        tree,
        "assistant/skills/qfai-sdd/references/sdd-phase-checklists.md",
      );
      expect(checklists).toContain("rather than composing a section for it to name");

      const rules = await read(
        tree,
        "assistant/skills/qfai-sdd/references/spec-traceability-rules.md",
      );
      expect(unwrap(rules)).toContain("anchors nothing: record `no evidence");
