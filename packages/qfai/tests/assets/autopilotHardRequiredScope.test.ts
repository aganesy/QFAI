 ...
 *
 * `## Default Autopilot Policy` listed `primarySpecId` (when absent from
 ...
 * `## Spec Auto-Discovery Protocol` > `### User Selection Flow` covers exactly
 * that condition and lets the single-spec branch announce and proceed.
 *
 * The bucket entry now names the carve-out the two other branches already
 * implement, and `hard-required` gets a definition beside the buckets' only
 * other home so the next collision is decidable from the text.
 */
...
describe.each(QFAI_TREES)("%s", (tree) => {
  it("scopes the hard-required entry out of the branch auto-discovery settles", async () => {
    const skill = await read(tree, SKILL);
    expect(skill).toContain(
      "`primarySpecId` (when absent from inputs and auto-discovery does not resolve exactly one candidate",
    );
    expect(skill).toContain("one candidate — see `## Spec Auto-Discovery Protocol`)");
  });

  it("leaves no unqualified hard-required entry behind to be read instead", async () => {
    // The collision was textual: whichever section the agent read last won.
    const skill = await read(tree, SKILL);
    expect(skill).not.toContain("`primarySpecId` (when absent from inputs)");
  });

  it("keeps the single-spec branch as the rule the bucket now defers to", async () => {
    const skill = await read(tree, SKILL);
    expect(skill).toContain("Single spec: announce the detected spec");
  ...
  it("defines hard-required where the buckets' other home already is", async () => {
    ...
    expect(baseline).toContain(
      "`hard-required` = the run may not start until the value is known, and the agent MUST obtain it from the user unless the bucket entry itself names the derivation that supplies it",
    );
  });
