...
describe("operator-facing CLI message language", () => {
  ...
  it(
    ...
    async () => {
      ...
      const files = await fg("**/*.ts", { cwd: SRC_DIR, absolute: true });
      const offenders: string[] = [];
      for (const [rel, source] of await readSources(files, SRC_DIR)) {
        source.split(/\r?\n/).forEach((line, index) => {
          if (!line.includes("npx qfai ")) {
            return;
          }
          // Comments explain the implementation and are not read by an
          // operator; the generated-file header is documentation in the
          // reader's tree, so it takes the documentation rule.
          const trimmed = line.trimStart();
          const isComment =
            trimmed.startsWith("*") || trimmed.startsWith("//") || trimmed.startsWith("/*");
          if (isComment || line.includes("# Generated by")) {
            return;
          }
