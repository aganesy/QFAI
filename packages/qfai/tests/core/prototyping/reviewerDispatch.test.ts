 ...
 */
/**
 * Comments blanked, literals kept — a single shared reduction.
 *
 * Two independent `replace` passes are not safe here: each delimiter
 * occurs inside the other's body, so whichever regex ran first would
 * read the other's content as its own opener. On this file's ONE
 * subject, `cli/commands/prototypingIterate.ts`, that shape of bug
 * removes **11,381 characters of non-comment text** and loses 91
 * identifiers — among them `resolvedCaptureScreens`, adjacent to the
 * very token asserted against below.
 *
 * The assertion here is a NEGATIVE, so over-deletion makes it easier to
 * pass: this guard could go quiet without ever reddening.
 */
