...
describe("traceability helpers", () => {
  it("collects SC ids from scenario files", async () => {
    ...
    expect(Array.from(result).sort()).toEqual(["SC-0001-0001"]);
  });
  ...
  it("collects SC sources from scenario files", async () => {
    ...
    const sc0001 = Array.from(sources.get("SC-0001-0001") ?? []).sort();
  ...
  it("collects SC references from tests", async () => {
    ...
    const sc0002 = Array.from(refsResult.refs.get("SC-0001-0002") ?? []).sort();
  ...
  it("builds SC coverage summary", () => {
    const refs = new Map<string, Set<string>>([
      ["SC-0001-0001", new Set(["tests/alpha.test.ts"])],
      ["SC-0001-0002", new Set()],
    ]);

    const coverage = buildScCoverage(["SC-0001-0001", "SC-0001-0002"], refs);
    expect(coverage.total).toBe(2);
    expect(coverage.covered).toBe(1);
    expect(coverage.missing).toBe(1);
    expect(coverage.missingIds).toEqual(["SC-0001-0002"]);
    expect(coverage.refs["SC-0001-0001"]).toEqual(["tests/alpha.test.ts"]);
    expect(coverage.refs["SC-0001-0002"]).toEqual([]);
  });
});
