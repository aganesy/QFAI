...
describe("the self-governance drift detectors are reported by their precondition", () => {
  ...
  it("keeps them off the list once the package source tree is present", async () => {
    await withProject(async (root) => {
  ...
  it("runs the detectors only where the package source tree exists", async () => {
    await withProject(async (root) => {
      // Asymmetric Pair III edit: probe side declares the canonical token,
      // schema side does not. Without the gating tree there is no signal.
      ...
      for (const [rel, body] of [
        [pair.probeImplRel, `export const x = ${pair.probeToken};\n`],
        [pair.schemaRel, "export const y = 1;\n"],
      ] as const) {
