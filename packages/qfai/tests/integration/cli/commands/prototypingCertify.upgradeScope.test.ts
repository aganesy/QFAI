...
/**
 * Seed the "all gates pass" signal at `.qfai/output/validate-saas-package.json`
 * so the upgrade-scope impl can confirm the saas-package-skipped gates now
 * pass. The signal is a validate-style record with `counts.error: 0` plus
 * an explicit per-gate result map keyed by `SAAS_PACKAGE_SKIPPED_GATES`.
 */
