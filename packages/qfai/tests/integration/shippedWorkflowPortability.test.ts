...
describe("TC-0003-0044 (TDD-0044): absent packageManager field fails closed with an actionable annotation", () => {
  ...
  it("the failure annotation names the package.json packageManager field as the fix site, and nothing else authors on that channel", async () => {
    for (const job of await installJobs()) {
      ...
      expect(output, `${job.file}: the stop emitted no ::error:: annotation`).toContain("::error");
      expect(
        output,
...
describe("TC-0003-0053 (TDD-0053): version file plus packageManager field is the non-degrading happy path", () => {
  ...
  it("the install succeeds through the lockfile's own branch and the lane result follows it", async () => {
    for (const job of await setupInstallJobs()) {
      const run = await runShell(installBody(job), await nonDegradingTree(), PACKAGE_MANAGER_STUBS);
      expect(
        run.status,
  ...
  it("neither degrade clause fires on the happy path, and both fire on the degraded control", async () => {
    for (const job of await setupInstallJobs()) {
      ...
      expect(happyGuard.status, `${job.file}: the fail-closed clause fired on the happy path`).toBe(
        0,
      );
      expect(
      ...
      const degradedGuard = await runShell(requiredBody(job, PACKAGE_MANAGER_STEP_ID), degradedDir);
