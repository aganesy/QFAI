...
function project(): Promise<string> {
  projectPromise ??= (async () => {
    const dir = await mkdtemp(path.join(os.tmpdir(), "qfai-int-check-independence-"));
...
async function jobsOf(file: string): Promise<Record<string, Record<string, unknown>>> {
  const text = await readFile(path.join(await project(), ".github", "workflows", file), "utf-8");
...
async function runStep(body: string, cwd: string, env: NodeJS.ProcessEnv = {}): Promise<StepRun> {
  const stage = await mkdtemp(path.join(os.tmpdir(), "qfai-int-check-independence-step-"));
  try {
