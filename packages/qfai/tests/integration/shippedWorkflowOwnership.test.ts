...
describe("TC-0003-0052 (TDD-0052): pruneMatchingEntries is exported and receives a retired-name predicate", () => {
  ...
  it("does not prune through a workflows directory it refused to write through", async () => {
    // `workflowsDirIsOwn` must exclude more than just the COPY, or a
    // `.github/workflows` that is a link to a shared directory or another repository would still
    // be enumerated by the retired prune — and a retired workflow on the far side whose bytes
    // match a recorded digest would be quarantined and deleted. The run that refused to write
...
describe("TC-0003-0051 (TDD-0051): declined name is excluded from the copy set before the copy runs", () => {
  ...
  it("single state derivation: init.ts reads the provenance record exactly once (the resolver must consume the pre-init capture)", async () => {
    ...
    expect(readCallSites, "init.ts must contain exactly one readInstallProvenance call site").toBe(
      1,
    );
  });
});
...
describe("TC-0003-0048 (TDD-0048): write and removal path contains no filesystem call of its own", () => {
  ...
  it("no filesystem mutation call of its own: the workflow-path functions and the runInit workflows segment use the primitives only", async () => {
    ...
    expect(end, "runInit must aggregate removals after the workflows prune").toBeGreaterThan(start);
    ...
    for (const callee of [
      ...
      expect(segment, `the workflows segment must route through ${callee.slice(0, -1)}`).toContain(
        callee,
      );
    }
  });
  ...
  it("the collision, declined and modified fixtures are processed through the primitives with the contract outcomes", async () => {
    ...
    await expect(observeFixtureState(collisionDir, packagedDigest)).resolves.toBe("adopter-owned");
...
describe("the workflows parent is pinned across the copy, not only before it", () => {
  ...
  it("captures the ancestors' identity before the copy and compares it after", async () => {
    ...
    // The copy that writes the WORKFLOWS, which is separated from the copy of
...
describe("a run with nothing to copy is not a directory swap", () => {
  ...
  // Treating an absent component as a refusal would turn that ordinary no-op into a reported
  // swap, telling the operator their workflows may have been written outside the repository when
