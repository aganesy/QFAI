...
export const ISSUE_EXPECTED_BY_CODE: Record<string, string> = {
  ...
  "QFAI-TDDLIST-019":
    "A `done` row whose evidence entry declares its original run's output unretained is exempt from the reviewer-pack and seal fields, and says so here: `done` is read as reviewed, so a row complete without the verdicts stays visible rather than passing as one that has them.",
