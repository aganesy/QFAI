...
export const ISSUE_EXPECTED_BY_CODE: Record<string, string> = {
  ...
  "QFAI-SKILLS-015":
    "Every skill carries a `description:` with text in it, which is what a host reads to register it — a skill that should not be offered to the model declares `disable-model-invocation: true` and keeps the description, rather than dropping the field and losing the registration with it.",
