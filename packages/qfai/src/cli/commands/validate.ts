 ...
 * calls it.
 *
 * Entries are prefix globs, never bare codes typed out by hand: a gate that
 ...
 * gates whose emitted codes it scans. A group may instead spread the same
 * constant its validator filters on (`tdd-ledger-seed` below) — that is the
 * tighter form of the rule, not an exception to it: the list cannot drift from
 * what the gate evaluates because it is that list.
 */
export const GATE_GROUP_FAMILIES = {
  ...
  tdd: ["TDDLIST_* (execution state)", "QFAI-TEST-*", "QFAI-TRACE-*"],
...
const PROFILE_GATE_GROUPS: Record<ValidationProfile, readonly GateGroup[]> = {
  ...
  sdd: ["sdd", "package-self-governance", "review-artifacts", "tdd-ledger-seed"],
