...
function buildProjectSteeringEntryTemplate(): string {
  ...
  return [
    ...
    // reports `_template/entry.md differs from the seed this qfai release generates`
    ...
    "kind: decision # required; see .qfai/assistant/rule/worklog-entry.schema.md",
...
async function seedProjectSteering(
  ...
  const targets: Array<{ rel: string[]; body: string; derived: boolean }> = [
    ...
    { rel: ["_template", "entry.md"], body: buildProjectSteeringEntryTemplate(), derived: true },
  ];
...
function buildCopilotInstructions(): string {
  return [
    ...
    "  - AI work-log surface (per-project): `.qfai/steering/` (entry frontmatter schema: `.qfai/assistant/rule/worklog-entry.schema.md`)",
