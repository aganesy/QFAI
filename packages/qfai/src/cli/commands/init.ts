...
export async function runInit(options: InitOptions): Promise<void> {
  ...
  const gitignoreUpdated = await ensureRootGitignoreEntries(destRoot, options.dryRun);
  ...
  report(
    [
      ...
      ...(gitignoreUpdated ? [path.join(destRoot, ".gitignore")] : []),
    ],
    [
      ...rootResult.skipped,
      ...qfaiResult.skipped,
...
/**
 * Append QFAI ignore entries to the root `.gitignore`.
 * Idempotent — skips if the marker comment is already present.
 * Returns `true` when the file was created or modified.
 */
async function ensureRootGitignoreEntries(destRoot: string, dryRun: boolean): Promise<boolean> {
