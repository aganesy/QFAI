...
async function syncIntegrationWrappers(
  ...
  // No README is written into the integration directories. Each one restated
  // the package README's agent-integrations section into a file the adopter now
  // owns and no release refreshes, and the shipped skills already forbid the
  // shape: artifact guidance belongs in a skill's references and templates,
  // which are loaded, rather than in scaffold documentation, which is not.
