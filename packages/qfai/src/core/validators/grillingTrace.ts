...
/** Files under `.qfai/discussion/` whose presence means a pack was authored. */
 ...
 */
async function specStageFindings(root: string, config: QfaiConfig): Promise<Issue[]> {
  const evidenceDir = resolvePath(root, config, "evidenceDir");
...
async function discussionFindings(root: string, config: QfaiConfig): Promise<Issue[]> {
  ...
  for (const pack of await entriesOf(discussionDir)) {
    ...
    const relPath = path
      .relative(root, path.join(discussionDir, pack))
      .replace(/\\/g, "/");
...
export async function validateGrillingTrace(
  ...
  return [...(await specStageFindings(root, config)), ...(await discussionFindings(root, config))];
}
