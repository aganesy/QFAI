...
export async function validateSpecSections(root: string, config: QfaiConfig): Promise<Issue[]> {
  ...
  const specSectionsSeverity = "error";
  ...
  const issues: Issue[] =
    invalid.length > 0 ? [invalidSectionsIssue(invalid, specSectionsSeverity)] : [];
  ...
  for (const entry of entries) {
    ...
    issues.push(missingSectionsIssue(entry.dir, missing, specSectionsSeverity));
  }
  return issues;
}
