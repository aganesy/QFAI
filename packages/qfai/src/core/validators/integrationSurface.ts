    ...
    issues.push(
      issue(
        ...
        [
          ...
          "On Windows, `git worktree add` writes these wrappers as FILE symlinks where the main checkout has DIRECTORY symlinks, and the OS will not follow one to a directory. Nothing else about the tree is wrong, which is why a content check finds it healthy. `qfai init --force` rewrites them, and `qfai doctor` reports the same condition under `integration.links`, so the diagnostic and this gate name the same tree.",
