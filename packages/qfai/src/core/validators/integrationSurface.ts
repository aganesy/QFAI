 ...
 * dirs are pruned by resolved target under `--force`, but the skill dirs are
 * still matched by a `qfai-` prefix, and `web-research` is the standing proof
 * that a shipped name need not have one.
 *
  ...
  const issues = [
    issue(
      ...
      [
        ...
        "**`which this version does not ship` は退役した wrapper です。** アップグレードで削除・改名された skill / agent の wrapper が残っており、解決できてしまうため assistant は今も旧命令を読み込みます。`qfai init --force` は `.qfai/assistant/` 配下へ解決する wrapper のうち現行 roster に無いものを削除するので、まずこれを実行してください（`--force` なしの再実行では消えません）。**canonical 側（`.qfai/assistant/skills/…` / `.qfai/assistant/agents/…`）は init が削除しません。** プロジェクトが独自の skill / agent をそこへ追加している場合と区別できないためで、退役した canonical を消したいときは手で削除してください。プロジェクト独自の canonical に対して手で貼った wrapper も同じ形になります — その場合も qfai の管理外なので、意図的に残すかどうかを決めてください（`--force` は解決先が canonical roster に無ければ削除します）。",
