...
export const QFAI_GITIGNORE_RECOMMENDED_ENTRIES: readonly string[] = [
  ...
  // Spelled unanchored on purpose: this list is matched with
  // `content.includes(entry)` over the whole file, so it is satisfied by the
  // anchored `/tmp/` the managed block writes as well as by a project's own
  // `tmp/` line.
  "tmp/",
] as const;
