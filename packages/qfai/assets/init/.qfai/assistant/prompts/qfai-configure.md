...
These principles are inspired by Бgconstitution / articlesБh patterns used by other agent frameworks, but tailored to QFAI.
...
2. **Traceability is mandatory**  
   Every meaningful change must be traceable: **Require Би Spec Би Scenario Би Tests Би Code Би Verification evidence**.
...
4. **Minimize scope, but never hide gaps**  
   Keep changes minimal, but do not Бgpaper overБh missing decisions. If something blocks correctness, stop and ask.
...
**All outputs MUST be written in the userБfs working language for this session.**
...
This workflow assumes the environment _may_ support subagents (e.g., Claude Code БgTaskБh tool) or may not.

### If subagents are supported

Delegate to multiple roles and then merge the results. Use a Бgreal?world workflowБh order:

- Facilitator Би Interviewer Би Requirements Analyst Би Planner Би Architect Би (Contract Designer) Би Test Engineer Би QA Engineer Би Code Reviewer Би DevOps/CI Engineer
...
- Write a short Бgrole outputБh section per role, then consolidate into the final deliverable(s).
