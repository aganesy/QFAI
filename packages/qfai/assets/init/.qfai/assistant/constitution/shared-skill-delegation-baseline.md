...
- **How to compute it, exactly** — one procedure, four steps, stated once in
  `references/audited-evidence-hash.md` and run by the reviewer and by gate
  item 10 alike. The subject is part of a file, so a file-level manifest alone
  is ambiguous: that file names the per-observation subjects, the
  normalization, the record form, and the per-member rule for a T1 coherent
  group.
