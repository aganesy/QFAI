...
When validate, doctor, test, lint, typecheck, build, capture, or report gates
fail — **or when a blocking reviewer returns `FAIL` / `REVISE`**:
...
- stop for destructive changes, ambiguous product/spec decisions, missing
  permissions/tools, or repeated no-progress failures;
- stop on **round count** as well as on lack of progress: a reviewer gate that
  would enter its third round escalates to the user, even when every round has
  made progress. See
  `shared-skill-delegation-baseline.md#round-budget-must`.
