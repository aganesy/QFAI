...
This file is the SSOT for the layer set below: `npx qfai validate` reads the
layers from here and applies them to two checks — `QFAI-EX-005` on the legacy
spec-pack layout, and `QFAI-EX-105` on the layered layout `npx qfai init`
produces. Until both consumed it, the file was read, reported on, and then
ignored on every modern project.
