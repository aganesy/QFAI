...
`.qfai/prototype/iter-NN/index.html` but is not rendered into the
...
backed up: `.qfai/prototype/iter-00/` is left in place and your next
...
| ------------------------------------- | -------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------- |
| `.qfai/prototype/iter-NN/`            | one `index.html`                                         | **the generator** (you)                                                                                                                                   | `--auto-serve`, the operator, `/qfai-implement` |
...
declared UI contract screens reachable" above means operationally.
...
- The fallback needs an `index.html` to fall back _to_, and it has to
  ...
  `.qfai/prototype/iter-NN/` and nothing else. `--emit-skeletons` does
  ...
  `.qfai/prototype/iter-NN/` and an `index.html` alongside them: the
...
not `.qfai/prototype/`. Do not serve or look for them under
`prototypes/`. The `htmlSourceCopy` capture option likewise operates on
per-screen files, but reads them from `.qfai/prototype/iter-NN/`.
Neither writes an `index.html`.

An accepted iteration must still carry `iter-NN/index.html`: Handoff
below copies it to `.qfai/prototype/final/index.html`, and without it
...
`.qfai/prototype/final/index.html` is a copy of the accepted
