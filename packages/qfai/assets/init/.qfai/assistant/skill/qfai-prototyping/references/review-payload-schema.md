...
parsable payload for every (UI contract, screen) pair, while a Reviewer
...
1. The file exists for every (UI contract, screen) pair in the frozen set.
2. It parses against this closed schema.
3. Its `uiContractId` / `screenId` / `cycle` match the pair and the accepted
...
frozen set holds one UI contract or several: a single-contract run does not get a
