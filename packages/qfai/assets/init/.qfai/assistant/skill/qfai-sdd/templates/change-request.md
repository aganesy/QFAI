...
| ---------- | ------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------- | ------ |
| `DEC-NNNN` | `Change request: <authorized paths or IDs>; <conflicting fact and source reference>` | `<proposed change, reason, affected downstream work, and owner rerun or validation required>` | `TODO` |
...
- Describe the actual contradiction and its source in `Content`. In `Approach`, state the proposed correction, affected work, and the gate that proves it is applied. Keep both cells concise enough to review as a table row.
