...
The mandatory roles in the routing entry are minimum participation, not exclusive ownership. `requirements-analyst` contributes to both triage and requirement-aligned story drafting. `test-design-analyst` checks observable outcomes and traceable test obligations during design. A UI-bearing target also routes the configured experience and surface roles. Follow the current routing manifest for the exact conditional agent set and review profile.

For `--contract <CON-ID-or-path>`, scope design to the named contract, its paired contracts, and the existing flows whose AC, EX, or BR depend on them. Reconcile those obligations after each contract change. Record the review and gate in each affected BF's evidence. If no BF exists, record the incomplete stage in the work log and do not claim DONE. No previous triage approval can substitute for approval of a new change-request row.
