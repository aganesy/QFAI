...
- Before writing or editing any `.qfai/**` artifact, read the relevant skill-local reference or template:
  ...
  - `.qfai/assistant/skill/qfai-prototyping/references/iteration-loop.md`

## Inputs Priority (Preflight)

When unsure, read inputs in this order:

- P1: `.qfai/assistant/rule/*`, and `.qfai/assistant/skill/qfai-grilling/SKILL.md` before the confidence check, so it is loaded for a preflight round and for any session detection opens later (see Grilling)
...
- P3: `.qfai/spec/decisions.md` (Decision Records; if none apply, state "not applicable").
- P4: relevant business flows, stories, contracts, tests, and evidence.
...
- **Prototyping scope.** When `/qfai-verify` satisfies the prototyping DONE gate before `npx qfai prototyping certify`, run `npx qfai validate --profile prototyping --fail-on error` and write `scope: "prototyping"`. The certificate accepts only that scope. Later BF, AC, and EX coverage belongs to the ATDD or full stage and must not be fabricated to pass this earlier gate.

## Verify Output Contract — `.qfai/report/verify.json`

`/qfai-verify` MUST write `.qfai/report/verify.json` at the end of the run: it is
the machine-readable verdict, and `.qfai/evidence/verify-<run-id>.md` does not
...
- The stage evidence's `## Grilling Session` section carries `Run started`,
  ...
  and each row's `Open` count matches the register lines naming that row's `Session`, **and its `Escalated` count matches the escalation lines naming it**, **and its `Decisions` count matches the `grilling(<Session>@<run key>/<adjudication>)` rows the Work Orders Summary carries under that `Session` and this run's start** — a count checked against nothing lets a row claim `0` over decisions it took. **And the other way: every `grilling(<Session>@<run key>/<adjudication>)` row the summary carries for this run names a `Session` exactly one row of this block holds.** Counted only from the rows inward, a summary row keyed to a session the block never opened is checked by nothing, so the record can claim a decision for a session that did not exist — and, beside the run's `none` marker, claim in the same breath that the run settled none.
  ...
  **A row whose ending lets the work go on, and after which the stage wrote,
  ...
  writes `none — <why>` instead**, which the gate accepts on those four
...
- **Each ending carries its own condition, and the name alone is not one.** A
  ...
  | `adopted`     | `Frontier` empty, `Lookups` none in flight and `Open` 0 — so every critical decision the session escalated has the user's answer, which is when the rule master ends a delegated session `adopted`. **Every decision the row counts that no escalation line settled is a `grilling(<Session>@<run key>/agents)` row whose `Output (refs)` carries the reason and each disagreeing position.** An adopted decision missing that row is a `REVISE`, and so is an `agents` row whose decision is critical. An adopted decision that is not critical is not a finding, whatever the reviewer would have chosen: doubt about its merit is an ordinary finding against the artifact                                                                                                                                                                                                                                                                                 |
...
- Reviewer checks:
  ...
  - validate evidence exists: `npx qfai validate --profile verify --fail-on error` completed with `error=0`, or the `prototyping` profile did so for a prototyping run. The reviewer checks the profile matches the declared `verify.json` scope;
  - for a prototyping-scoped run, per-iter evidence (screenshot + HTML + review.json) exists under `.qfai/evidence/prototyping/iter-NN/`, and the recorded final iteration in `.qfai/evidence/prototyping/prototyping.json#iterations[]` has both screenshot and HTML on disk. The completion certificate is not a verify-gate input — `npx qfai prototyping certify` runs after `/qfai-verify` and reads its passing `.qfai/report/verify.json`. Cert digest validation belongs to `certify --check` during handoff or after brand asset edits;
...
Article IX of `.qfai/assistant/rule/constitution.md` owns both sessions
...
- **Neither session changes settled input.** Where one concludes that settled
  input must change, `.qfai/assistant/rule/drift-protocol.md` governs:
...
- **Record both sessions where the gate reads them.** The method writes no
  ...
  same tree. `.qfai/evidence/verify-<run-id>.md` carries a `## Grilling Session`
  ...
  | ---- | ---------------- | -------------- | ---------- | ------------ | ------------- | ---------------------------- |
  | 2 | <role> | <instance> | grilling(S1@2026-01-01T09:02:00.417Z/agents): <a decision S1 adopted> | <refs> | <refs>; <why it was taken; any position that disagreed> | PASS |
  | 3 | <role> | <instance> | grilling(S1@2026-01-01T09:02:00.417Z/agents): <another decision S1 adopted> | <refs> | <refs>; <why it was taken; any position that disagreed> | PASS |
  ...
  stage's lone `Authoring began`: this stage holds more than one session, so a row says which, and the preflight session's says `preflight`. `Work resumed` is when the stage next wrote — the first gate result for the preflight session, the
  ...
  **One block per stage as well as per run**, because two stages share an
  ...
  the ATDD evidence file, which `/qfai-atdd` wrote its own sessions into and
  ...
  **`Preflight` says whether the confidence check opened a session.** Article IX
  ...
  `Ended` is `confirmed`, `user-closed`, `adopted`, `no-question` or `stopped`
  — the five endings `.agents/rules/grilling.md` names — and only the first four
  let the work go on; a delegated session that completes ends `adopted`.
  ...
  **`Decisions` counts what the session settled, and each has a row that
  ...
  one the session adopted; an open node and an unanswered escalation do not.
  Each counted decision is a `grilling(<where>/<adjudication>): <the decision>`
  row in the stage's Work Orders Summary
  (`.qfai/assistant/rule/shared-skill-delegation-baseline.md`), so the
...
- **The escalated decisions go under that table too.** One line per critical
  decision a session sent to the user, naming the `Session` it came from
  ...
  escalation nobody answered is a decision nobody took. `Escalated` counts these
  lines, and the gate reconciles the two the way it reconciles `Open` with the
  register.
...
- You MUST produce the required evidence file: `.qfai/evidence/verify-<run-id>.md`.
  - The run-scoped evidence stays local under the project's managed `.gitignore` block. Summarize its key outcomes in the PR description.
...
- A waiver's `rule:` is the finding's `issues[].code` in `.qfai/report/validate.json`, copied verbatim. The array is `issues`, not `findings`; see `references/validate-json-schema.md`.
...
- In a prototyping-scoped run, declared screens have screenshot and HTML evidence.
- A concise evidence summary exists (copy‑paste for PR).
- The PR-ready summary includes **Change Classification (Primary/Tags)** per `.qfai/assistant/rule/change-classification.md`.
- Evidence file exists: `.qfai/evidence/verify-<run-id>.md`.
...
- Static policy checks:
  ...
  - all `.qfai/assistant/skill/*/SKILL.md` include `[DRIFT-PROTOCOL:MANDATORY]`.
...
QFAI expects project-specific facts under `.qfai/spec/01_policy/` and `.qfai/spec/03_contract/` so design, testing, and implementation fit this repository.
...
- derive “what/why/users/success/non-goals” from README/docs/issues (`objective.md`)
- derive runtime/tooling versions + constraints + standard gate commands from the task-runner manifest (`package.json` scripts, `Makefile`, `justfile`, `pyproject.toml`, `Cargo.toml`, …), then CI config, then lockfiles (`03_contract/tech.md#standard-commands-copy-paste`)
- derive repo layout + key directories from the file tree and scripts (`03_contract/structure.md`)
...
- [ ] objective.md: what we build / users / success / non-goals / release posture
- [ ] 03_contract/tech.md: runtime / package manager / test / lint / CI constraints and Standard commands
- [ ] 03_contract/structure.md: repo layout, key packages, entrypoints, and local setup; gate commands stay in tech.md
...
`.qfai/spec/03_contract/tech.md#standard-commands-copy-paste` — that is the
section `/qfai-implement` reads, and a capability left without an entry there is
UNRUN. Resolve the missing command in the contract before claiming PASS.
...
- The repository's full verification gate uses the verify or default profile. The prototyping profile is a stage gate before certification; see "Verify Scope Rule".
...
- key errors/warnings
- file paths affected
- `QFAI-STORY-006` through `QFAI-STORY-009` findings for BF, AC, and EX obligations. BF requires E2E, AC requires integration or API, and EX requires a selected non-E2E test. Record uncovered IDs and the affected flow. `QFAI-SCAN-002` means the test scan cannot prove coverage; do not treat a missing layer as a passing scan.
...
- Change classification (SSOT: `.qfai/assistant/rule/change-classification.md`):
  - Primary:
  - Tags:
  - rationale (1-3 lines):
...
Create and update `.qfai/evidence/verify-<run-id>.md`. Evidence must include:
...
- `.qfai/evidence/verify-<run-id>.md` — the human-readable evidence summary with all gate results
...
- Referenced inputs: instructions, project policy and contracts, `decisions.md`, and applicable stories.
...
The message lists each adopted decision — every `grilling(<Session>@<run key>/agents)`
row — with its reason and any disagreeing position, and does not wait for an answer.
...
- Completion requires zero errors in the declared profile, zero leakage in the distributed-surface guard, and a clean branch version pin. A `scope: "prototyping"` run covers only the prototyping profile; BF, AC, and EX test obligations are checked in the later ATDD or full run.
