...
| `/qfai-migration-spec-to-story` | Migration plan and report fidelity, ID mapping, archive completeness                                | New story content and downstream implementation quality                                 |
...
- `Audited evidence hash` is required when a verdict audits ATDD or implementation evidence. The reviewer computes it over the phase-authored EX round or BF acceptance evidence, and the current Coverage Depth Matrix when applicable. Use `rule/audited-evidence-hash.md` for subjects, normalization and serialization. An orchestrator cannot compute the hash on the reviewer's behalf.
