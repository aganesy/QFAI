...
テストファイル走査が上限に達するか失敗すると、`QFAI-SCAN-002` が `error` の issue として出力される。通常の issue 行の文法に従い、集計にも含まれる。専用の警告行は出力されない。
...
1. `[info] ` / `[warning] ` / `[error] ` で始まる行 — 新しい issue のヘッダー行
2. `counts: ` で始まる行 — 集計行。issue 出力の終端
3. `fail-on: ` で始まる行 — 実効しきい値行。集計行の直後にのみ現れる
4. `timings: ` で始まる行 — 予算超過行。実効しきい値行の直後にのみ現れ、超過がなければ出力されない
...
メッセージ継続の終了条件は 1 / 2 / 6 のいずれか。しきい値に達しない severity では詳細ブロックが出力されないため 1 または 2 で終わる。
...
> **Note:** `<paths.contractsDir>/design/design-tokens*.yaml`（既定は `.qfai/spec/03_contract/design/`）は **optional supporting artifact** である。init 直後にファイルが存在しなくても異常ではなく、token validator は token file が作成された場合にのみ実行される。

## Message Language

operator 向けに qfai の CLI が出力する文字列は **英語** で書く — これは
qfai 自身が印字する文字列の規定であり、アシスタントの応答言語は固定しない
(それは `.qfai/assistant/rule/constitution.md` の Absolute Rule が
...
理由: rule code、`<paths.contractsDir>/cli/` 配下の CLI contract、および
