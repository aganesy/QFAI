...
1. **Observed RED (preferred).** Write the journey or API test **against the
   current tree**, before this cycle builds any surface it needs, and run it.
   An admissible failure is an assertion — or an expected-status check — inside
   this row's own selector; a 404 from an unregistered route, an import error or
   a fixture failure is a missing seam, not a RED. Record the command and output
   as the row's RED pair, then build the surface and re-run for GREEN. Stage gate
   **P1b** is where this happens.
2. **Falsifiability (surface already exists).** When the surface predates this
   cycle, the correct test passes first run. Do **not** weaken it to manufacture
   a failure. Use the shared path in
