...
row to this stage: an acceptance test is **this** skill's owned artifact and is
never edited in `/qfai-implement` (its Non-goals), whatever `agent-routing.yml`
routes into any phase there — the rule is ownership, not who happens to be
available, so it survives a routing change. What the corrected test owes — a fresh
