...
- ATDD-specific reviewer checks:
  ...
  - validation evidence exists and `npx --no-install qfai validate --profile atdd --fail-on error` passes;
...
- Completion gate is validation with zero errors (`npx --no-install qfai validate --profile atdd --fail-on error`).
...
- Validation passes: `npx --no-install qfai validate --profile atdd --fail-on error`.
...
2. Run:
   ...
   ```bash
   npx --no-install qfai validate --profile atdd --fail-on error
   ```
