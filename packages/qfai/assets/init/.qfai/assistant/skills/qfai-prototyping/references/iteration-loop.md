...
The last path is mandatory: the reviewer writes one payload per
`(spec, screen)` pair alongside the per-cycle `review.json` summary
(schema: `references/review-payload-schema.md`, aggregation rule:
`references/reviewer-prompt.md`). `npx qfai prototyping certify`
rejects the run (exit `64`) when a declared pair has no payload, so a
multi-spec run that only writes the flat summary cannot be certified.
