...
when `npx --no-install qfai prototyping iterate` returned exit 64 (convergence) or 65
(max-iterations).
...
Order is load-bearing: `npx --no-install qfai prototyping certify` requires
...
1. `npx --no-install qfai validate --profile prototyping --fail-on error` — writes
   `.qfai/output/validate.json`.
2. `/qfai-verify` — writes `.qfai/output/verify.json`.
3. `npx --no-install qfai prototyping certify` — produces
