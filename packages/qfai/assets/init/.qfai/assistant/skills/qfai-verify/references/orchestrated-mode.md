...
What `/qfai-verify` does when `qfai workflow` hands it a work order. The field
...
- The stage writes this run's `.qfai/report/verify.json` and names it in
  `artifactRefs`.
- A `verify.json` written by another run, for another spec or kept in a shared
...
These three are the only repair kinds.
