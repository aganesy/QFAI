...
- Reviewer checks:
  ...
  - validate evidence exists: `npx --no-install qfai validate --profile verify --fail-on error` completed with `error=0`;
  - per-iter evidence (screenshot + HTML + review.json) exists under `.qfai/evidence/prototyping/iter-NN/`, and the recorded final iteration in `.qfai/evidence/prototyping/prototyping.json#iterations[]` has both screenshot and HTML on disk. The completion-certificate is NOT a verify-gate input — `npx --no-install qfai prototyping certify` runs AFTER `/qfai-verify` (it requires a passing `verify.json`). Cert digest validation belongs to `certify --check`, run during the prototyping handoff or after edits to brand assets, not here;
...
- In CI, you MUST keep QFAI validation on full-scan mode (`npx --no-install qfai validate --profile verify --fail-on error` or default `npx --no-install qfai validate --fail-on error`). Do NOT use partial profiles.
...
- QFAI checks PASS (at minimum: `npx --no-install qfai validate --profile verify`, and optionally `npx --no-install qfai report`).
...
- `npx --no-install qfai validate --profile verify --fail-on error`
- `npx --no-install qfai report` (if used in this repo)
...
- Need a report artifact:
  Action: run `npx --no-install qfai report` after validation outputs are up to date.
