...
- Delegate: Analyst, Diagrammer create first drafts of requirements draft and business flow artifacts.
...
- `business-flows.md` MUST use Mermaid `sequenceDiagram`; legacy bullet `Steps:` format is not allowed.
...
- `business-flows.md` uses Mermaid `sequenceDiagram` with BF step IDs on message lines.
...
- `business-flows.md` has Mermaid `sequenceDiagram` and no legacy bullet `Steps:` format.
...
- Use Mermaid `sequenceDiagram` in `business-flows.md` and embed BF step IDs in message lines.
