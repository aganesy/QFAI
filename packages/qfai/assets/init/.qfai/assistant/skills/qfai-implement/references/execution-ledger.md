...
scope picks the one the id resolves against: a spec-scoped `DR-NNNN-MMMM` is
declared in that spec's `07_Decisions.md`, and a policy-level `DR-NNNN` is
declared in `_policies/08_Decisions.md`. Resolving against the wrong one is what
`TDDLIST_EXCEPTION_UNRESOLVED_DR` reports. Do **not** write `07_Decisions.md`
or `09_delta.md`.
