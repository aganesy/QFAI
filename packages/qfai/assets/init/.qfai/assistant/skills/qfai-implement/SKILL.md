...
- The item's evidence file (item 10) does not exist, or does not record **every routed reviewer's** verdict for the item — including `Prototype parity` on a UI-affecting item (this is the single blocking statement about the evidence file; its absence of _verdicts_ is never blocking before items 7-8)
