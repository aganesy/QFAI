...
- Writing acceptance tests (use `/qfai-atdd`). `Layer = E2E` / `Layer = API` / `Layer = Integration` ledger rows are tracked here but their tests are authored there, and the RED provenance those rows carry is defined in `../qfai-atdd/references/red-provenance.md` — this skill writes their `Status` / `DR-ID` / `Evidence` from the evidence that stage produced. **All three, including `Integration`**: `QFAI-ATDD-112` files every `L3` TC — and every one whose `Level` that vocabulary cannot read — in `tests/integration/**`, and `/qfai-atdd` P4 writes those tests, so an `Integration` row is ATDD-owned on exactly the terms an `E2E` one is (`references/execution-ledger.md#atdd-owned-rows`). `/qfai-sdd` Phase 2b is what seeds them (`references/ledger-preconditions.md#producer`).
...
3. Write a **failing test** from the row's obligation column, selected by `Layer`: `TC-Refs` for `Unit` / `Component` / `Integration`, `US-Refs` for `E2E`, `CON-API-Refs` for `API`. An `E2E`, `API` **or `Integration`** row's test is authored by `/qfai-atdd` (Non-goals) — all three, so this step never writes the integration test a `Layer = Integration` row already owes to that stage, and reading `TC-Refs` here is about which obligation the row carries, not about who writes from it; this skill only drives such a row's status and evidence once the acceptance test exists, and stops with a handoff note if it does not.
