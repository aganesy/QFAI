...
5. If the test unexpectedly passes, classify **why** before doing anything else:
...
The `Evidence` cell then carries `Satisfied-by`, `Falsifiability command`,
`Falsifiability result` and the GREEN pair in place of a RED pair. Item 3 of
the 11-point gate is satisfied by the falsifiability evidence; the completion
prohibition "No RED fresh evidence exists for the item" does not apply to a row
carrying it.
