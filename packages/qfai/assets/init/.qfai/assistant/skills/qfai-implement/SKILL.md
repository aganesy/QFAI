...
- `Revision` — the state the observation was made against: `git rev-parse HEAD`, or `working-tree+<content hash>` for an uncommitted tree. One per round block, and one for the refactor-verify pair (`references/evidence-revision.md`)
