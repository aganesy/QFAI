...
This is a precondition of the RED rule, not a relaxation of it: against a running skeleton, `404 where the row asserts 200` is an assertion failure inside the row's own `Selector` rather than a missing seam, which is what makes `Layer = E2E` and `Layer = API` rows observable at all (`references/red-admissibility.md`). The phase has its own `skeleton` routing entry because the orchestrator may not write code and every other phase here is per-row, and `/qfai-atdd` invokes it on its own when its P6 runtime gate has no program to start. The applicability test, the entrypoint unit, the smoke-script contract, the halt taxonomy and the evidence fields are in `references/walking-skeleton.md`.
