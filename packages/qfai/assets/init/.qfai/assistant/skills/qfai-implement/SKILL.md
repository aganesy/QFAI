...
- `QFAI-ATDD-111` and `QFAI-ATDD-113` are clean for this spec — every declared
  `US-*` and `CON-API-*` is referenced from the test tree. Phase 2b seeds an `E2E` /
  `API` row per **active** obligation, so those rows are present unless every one is
  exempt (`../qfai-atdd/references/red-provenance.md#a-spec-with-no-atdd-owned-rows`)
