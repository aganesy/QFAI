...
- Write a `.qfai/steering/<id>.md` work-log entry when this stage hits a condition in the `kind` trigger table of `.qfai/assistant/catalog/worklog-entry.schema.md` — `blocker`, `handoff`, `consultation-needed` and `decision` are the ones it reaches most. A stop is the one omission the tool can see: with a `Status=blocked` row in the ledger and no `kind: blocker` / `kind: handoff` entry naming the spec (via `scope:` or `links:`), `npx qfai validate` reports `TDDLIST_BLOCKED_NO_WORKLOG`. **The other triggers are advisory — nothing detects those omissions**, so an unwritten entry there is simply lost.
