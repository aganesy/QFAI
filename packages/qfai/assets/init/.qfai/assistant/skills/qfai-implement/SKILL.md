...
4. Run the test and **watch it fail**. Admissible only when an assertion — or an expected-exception check — inside this row's `Selector` raised the failure and its message names the predicate the row owns; a collection / import / syntax / fixture error, or an unasserted throw, is a **missing seam**, not a RED (`references/red-admissibility.md`). Observe each `Selector` entry's failure separately; one aggregate run is not a valid RED observation. Then neutralize the row's assertions in the compilable form `references/red-admissibility.md` defines, re-run the same command, confirm it **passes**, capture the strip diff **before** restoring, restore the test, and record both as `RED assertion-stripped result` — criterion 4 is the one criterion the recorded failure itself cannot show, so without its own run no gate can fail a row on it, and after the restore the passing line alone no longer distinguishes a strip from a skipped selector. Submit that run to `qa-gatekeeper` and obtain confirmation **before** any production code exists — routing phase `red`.
...
- `RED assertion-stripped result` — **required wherever a RED pair is present**: the `RED command` re-run with every assertion the row's `Selector` executes neutralized, the **strip diff** taken before the restore, its **passing** output, and the test restored immediately. This is criterion 4 of `references/red-admissibility.md`, and it is the only one of the four that leaves no trace in `RED result` — `RED failure mode: assertion` reads identically whether the stripped run happened or not, so stated as a conjunct with nowhere to record it, it was the one criterion no gate could ever fail a row on. Neutralized, not deleted: in a language where an unused local or import is a build error, deleting the expression fails the re-run for a reason criterion 4 is not about, and that file defines the compilable form. A `falsifiability` row does not carry it: it has no RED pair to strip, and its mutation run answers the same question. **Required wherever a RED pair is present** means the pair this contract governs — a round whose RED was observed before this field existed cannot re-take it and records `pre-contract` per `references/round-evidence.md`, which is the only admissible absence and never available to a round opened now
