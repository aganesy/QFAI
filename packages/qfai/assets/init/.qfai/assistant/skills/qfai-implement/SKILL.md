...
- Review rendered output, screenshot evidence, or HTML output before closing any UI-affecting item (`references/ui-affecting.md`) — on a cli-only target the reviewable rendering is the captured command output (stdout/stderr transcript, exit code) instead.
- Read spec + contract inputs first whenever the item is UI-affecting (`references/ui-affecting.md`). Read order: `01_Spec.md` → `03_Acceptance-Criteria.md` → `05_Examples.md` → root `DESIGN.md` → `<contractsDir>/design/DESIGN.md.lock.yaml` → `<contractsDir>/design/design-system.yaml` (post-loop token mirror) → `<contractsDir>/design/prototype-handoff.yaml` → the declared UI contracts, resolved exactly as the trigger resolves them (`references/ui-affecting.md#the-two-inputs`: every `.yaml` **and** `.yml` under `<contractsDir>/ui/**` recursively) — a review must not skip the contract whose clause routed it → `.qfai/evidence/prototyping/iter-NN/<screen>.{png,html}` → `.qfai/prototypes/final/index.html`. **`<contractsDir>` is `paths.contractsDir` from `qfai.config.yaml` (default `.qfai/contracts`) for the design contracts as much as for the UI ones** — `designContractReadiness` resolves the three design files from that key, so pinning them to the default location here — while the UI contracts on the same line resolved dynamically — made a repointed project read a stale mirror or stop on a default file it never had. **cli-only target** — decided per implemented spec, never per session: read the classification from the `01_Context.md` of the discussion pack **this spec's own provenance names** (`Source: discussion-<ts>#...` in its `02_User-stories.md` / `03_Acceptance-Criteria.md`), and the target is cli-only when that block says `primary_surface: cli` with no visual `secondary_surfaces`. Consult `.qfai/state.json#discussion.currentId` only when the spec carries no pack provenance at all, and treat the target as visual when neither resolves — `/qfai-implement` takes any one spec from an argument or the queue, so a `discussion.currentId` pointer left on someone else's CLI pack must not strip a web spec of its `DESIGN.md` / lock / prototype inputs (nor the inverse demand them of an older CLI spec). On a cli-only target none of the four design-contract entries nor either prototype-evidence entry exists — `/qfai-discussion` authors no root `DESIGN.md`, `/qfai-sdd` Phase 0 skips the freeze and `/qfai-prototyping` rejects `cli` — so the read order is `01_Spec.md` → `03_Acceptance-Criteria.md` → `05_Examples.md` → `<contractsDir>/ui/**` and nothing is missing.
...
- `product-surface-reviewer`
  - reviews UI-affecting items, and only those (`references/ui-affecting.md`)
...
- `Prototype parity` — product-surface-reviewer result for UI-affecting items, written with the clause that routed the row (`PASS (clause N)` / `REVISE (clause N)`), or `n/a (not UI-affecting)` when no clause of `references/ui-affecting.md` selects the row. Never blank: a blank cell cannot tell the two apart, and the clause number is what makes the decision re-checkable against the declaration it came from. **Carries its own `Reviewed revision`, and an `Audited evidence hash` over the UI contracts the clause fired on together with the rendered evidence judged** — like items 7-8 and for the same reason: a verdict that dates from nothing can be carried forward onto a row whose production UI, UI contract or rendered evidence moved after the reviewer returned. An `n/a` row records the revision the clauses were evaluated at and omits the hash, having no reviewer and no rendered evidence (`references/ui-affecting.md#staleness-reviewed-revision-and-audited-evidence-hash`). Required of rows **this run takes through the gate** — a row left `done` by a version that predates the field is skipped on re-execution and is not retroactively blocked; it acquires a value only if an approved Change Request resets it (`references/ui-affecting.md#rows-completed-before-this-field-existed`). On a cli-only target the field records the captured-command-output surface review item 9 substitutes for parity; the field name does not change, so nothing downstream has to learn a second key
