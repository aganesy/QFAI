...
3. Write a **failing test** from the row's obligation column, selected by `Layer`: `TC-Refs` for `Unit` / `Component` / `Integration`. `US-Refs` / `CON-API-Refs` rows are not selected here at all — `/qfai-atdd` authors those tests **and** writes their `Status` / `DR-ID` / `Evidence`, per `qfai-atdd/SKILL.md#execution-ledger-the-rows-this-skill-owns`. Two skills claiming one row's status cells is what left those rows stalled between the stages.
