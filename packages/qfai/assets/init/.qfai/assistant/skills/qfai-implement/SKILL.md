...
- Writing spec artifacts other than this skill's own `tdd/test-list.md` ledger (use `/qfai-sdd`). The ledger's `Status` / `DR-ID` / `Evidence` / `Blocked-By` cells are carved out unconditionally by the Drift Protocol, and its `Test file` / `Selector` cells conditionally — a placeholder may be filled, and a selector that does not resolve against the row's named test file may be repaired, but neither may be rewritten once its condition has ceased to hold, i.e. a `Test file` that names a path and a `Selector` that resolves (`constitution/drift-protocol.md#allowed-exceptions-minimal-whitelist`, which states both conditions); its rows, and the columns carrying their obligation identity, are still upstream, so a row this skill needs but does not have is a Change Request, never a write. A row whose own test asserts over the CONTENT of an artifact this skill may not write is not implementable here at all, and the three moves available to it all lose: `references/upstream-artifact-ordering.md`.
...
1. Read `test-list.md`. **A named row wins**: when the invocation names one or more `TDD-ID`s — `/qfai-atdd` stage gate P1c hands over by id — process exactly those, in the order given, and select nothing else. Row order in the ledger is not a priority: a branch-2 row sitting above the named one would otherwise be picked up first, and its checkpoint runs the full suite against a tree that still holds the named row's deliberate RED, so it fails and lands at `refactor`, which this step does not re-select. Otherwise, **rework first**: if any row is at `review-fix`, select the first such row and resume its rework (`references/round-evidence.md`) before any `todo` row — one left by an interrupted session is otherwise never picked up. Otherwise select the first row with `Status = todo`, skipping any `blocked` row — it cannot be started, and re-issuing it is what made the determination get re-derived every pass (`references/execution-ledger.md#blocked-rows`). **A mutation-only request wins over even that, and moves no row.** `/qfai-atdd` sends one when a stage that owns no ledger row has edited a shared test artifact a completed row reads (`../qfai-atdd/references/shared-test-artifacts.md`): it names, per affected row, that row's spec and `TDD-ID`, the mutation its `Oracle proof` plan or `Satisfied-by` names, and the selector to run under the changed artifact. Apply the mutation, run the selector, capture the failure, **revert in the same step**, re-run for the restored GREEN, and return both. **Write nothing to `test-list.md` and nothing to that row's evidence**: the row stays `done`, no round block opens, and the returned pair is recorded by the requesting stage in its own `## Shared-artifact re-verify` block. This is the only entry that takes a row already at a terminal status, and it takes it read-only — every path below selects `todo`, a named row or `review-fix`, none of which a `done` row can be, so without this the request had no receiver and the requesting stage could never complete.
   > **A named `TDD-ID` that is already `blocked` is reported, never re-judged.** The named-row rule above selects by id whatever the status, so a re-invocation arriving while that row's residual `CR-*` still waits for approval would re-enter the judgement below on a row that has already been through it — filing a second `CR-*` for the same matrix shape and then aiming a `todo -> blocked` write at a row that is no longer `todo`. Return that row's existing `Blocked-By` as this id's outcome, finish the id, and continue with the remaining named ids. **The shape judgement below applies only to a row actually at `Status = todo`, and this narrows that judgement and nothing else**: a `review-fix` row selected for rework is not a `todo` row and never was, so reading the restriction as a gate on the whole step stranded it here instead of letting it resume its rework in place through step 3b and Phase Green.
   > **Judge the selected row's selector shape here, while it is still `todo` and before step 2 writes `todo -> red`.** A selector that accumulates unrelated boundaries **invalidates its own RED observation** — a single test function can fail only once, so every assertion behind the first is unobserved on every RED run (`references/execution-ledger.md#selector-granularity-must`; rules and examples: `references/selector-granularity.md`). The shape is visible at selection, and the row usually fails as expected on its first assert, so nothing later in this phase re-examines it: judged only at step 5 the check never ran. **This skill does not split it.** The split is seeded by `/qfai-sdd` Phase 2b (`../qfai-sdd/references/sdd-phase-checklists.md`), and adding or re-scoping a row is an upstream write this skill does not own — `Status`, `DR-ID`, `Evidence` and `Blocked-By` are its only cells (`../qfai-sdd/references/spec-traceability-rules.md` **Ownership split**, `.qfai/assistant/constitution/drift-protocol.md#allowed-exceptions-minimal-whitelist`).
   > A matrix shape first visible here is the residual case and takes a named, bounded path instead: raise a Change Request per `.qfai/assistant/constitution/drift-protocol.md#when-drift-is-detected` — its template, its wait for explicit user approval and its owner rerun, not the `CR-*` file alone — naming the boundaries the row conflates and the split it asks `/qfai-sdd` for; creating that record is whitelisted. **Then write `todo -> blocked` with that `CR-*` in `Blocked-By` as soon as that file exists — at the protocol's step 2, ahead of its step 3 wait for approval, never after the owner rerun.** That step otherwise makes one write only, so the parking is named there as its single exception and is bounded to it: this cell pair on the rows that CR's blocked set enumerates, and nothing else. The wait spans sessions, and a row left at `todo` across it is re-selected and its determination re-derived on every pass — the exact loop `blocked` exists to stop (`references/execution-ledger.md#blocked-rows`); after the rerun the write has no correct target either, because Phase 2b re-scopes that row, so it would land on nothing or re-block what the approved CR just resolved. Phase 2b seeds that column, so naming the blocker fills a cell rather than creating one — **and check it is there before writing, because an upgraded ledger seeded at the eight required columns has no `Blocked-By` at all**: adding it is a table-shape write only Phase 2b may make, so a ledger short of the column stops here with a handoff note asking for that column migration (`../qfai-sdd/references/sdd-phase-checklists.md`) and leaves the row at `todo`. Blocking a row whose blocker cannot be written is `TDDLIST_BLOCKED_MISSING_REF`, and the migration runs only on a `/qfai-sdd` pass — a `/qfai-implement` rerun straight after the upgrade never reaches it on its own. The row leaves `blocked` only through that owner rerun **and the preflight that follows it**, in that order: Phase 2b writes row identity alone — it narrows the kept row's `Selector` and appends the remaining boundaries as new `todo` rows (`../qfai-sdd/references/sdd-phase-checklists.md`) — and this skill's Stage 0 Change-Request preflight then returns it to `todo`, records that `CR-*` in `DR-ID` and clears the `Blocked-By` (`references/change-request-reset.md`) — **but only when no other open `CR-*` still names the row**. Blocked sets compose per CR and a row on two of them resumes when both release (`.qfai/assistant/constitution/drift-protocol.md`, resume step), so the preflight recomposes every open CR's set before it writes: a row still named by an unresolved one keeps `blocked`, and only the matrix `CR-*` leaves its `Blocked-By`. Releasing on the matrix CR alone re-runs a test whose other approved change has not landed. The reset is a cell write, so it stays with the cells' owner: had Phase 2b written it, `/qfai-sdd` would edit `Status` and `Blocked-By` it does not own, and doing it here before the rerun would unblock a row nobody had re-scoped. **Then continue exactly where this step's own selection rules point**: on an invocation that named `TDD-ID`s, take only the remaining named ids in the order given and return to the caller when that queue is empty — never an unnamed `todo` row, which would break the handoff's scope; otherwise re-enter this step and select the next row. A row already past `todo` when the shape surfaces keeps its status and stops with a handoff note naming the `CR-*`. Never split in place, and never proceed to Green on an invalidated RED.
...
- Once `qa-gatekeeper` PASSes the observation for a RED round, item scope MUST NOT be re-litigated for that round. A newly discovered scope gap is a **new** `test-list.md` row rather than a reopening of the passed one — and this skill does not open it: raise a Change Request per `.qfai/assistant/constitution/drift-protocol.md#when-drift-is-detected` naming the uncovered obligation, and `/qfai-sdd` Phase 2b seeds the row it asks for (`../qfai-sdd/references/sdd-phase-checklists.md`).
- If a scope objection nonetheless arrives after step 3, it is treated as a new-row request and takes that same upstream path; the existing PASS stands and is not discarded.
