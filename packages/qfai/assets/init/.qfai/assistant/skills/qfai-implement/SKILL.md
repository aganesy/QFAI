...
- `Prototype parity` — product-surface-reviewer result for UI-affecting items (PASS or REVISE) with its `Reviewed revision` and `Audited evidence hash`, the same two the other verdicts carry, so gate item 10 recomputes this verdict's hash as it does theirs
