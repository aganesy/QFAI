...
1. The brand intent is captured in the `uiux/` sidecars: a brand name and voice, the emotion
   the audience should feel, what the product must not read as, and the do/don't and brand
   signals that become the `# Brand Philosophy` body, with exploration references
   framed as **deviate-from** inputs. Visual-prototyping surfaces only — see `## CLI Packs`
   below. `/qfai-sdd` Phase 0 authors root `DESIGN.md` from this; a discussion writes nothing
...
4. Exploration directions are carried unranked — no single visual winner is selected and the
   design system is not finalized here (discussion is planner-first).
5. No forbidden legacy sidecar exists under `uiux/` (see
   `templates/uiux/00_index.md#Forbidden Legacy Files`).
6. `Disposition: open` count is zero in `11_OQ-Register.md`.
...
so condition 1 above does not apply to it:

- No root `DESIGN.md` required, so no brand intent to capture and no `visual.*` token tree.
  `/qfai-prototyping` rejects `cli`, so nothing downstream reads the token values.
  `/qfai-sdd` Phase 0 authors and freezes nothing for a cli-only project, and
  `validators/designContractReadiness.ts` skips `QFAI-DCON-030`/`-031` for it.
- Conditions 2-6 apply unchanged: all three canonical `uiux/` sidecars, the full
...
- `primary_surface: cli` with a visual `secondary_surfaces` entry is NOT a cli-only pack —
  condition 1 stays blocking for it.
