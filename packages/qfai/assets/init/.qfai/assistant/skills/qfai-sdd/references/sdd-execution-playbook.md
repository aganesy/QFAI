...
1. Write `.qfai/evidence/import-lite-<timestamp>.md` from
   `templates/evidence/import-lite.md` before editing any spec.
2. Fill `generated_at` plus at least one real `Sources` entry or user excerpt.
   `<...>` placeholders and fillers (`TBD`, `none`, `n/a`) are rejected, and an
   excerpt fence left unclosed is rejected with them.
