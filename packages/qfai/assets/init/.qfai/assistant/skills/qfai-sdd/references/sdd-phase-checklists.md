...
- Reconcile **per boundary, not only per TC**. A matrix-shaped TC holds several rows, so re-derive its boundary set from `06_Test-Cases.md` and match the existing rows to it by `Selector`: retire the row of a boundary the TC no longer declares, append a row at `todo` for a boundary it has gained, and reset the rest under the rule above. Keyed on the TC alone, a TC that drops from three boundaries to two keeps all three rows — the TC still exists and is still `L3`, so nothing retires the third, and the changed-TC reset makes it selectable again for behaviour the spec no longer states.
