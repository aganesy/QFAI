...
- Seed each row's `Tier` alongside its `Layer`, from the tier table and the criticality list in `../../qfai-implement/references/volume-policy.md`. Blank or `-` is read downstream as `T1`, so escalate here or not at all — `/qfai-implement` never writes the tier into `Evidence`.
- Delta only: an unchanged TC's row keeps its `TDD-ID`, `Status`, `Test file`, `Selector`, `DR-ID` and `Evidence`.
- Re-derive `Tier` on every re-run, and treat a **raise** (`T1 -> T2` / `T3`, `T2 -> T3`) as an upstream change to that row even when the TC itself is unchanged — it overrides the delta rule above: return `Status` to `todo`, clear the now-void `Evidence`, and record the driving `CR-*` in `DR-ID` as the upstream reset requires. A batched T1 trail is not evidence for the per-row and `product-surface-reviewer` turns the raised tier owes, so a `done` row left standing would be complete on a review that never ran. A **lowered** tier keeps `Status` and `Evidence`: the stronger ceremony already ran.
