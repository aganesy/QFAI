...
- Fill `BR-Ref` — the TC's `EX-Ref`, then that `EX`'s `BR-Ref` in `05_Examples.md`; only a TC with no `EX-Ref` falls back to its `AC-Refs` and the `BR` whose `AC-Refs` names one of them in `04_Business-Rules.md`; lowest-numbered `BR-*` wins, `-` when none does. It is `/qfai-implement`'s T1 review-group key and this is the only phase that can resolve it. Optional to `npx qfai validate`, so an unseeded ledger still passes — and still cannot be batched.
