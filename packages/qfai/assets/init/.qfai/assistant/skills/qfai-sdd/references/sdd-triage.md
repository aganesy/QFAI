...
5. **Approval pass.** For every row whose Operation requires approval
   ...
   `--auto` scope: ask when an operator is present, and otherwise stop at
   step 7. Never synthesize an `Approved By` value — the column records who
   authorized the operation, so an invented approver is a false audit record.
   See `../SKILL.md#--auto-and-approval-required-rows`.
...
7. **Stop.** Do not enter Phase 0 until every required-approval row has
   ...
   `Approved By` as `-`, write a `blocker` work-log entry naming each
   unapproved row with its Operation and target, and report the
   `QFAI-TRIAGE-005` errors as the reason the run stopped.
