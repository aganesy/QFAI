...
3. Re-run `npx qfai sdd preflight` after Triage so `.qfai/report/preflight_summary.md` reflects the approved intake (the `Open Questions (Carry-over)` entries recorded in Stage 0 are preserved).
...
- `.qfai/report/preflight_summary.md` (written by `npx qfai sdd preflight`; carry-over findings via `--assume`)
