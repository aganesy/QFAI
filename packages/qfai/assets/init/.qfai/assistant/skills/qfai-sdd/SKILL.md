...
3. **Append-first**: default to UPDATE on an existing active spec whose subject tokens overlap the REQ; walk the impact cascade and add MODIFY/REMOVE rows on companion specs. CREATE only when there is **zero subject-token overlap with any active spec** AND the REQ introduces a genuinely new capability — first add the `CAP-NNNN` row to `_policies/03_Capabilities.md` and fill its `Spec` cell with the next unused `spec-NNNN` (an empty cell reports `QFAI-SPLIT-106`; that code is an error, so `validate --fail-on error` stops on it), then cite it in the CREATE row Rationale (`QFAI-TRIAGE-006`). Under `--auto` that catalog row is not written until the CREATE row is approved — see "User Questions" below. See `references/sdd-triage.md` for the precise APPEND-vs-CREATE algorithm.
...
- Read before writing `.qfai/**`:
  ...
  - `.qfai/assistant/manifest/agent-catalog.yml` — the acting `orchestrator`'s and every routed
    role's entry, per the Inputs Priority scope below; not the whole file
...
1. Use only templates under `.qfai/assistant/skills/qfai-sdd/templates/` — the whole directory, not an enumerated subset, so a new template directory is covered on the day it ships.
   - No cross-skill exception. Phase 0 authors root `DESIGN.md` from this skill's own `references/design-dna-intake.md`, so there is no sibling skill's template to reach into.
...
7. Phase 2b: Seed each target spec's `tdd/test-list.md` in **four groups**, all
   ...
   **The two TC groups are exclusive** — read each TC's `Level` once and route
   ...
   (`../qfai-implement/references/ledger-preconditions.md#producer`). This phase
   owns the rows, so a shape left un-split here reaches RED where nobody may fix
   it (`references/sdd-phase-checklists.md`).
   The same is true of the columns: **migrate an existing ledger's columns to
   the template's** when the file is already there — the copy above fires only
   when it is absent, and an eight-column ledger predates `Blocked-By`. Only
   this phase may change the table's shape, so a ledger left short of a column
   makes the write that needs it unwritable downstream.
   **A seeded E2E/API row leaves `Test file` and `Selector` at `-`, and
   ...
   never writes this ledger, so it records that test's own path and selector as
   the row identity in its handoff entry, and `/qfai-implement` Phase Red step
   3b writes both cells from that entry in the same edit that moves the row out
   of `todo`. Naming no writer for them left both at `-` permanently — reaching
   ...
   **A surface-typing flip re-runs the E2E delta over every spec.** That
   precondition is a property of the **project**, not of the target, so a run
   that adds the project's **first** surface signal — or removes its **last** —
   moves every other spec's `US-*` across the active/exempt line while this
   phase otherwise touches the target spec alone. When this run flips it, apply
   the E2E-row delta to **every** spec's ledger: losing the last signal returns
   `QFAI-ATDD-111` to project-wide, so each non-target spec now owes E2E rows it
   has none of, and gaining the first one makes the specs with no surface signal
   exempt, so their existing E2E rows must be retired. Left at the target only,
   the flip yields either a gate no ledger can clear or `todo` rows on tests
   that must not be written — in every spec but one.
...
with no visual secondary surface). Such a pack gets no root `DESIGN.md`
authored at all, `/qfai-prototyping` rejects `cli`, and
...
1. Read root `DESIGN.md` at `<consuming-project-root>/DESIGN.md`. If it is
   missing, **author it here** per `references/design-dna-intake.md`, from
   the brand intent the discussion pack's `uiux/` sidecars record — a brand
   name and voice, the emotion the audience should feel, what the product
   must not read as, and the do/don't and brand signals that become the
   `# Brand Philosophy` body. A spec taken in through import-lite has no
   pack; read the same intent from the import-lite evidence its provenance
   names, and ask for what neither states.

   Fill the required `brand.archetype` field in two phases. **Phase A**
   picks the closest-fitting archetype from
   `references/design-md-brand-catalog.md` and takes its
   `aesthetic_properties` as draft defaults. **Phase B** routes each default
   to its own home: `color_tendency` / `typography` / `spacing` into the
   `visual.*` token tree, and the `interaction` default into
   `accessibility.motion` (`visual.*` accepts only
   `colors | typography | radius | shadow | spacing`).

   This fills the draft brand SSOT only — exploration directions stay unranked
   and the design system is not finalized here. `/qfai-prototyping` iterates
   against these tokens and its handoff is what settles the design system.

   Then continue with step 2, which judges what was just written by the same
   rule it judges a hand-authored file by. Authoring here does not exempt the
   result from the sample check, the parse or the validation below.
...
- Phase 2b seeds each target spec's tdd/test-list.md from 06_Test-Cases.md (one row per coverage-target TC, Status = todo) and is a delta: existing rows keep their TDD-ID, Status, Test file, Selector, DR-ID and Evidence. Three exceptions, all because Phase 2b is the only phase that may add, remove or re-scope a row: a matrix-shaped TC (many rejection reasons, a status-code matrix, several independent state transitions) takes one row per independently observable boundary, each carrying that TC in TC-Refs — and because Phase Red judges the selector of whatever row it selected, re-scoping is this phase's write at every Layer, so a legal E2E / API row splits here too, by the boundaries its own US-Refs / CON-API-Refs source names and repeating that same obligation column; a matrix row already past todo — done included — is re-scoped here rather than shielded by the delta rule (narrow its Selector to the kept boundary, append a todo row per remaining boundary, and always have the kept row re-executed, since narrowing the selector invalidates the evidence and verdicts bound to the old one — this phase writes the row identity only, and the status/DR-ID/Evidence reset is handed to /qfai-implement's Change-Request preflight). Which boundary is kept follows what the row actually observed, and an observation is never invented: a row with a recorded natural RED keeps the boundary that RED observed (the first failing assert), while a blocked row — the residual path's normal state, since Phase Red blocks at selection ahead of RED — and a falsifiability row at red or beyond — whose mutation trio stands in for a natural RED — have no first failing assert at all, so the kept boundary is the one the driving CR-\* names first, in the order that row's obligation source lists them when the CR gives an unordered set, and on a falsifiability row the boundary the predicate its Satisfied-by names covers, falling back to that same order. Third, an existing ledger's columns are migrated to the template's (Blocked-By on an eight-column ledger) so a downstream blocked row never has to add one.
