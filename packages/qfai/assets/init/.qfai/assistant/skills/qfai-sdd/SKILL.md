...
7. Phase 2b: Seed each target spec's `tdd/test-list.md` in **three groups**, all
   ...
   **A matrix-shaped TC takes more than one row**: many rejection reasons, a
   status-code matrix or several independent state transitions are split here
   into one row per independently observable boundary, each carrying that
   `TC-*` in `TC-Refs`. This phase owns the rows, so a shape left un-split here
   reaches RED where nobody may fix it (`references/sdd-phase-checklists.md`).
