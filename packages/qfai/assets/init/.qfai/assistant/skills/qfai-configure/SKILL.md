...
- You MUST produce the required evidence file: `.qfai/evidence/configure-<run-id>.md`.
  ...
  - Durable per-item `implement-*.md` and `atdd-*.md` governance records are committed through the managed negations. Do not commit the configure run file; summarize its key outcomes in the PR description instead.
...
Follow `.qfai/assistant/constitution/shared-skill-operating-baseline.md#completion-contract-shared`. **Smallest applicable smoke check** (this skill's override): one terminating command drawn from the minimum runnable path you recorded — its smallest test or check invocation, or a readiness probe against a service the path starts — plus `npx qfai doctor --fail-on error`, the command the Reviewer Gate requires. **That exit code is the verdict**, because at `error` severity `doctor` decides on `config.load` (the `qfai.config.yaml` this run wrote), `traceability.testGlobs` (globs it wrote that fail to scan, **and globs it wrote that match no test file** while `scMustHaveTest` is on and scenarios exist — the same defect `validate` raises as a `QFAI-TRACE-124` error), `output.outDirCollision` (the `outDir` it set) and two broken-install blockers that stop this skill too, `agents.frontmatter` and `integration.links` (wrappers that do not resolve, so no skill loads); `spec.layout` and `guardrails.present` cap at `warning`, and no spec, ATDD or prototyping artifact is read at `error` at all — so no other stage's unresolved finding can fail it. Read one more line of that output: **`traceability.testGlobs` must report `[ok]`**. Unset globs and a truncated scan stay `warning` on that check, so exit 0 alone would still pass a configuration that collects no test at all — the defect this skill exists to prevent. Anything but `ok` there is this bullet's FAIL, a rule that can only make the verdict stricter, never explain a non-zero one away. `npx qfai validate --fail-on error` is **not** this bullet's verdict: without a profile it runs every validator and exits 1 on any `counts.error`, so a correct run would fail on an artifact this skill may not repair. Run it for the `QFAI-TRACE-124` success criterion, read it by finding code, fix the config-load and traceability `error`s it reports, and hand a spec / ATDD / prototyping `error` to that stage as a pre-existing blocker. The long-running parts of the recorded path (dev server, database) are setup for the probe, never the check itself: a command that does not exit cannot return the pass/fail this bullet needs. A path whose terminating command was recorded but never executed is UNRUN, not a pass.
...
Copy `templates/configure-evidence.md`.
...
1. Read relevant **project steering** (if present):
   ...
   - any additional file under `.qfai/assistant/catalog/`, and
     ...
     `.qfai/assistant/manifest/review-profiles.yml`. From
     `.qfai/assistant/manifest/agent-catalog.yml` read the acting role's entry
     when a role needs one, not the whole file — its `developer_instructions`
     bodies are derived from the agent cards, so skip a body that matches the
     card already in context, and take the card when the two differ
