...
- Exception: `agent-catalog.yml#agents[].developer_instructions` is a derived copy of `.qfai/assistant/agents/<id>.md`. Edit the markdown file — the source — and regenerate the catalog block from it; never hand-edit the block. `QFAI-AGENT-014` warns when the two diverge.
