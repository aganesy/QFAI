...
- Read every error path the change adds or alters for silent failure: an empty
  catch, a silent return, a catch that also swallows errors it did not expect,
  and a fallback that hides the problem instead of handling it. A failure handled
  where `.agents/rules/minimal-implementation.md` § 2 does not admit it should
  propagate instead. A deliberate fallback carries the ceiling and lifting
  condition § 3 requires; one without them is an unmarked simplification. A
  failure kept here is logged with enough context to debug it, and its message
  tells the user what to do next.
- Read every type the change adds or alters for invariants it owns: a value
  crossing a trust boundary is parsed there into a form that cannot hold an
  invalid value (§ 2 of the same rule), construction validates, every mutation
  keeps the invariant, internals are not exposed for outside code to change, and
  no invariant lives only in a comment or in its callers.
