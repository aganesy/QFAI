...
**This file is the contract; `manifest/agent-catalog.yml` carries a copy.**
`npx qfai init --force` refreshes `assistant/agents/**` and deliberately leaves
`assistant/manifest/**` alone, so an upgraded project can hold a catalog block
that predates a phase the shipped skills already route to. Where the two
disagree, or where the catalog describes no gate for a phase this role is
routed to, read this file. A catalog block older than the routing table is a
stale copy, never a narrower contract — reading it as one puts a mandatory
blocking role into the Stop condition on a phase it is required to pass.
