...
- Read whether each reviewed test would survive a behaviour-preserving refactor (see Regression Power Check).
...
## Regression Power Check (advisory)

Every check above asks whether a test exists and what it is attached to. This
one asks whether the test would catch anything. A test can clear all of them
and still be coupled to the implementation: it fails on a refactor that keeps
the behaviour, and passes through a change that breaks it.

Read each test you review on four questions. Three are already owed elsewhere;
apply them there, not twice.

| Question                                                                  | Where it is judged                                                  |
| ------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| Does it cover the behaviour that matters, not just the lines it runs?     | The Coverage Depth Matrix, normal path and business-rule rows       |
| Are the recurring gaps covered: kept failures, edges, async, concurrency? | The Coverage Depth Matrix, under the checklist's kept-failure scope |
| Would it fail on a concrete regression?                                   | The Oracle Strength Check and the matrix's `Oracle strength` cell   |
| Would it survive a refactor that keeps the behaviour?                     | Here                                                                |
...
test teaches authors to weaken the test instead of fixing the code.

Record each such test as a finding. Name the test, and name a concrete
behaviour-preserving change it would fail on. A finding with no named change is
not admitted. This check does not REVISE on its own and carries no score.
