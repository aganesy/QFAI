...
- **The per-item RED/GREEN evidence for the row under review — in the file its
  ...
  `## Ledger rows advanced`, for a `Layer = E2E` / `Layer = API` row;
  ...
  `E2E` / `API` has no implement file. Either way the gate would stop before
...
- `.qfai/report/validate.spec-<id>.json` for the spec under review, or the
  `<report>/run-*/` directory of the run that produced it. **Not
  `.qfai/report/validate.log`**: it and the run-log pointer are shared by every
