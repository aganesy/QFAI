...
- Check the implementation against `prototype-handoff.yaml` `procurement` before judging resemblance: a region the manifest says was procured and the code rebuilt is a finding with a name, and an authored region the manifest does not list is one too.
