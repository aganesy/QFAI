...
At both stages: when business rules (BR-\*) exist, verify each BR has at least one positive and one negative test case. From the ATDD stage onward that verdict is recorded per BR in the business rule coverage table under the Coverage Depth Matrix, which has one row per rule — a `US/TC` row cannot carry it.
