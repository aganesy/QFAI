...
repository's own operation rather than a convention the package distributes.

- Indexed in `_policies/05_Contracts.md` under **CLI Contracts**, with `CLI-*`
  short ids.
- Markdown, carrying no `QFAI-CONTRACT-ID`. The `ui/` / `api/` / `db/` contract
  validators do not scan it.
- Named `qfai-<command>.md`. A schema or ownership boundary shared by more than
  one command is named for its subject instead — `worklog-entry.schema.md`,
