...
- Every question in the round is put at once, so the user sees the whole of
  ...
  round holds is the one exception: the round is delivered in host-sized
  batches, the frontier is **not** recomputed between them, each batch's answers
  are read for a stop before the next is put, and no answer is acted on until
  the round is exhausted or the user ends it. Batching is how one round reaches
  a host that cannot show it whole. It never makes two rounds, and
  `user-questions.md` § 4 carries the rest of its mechanics.
