...
- Put every question to the user in the form `.claude/rules/user-questions.md`
  ...
  enough to skip it. The rule bounds the form only. How many _clarifying_
  questions are worth asking is the clarification budget's subject; a grilling
  question, a mandatory approval and a needed `hard-required` input are outside
  it and are asked however many there are.
