...
| 1c  | Split the obligation: keep the helper's half here, and record the wiring as its own requirement with its own row                            | One test case, one new requirement chain, one ledger row                | Two records where there was one, and the new row is `todo` from the day it is written                                            | ✅          |

Option 1c is recommended because 1b and 1a each discard half of a real
...
- Overlapping open CRs: `CR-20260913-0007` and `CR-20260913-0008` edit other
  statements of the same pack, and `CR-20260913-0007` edits the same
...
2. Downstream ledger sweep.
   - Reset to `todo`, recording this CR's ID in `DR-ID`: `spec-0013/TDD-0025`
     ...
     `spec-0013/TDD-0026` under `2a`. Each recorded observation is of a rule
     being replaced.
   ...
   - Under `2b`, `TDD-0026` is not reset: that option changes the product to
     match statements that do not move, which is the in-place shared-artifact
     re-verification rather than an upstream invalidation. Under `2c` nothing
     moves.
