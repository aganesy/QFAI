...
| `QFAI-SKILLS-015`            | error                                                  | A skill's entry point carries no usable `name:` or `description:`. A host reads both to register the skill, so without them it is not loaded and the user cannot invoke it by name. Emitted per entry point, in text and JSON output alike. | The host's own registration contract; no spec row                                       |
