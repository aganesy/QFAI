...
| `QFAI-CONTRACT-042`          | error                                                  | A `screens[]` entry in a UI contract that no screen is read from: not a mapping, no `id`, no `route`, or an `id` an earlier entry has. Readers keep the first entry per `id` and skip the rest, so nothing checks what it states. Names the file and position, and for a repeated `id` the entry read instead.                                                                                        | The UI contract reader; no spec row                                                     |
