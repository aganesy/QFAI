...
| `D-DEPRECATED-PATH`          | error                    | An old-layout assistant-tree file path is referenced or present. Warning text MUST name the sunset version per spec-0003 REQ-0023.                                                                                                       | spec-0004 REQ-0040, spec-0003 REQ-0023                                                  |
...
| `W-SKILL-DOC-BROKEN-REF`     | error                    | A `qfai-*` SKILL.md references an assistant path that does not resolve in the current layout.                                                                                                                                            | spec-0004 REQ-0043                                                                      |
...
`qfai validate` reads both the pre-recut layout and the post-recut layout, so a project mid-migration is still validated. An old-layout file is `D-DEPRECATED-PATH`, an error: the layout reached its announced retirement, and the reader stays only so the finding can name what it found.
...
### The retired `.qfai/output/validate.json`
...
- Nothing writes the legacy path. A stale legacy file left on disk, or
  a config still pointing at one, is `D-DEPRECATED-PATH` at `error`,
  and the message names the release the path was retired in (1.10.0).
- Read-side tracking is not implemented: a consumer still pointing at
  the legacy path sees no file, which is the signal to migrate.
