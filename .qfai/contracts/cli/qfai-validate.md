...
| `E-WORKLOG-SECRET`           | error                                                  | A pre-existing secret-pattern scan flagged secret content in a work-log entry. Hard-blocks merge. **Not implemented** — no validator emits this code today; the work-log lane is not wired to a secret scan. Reserved for when one is.   | (security policy in `10_Policy.md`)                                                     |
