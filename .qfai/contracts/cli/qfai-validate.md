...
| `QFAI-ATDD-134`              | error                                                  | The configured test globs cannot be read: the glob matcher refuses a pattern in `validation.traceability.testFileGlobs`, a NUL byte being the known case, or a directory they reach cannot be read. Reported by `--profile atdd`, whose stage reads only extensions out of those globs and would otherwise say nothing. `QFAI-TRACE-124` reports the same under `tdd` and `full`.                     | The ATDD stage gate; no spec row                                                        |
