...
| `QFAI-SKILLS-015`            | error                                                  | A skill's entry point carries no usable `name:` or `description:`, so a host does not register it and the user cannot invoke it by name. A name is usable when it is lowercase letters, digits and single hyphens, at most 64 characters, and the skill's own directory; a description is usable when it carries text, is at most 1024 characters, and has no `<` or `>`. A dot-prefixed directory directly under the skills root is not read, since the host lists none. Reported per entry point, in text and JSON output alike. | The host's own registration contract; no spec row                                       |
