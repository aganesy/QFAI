...
`packages/qfai/tests/integration/contractDeferralNotes.test.ts` enforces exactly that. It does not merely search the sources for the string — a comment or a dead constant would satisfy that. Each code must reach an emission in one of three shapes, in a module that is actually wired up: the code literal is the first argument of `issue(…)` or a `code:` field, or it sits in a code table the same module feeds to `issue(code, …)`, or it is a note a `cli/commands/*` module prints — and the module must be one `core/validate.ts` invokes or `cli/main.ts` imports. The work-log secret-scan hard block (secret content in a work-log entry) previously had a row here with no emitter, no wired scan and no owner, so it is not listed; its definition stays in `.qfai/specs/_policies/06_Glossary.md`, and the row returns together with the module that raises it.
