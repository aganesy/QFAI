...
| `R-HANDOFF-INCOMPLETE`   | error (advisory-failing)                               | Reviewer Gate finding: a `kind: handoff` entry body is missing one of the five required sections (state, next action, constraints, OQs, references).                                             | REQ-0017                            |
...
| `E-WORKLOG-SECRET`       | error                                                  | A pre-existing secret-pattern scan flagged secret content in a work-log entry. Hard-blocks merge.                                                                                                | (security policy in `10_Policy.md`) |
