...
| `QFAI-CONTRACT-042`          | error                                                  | A `screens[]` entry in a UI contract that no screen is read from: not a mapping, no `id`, no `route`, or an `id` an earlier entry of the same contract has; or a `screens` value that is not a list. Readers keep the first entry per `id` and skip the rest, so nothing checks what it states. Names the file and position, and for a repeated `id` the entry read instead.                          | The UI contract reader; no spec row                                                     |
