...
| `QFAI-ATDD-134`              | error                                                  | The glob matcher refuses a pattern in `validation.traceability.testFileGlobs`; a NUL byte is the known case. Reported by `--profile atdd`, whose stage reads only extensions out of those globs and would otherwise say nothing. A directory the globs reach that cannot be read is not reported here, as the stage never opens it. `QFAI-TRACE-124` reports the same refusal under `tdd` and `full`. | The ATDD stage gate; no spec row                                                        |
...
| `QFAI-ATDD-135`              | error                                                  | A directory under the acceptance test roots cannot be read by the account running `qfai validate`. Reported by the `atdd`, `tdd` and `full` profiles, naming the directory. The scan reads past it, so the run finishes with its other results, and no test inside is counted: a coverage finding may name an obligation one of them carries.                                                         | The ATDD stage gate; no spec row                                                        |
| `QFAI-SKILLS-015`            | error                                                  | A skill's entry point carries no usable `name:` or `description:`. A host reads both to register the skill, so without them it is not loaded and the user cannot invoke it by name. A name is usable when it is lowercase letters, digits and single hyphens, at most 64 characters, and the skill's own directory. Emitted per entry point, in text and JSON output alike.                           | The host's own registration contract; no spec row                                       |
