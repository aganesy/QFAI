...
| `QFAI-SKILLS-015`            | error                                                  | A skill's entry point carries no usable `name:` or `description:`. A host reads both to register the skill, so without them it is not loaded and the user cannot invoke it by name. A name is usable when it is lowercase letters, digits and single hyphens, at most 64 characters, and the skill's own directory; a description is usable when it carries text and is at most 1024 characters. A dot-prefixed directory is not read at all, because the host lists none. Emitted per entry point, in text and JSON output alike. | The host's own registration contract; no spec row                                       |
