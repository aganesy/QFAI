...
Neither seeded body is a static asset. `buildProjectSteeringReadmeBody` and `buildProjectSteeringEntryTemplate` in `packages/qfai/src/cli/commands/init.ts` derive them from `WORKLOG_ENTRY_KINDS` / `HANDOFF_REQUIRED_SECTIONS` so they cannot drift from the validator, and they therefore ship as string literals inside `dist/` rather than as files under `assets/`. Two consequences when auditing the obligation above:
...
- `packages/qfai/tests/integration/distributedSurfaceLeakage.test.ts` runs `qfai init` into a temp directory and scans the emitted tree with the same forbidden-class set, so the seeded bodies are checked on every test run regardless of build state.
