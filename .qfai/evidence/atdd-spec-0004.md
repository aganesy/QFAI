...
- The acceptance tests of `TDD-0067` … `TDD-0071` were written in five files under
  `packages/qfai/tests/integration/`: `spec0004WorklogSurfaceRemoval.test.ts`,
  `spec0004WithdrawnSchemaFinding.test.ts`, `spec0004BlockedRowNeedsOnlyBlockedBy.test.ts`,
  `spec0004BlockedRowEmptyBlockedBy.test.ts` and `spec0004SteeringUnreadableBlockedRow.test.ts`.
  `CR-20260925-0010` deleted all five with their rows, and their `tsconfig.tests.json`
  entries with them.
- This stage changed no production file. The production edits of the work-log removal
  are `/qfai-implement`'s, recorded in each row's section below. What they put at risk in
  other specs is under `## Cross-spec obligations`.
...
| `TC-*`     | 38 integration-routed                           | `packages/qfai/tests/integration/**`  | `TC-0004-0074` … `TC-0004-0076` written, then withdrawn with their rows by `CR-20260925-0010`  |
...
| 3 | qa-gatekeeper | atdd-s1-griller | grilling(S1@2026-09-23T19:33:24.738Z/user): Fix EX-0004-0044 / BR-0004-0035 (and AC-0004-0041, TC-0004-0076) through CR-20260923-0015 | `spec-0004/05_Examples.md`, `04_Business-Rules.md` | `CR-20260923-0015` (applied); the example gave a passing `Blocked-By` the kept `TDDLIST_BLOCKED_MISSING_REF` check rejects, and changing settled input is the user's to approve | PASS |
...
| 36 | acceptance-test-engineer | atdd-ate | TDD-0069 test + RED | `spec-0004/06_Test-Cases.md` TC-0004-0076, `05_Examples.md` EX-0004-0044, `03_Acceptance-Criteria.md` AC-0004-0041, `04_Business-Rules.md` BR-0004-0035 (as amended by `CR-20260923-0015`); S1 D3-D5 | `packages/qfai/tests/integration/spec0004BlockedRowNeedsOnlyBlockedBy.test.ts` (RED test hash `ddf4adda…dba10`), `packages/qfai/tsconfig.tests.json`; revised per the scope REVISE; approved RED and its stripped run at `working-tree+818c7ff4…e53e6f` in `### TDD-0069` Round 1 | PASS |
...
| 48 | delivery-planner | atdd-scope | Scope approval TDD-0071 | `### TDD-0071`; `spec0004SteeringUnreadableBlockedRow.test.ts` (test hash `74be7f2f…9b5768`); `spec-0004/06_Test-Cases.md` TC-0004-0076, `05_Examples.md` EX-0004-0044 third clause, `03_Acceptance-Criteria.md` AC-0004-0041, `04_Business-Rules.md` BR-0004-0035 (as amended by `CR-20260923-0015`); `tdd/test-list.md` TDD-0069 … TDD-0071 | Scope REVISE. `rowErrors` asserts the first tree's result, which `TDD-0069` owns. It fails on its own, and `TDD-0069`'s mutations would fail it. The TC's third-tree result is "no finding names `.qfai/steering/`" only. Reduce the assertion to `steeringTexts`, rename the selector, drop mutation 2, then resubmit before any RED. The fixture, the D6 setup and the `TDD-0003` read-proof stand | REVISE |
...
| 50 | acceptance-test-engineer | atdd-ate | TDD-0071 test + RED | `spec-0004/06_Test-Cases.md` TC-0004-0076, `05_Examples.md` EX-0004-0044, `03_Acceptance-Criteria.md` AC-0004-0041, `04_Business-Rules.md` BR-0004-0035 (as amended by `CR-20260923-0015`); S1 D3-D6; implement griller S2 | `packages/qfai/tests/integration/spec0004SteeringUnreadableBlockedRow.test.ts` (RED test hash `9f0091e3…06e7dc`), `packages/qfai/tsconfig.tests.json`; revised per the scope REVISE; approved RED and its stripped run at `working-tree+a495f6e2…210a4b` on win32 (`icacls`) in `### TDD-0071` Round 1 | PASS |
...
`CR-20260925-0010` deleted the source rows `TDD-0067`, `TDD-0068` and `TDD-0069`. The
production edits they made stay, so their entries stay under the retired ids, which
the tombstones in `.qfai/specs/spec-0004/tdd/test-list.md` keep unique.

### Files the source rows changed

- `TDD-0067`: `packages/qfai/assets/init/.qfai/assistant/catalog/worklog-entry.schema.md`, `packages/qfai/docs/finding-codes.md`, `packages/qfai/src/cli/commands/validate.ts`, `packages/qfai/src/core/emittedRuleCodes.ts`, `packages/qfai/src/core/parse/spec.ts`, `packages/qfai/src/core/paths/assistantPaths.ts`, `packages/qfai/src/core/validate.ts`, `packages/qfai/src/core/validators/index.ts`, `packages/qfai/src/core/validators/prototypingEvidence.ts`, `packages/qfai/src/core/validators/tddList.ts`, `packages/qfai/src/core/validators/worklogSurface.ts`, `packages/qfai/src/core/worklogEntries.ts`, `packages/qfai/tests/assets/rowNamesItsTestFile.test.ts`, `packages/qfai/tests/assets/worklogSchemaShipped.test.ts`, `packages/qfai/tests/core/findingCodeGrammar.test.ts`, `packages/qfai/tests/core/issueCodeUniqueness.test.ts`, `packages/qfai/tests/core/tddListBlockedStatus.test.ts`, `packages/qfai/tests/integration/cli/commands/validate.profileCoverageNotice.test.ts`, `packages/qfai/tests/integration/validatorConvergenceIntegration.test.ts`, `packages/qfai/tests/scripts/typeCheckEnumeration.allowlist.ts`, `packages/qfai/tests/validators/ruleCodeUniqueness.test.ts`, `packages/qfai/tests/validators/worklogSurface.test.ts`, `packages/qfai/tsconfig.tests.json`
- `TDD-0068`: `packages/qfai/README.md`, `packages/qfai/assets/init/.qfai/assistant/catalog/worklog-entry.schema.md`, `packages/qfai/src/core/emittedRuleCodes.ts`, `packages/qfai/src/core/governedAssistantManifest.ts`, `packages/qfai/src/core/validators/tddList.ts`, `packages/qfai/tests/assets/worklogSchemaShipped.test.ts`, `packages/qfai/tests/cli/init.test.ts`, `packages/qfai/tests/integration/initSpec0003.test.ts`, `packages/qfai/tests/scripts/typeCheckEnumeration.allowlist.ts`, `packages/qfai/tsconfig.tests.json`
- `TDD-0069`: `packages/qfai/src/cli/commands/init.ts`, `packages/qfai/src/cli/commands/validate.ts`, `packages/qfai/src/core/emittedRuleCodes.ts`, `packages/qfai/src/core/paths/assistantPaths.ts`, `packages/qfai/src/core/validators/tddList.ts`, `packages/qfai/src/core/worklogEntries.ts`, `packages/qfai/tests/assets/implementWorklogObligation.test.ts`, `packages/qfai/tests/core/findingCodeGrammar.test.ts`, `packages/qfai/tests/core/issueCodeUniqueness.test.ts`, `packages/qfai/tests/core/tddListBlockedStatus.test.ts`, `packages/qfai/tests/integration/cli/commands/validate.profileCoverageNotice.test.ts`, `packages/qfai/tests/validators/ruleCodeUniqueness.test.ts`, `packages/qfai/tsconfig.tests.json`

### Entries

| TDD-ID | Blocked spec | Blocked TDD-IDs | File | Change required | Obligation at risk | Resolution |
| ------ | ------------ | --------------- | ---- | --------------- | ------------------ | ---------- |
| TDD-0067, TDD-0068, TDD-0069 | spec-0002 | 2 rows: TDD-0001, TDD-0011 | the paths listed for each under `### Files the source rows changed` | Remove the work-log validator from validate's sdd composition; Withdraw `catalog/worklog-entry.schema.md` from the shipped governed set; Remove the check that a stopped ledger owes a work-log record, with its two finding codes and the `worklogEntries.ts` module it read | Matched through the package fallback. At `03762f3cf` every selector passes. The recorded proof of TDD-0001, TDD-0011 replays as recorded (`qa-gatekeeper` PASS). `xspec-cr-c` confirmed each holds. See `### Re-run results: spec-0002` | re-reviewed |
| TDD-0067, TDD-0068, TDD-0069 | spec-0002 | 1 row: TDD-0008 | the paths listed for each under `### Files the source rows changed` | Remove the work-log validator from validate's sdd composition; Withdraw `catalog/worklog-entry.schema.md` from the shipped governed set; Remove the check that a stopped ledger owes a work-log record, with its two finding codes and the `worklogEntries.ts` module it read | The selector passes, but its case checks three file names in `SKILL.md` and does not observe TC-0002-0008 (no winner in discuss). The approved option 1 records that the product contradicts the TC and resets the row. See `### Re-run results: spec-0002` | CR-20260912-0003 |
| TDD-0067, TDD-0068, TDD-0069 | spec-0002 | 1 row: TDD-0012 | the paths listed for each under `### Files the source rows changed` | Remove the work-log validator from validate's sdd composition; Withdraw `catalog/worklog-entry.schema.md` from the shipped governed set; Remove the check that a stopped ledger owes a work-log record, with its two finding codes and the `worklogEntries.ts` module it read | The selector passes, but the CR records that this wording test passes while contradicting the acceptance criterion on `prototyping.yaml` requiredness (TC-0002-0011). See `### Re-run results: spec-0002` | CR-20260912-0003 |
| TDD-0067 | spec-0002 | 2 rows: TDD-0009..0010 | the paths listed for it under `### Files the source rows changed` | Remove the work-log validator from validate's sdd composition | Not `done` at `03762f3cf`: `CR-20260924-0003`, applied on main, reset the row, and its blocked table names it. A row that is not `done` certifies nothing the change can break. See `### Re-run results: spec-0002` | CR-20260924-0003 |
| TDD-0067, TDD-0068, TDD-0069 | spec-0003 | 42 rows: TDD-0001, TDD-0018..0021, TDD-0023..0027, TDD-0029..0031, TDD-0033..0044, TDD-0046..0047, TDD-0049..0053, TDD-0055, TDD-0058..0063, TDD-0092..0094 | the paths listed for each under `### Files the source rows changed` | Remove the work-log validator from validate's sdd composition; Withdraw `catalog/worklog-entry.schema.md` from the shipped governed set; Remove the check that a stopped ledger owes a work-log record, with its two finding codes and the `worklogEntries.ts` module it read | Matched through the package fallback. At `03762f3cf` every selector passes. The recorded proof of TDD-0001, TDD-0018..0020, TDD-0027, TDD-0029..0031, TDD-0033..0044, TDD-0046..0047, TDD-0049..0053, TDD-0055, TDD-0058..0063, TDD-0092..0094 replays as recorded (`qa-gatekeeper` PASS). `xspec-cr-a` confirmed each holds. See `### Re-run results: spec-0003` | re-reviewed |
| TDD-0067, TDD-0068, TDD-0069 | spec-0003 | 3 rows: TDD-0045, TDD-0048, TDD-0054 | the paths listed for each under `### Files the source rows changed` | Remove the work-log validator from validate's sdd composition; Withdraw `catalog/worklog-entry.schema.md` from the shipped governed set; Remove the check that a stopped ledger owes a work-log record, with its two finding codes and the `worklogEntries.ts` module it read | The recorded mutation still survives on the current test (`init.ts` `6465aa79`, test `6a14f8e6`), so the test cannot fail on the predicate the row proves. The CR names the row. See `### Re-run results: spec-0003` | CR-20260925-0006 |
| TDD-0067, TDD-0068, TDD-0069 | spec-0003 | 6 rows: Withdrawn TDD-0094 (never merged), TDD-0095..0099 | the paths listed for each under `### Files the source rows changed` | Remove the work-log validator from validate's sdd composition; Withdraw `catalog/worklog-entry.schema.md` from the shipped governed set; Remove the check that a stopped ledger owes a work-log record, with its two finding codes and the `worklogEntries.ts` module it read | Deleted with its test by `CR-20260925-0010` (its X2 disposition), so the row certifies nothing. See `### Re-run results: spec-0003` | CR-20260925-0010 |
| TDD-0067, TDD-0068, TDD-0069 | spec-0006 | 23 rows: TDD-0012..0019, TDD-0021..0031, TDD-0033, TDD-0038..0040 | the paths listed for each under `### Files the source rows changed` | Remove the work-log validator from validate's sdd composition; Withdraw `catalog/worklog-entry.schema.md` from the shipped governed set; Remove the check that a stopped ledger owes a work-log record, with its two finding codes and the `worklogEntries.ts` module it read | Matched through the package fallback. At `03762f3cf` every selector passes. The recorded proof of TDD-0029..0031, TDD-0033, TDD-0038..0040 replays as recorded (`qa-gatekeeper` PASS). `xspec-cr-a` confirmed each holds. See `### Re-run results: spec-0006` | re-reviewed |
| TDD-0067, TDD-0068, TDD-0069 | spec-0006 | 1 row: TDD-0020 | the paths listed for each under `### Files the source rows changed` | Remove the work-log validator from validate's sdd composition; Withdraw `catalog/worklog-entry.schema.md` from the shipped governed set; Remove the check that a stopped ledger owes a work-log record, with its two finding codes and the `worklogEntries.ts` module it read | The selector selects no test at `03762f3cf` (zero-selected capture), so no current run can confirm the obligation. The CR's `## Blocked downstream items` table names the row. The CR is still `open`. See `### Re-run results: spec-0006` | CR-20260925-0007 |
| TDD-0067, TDD-0068, TDD-0069 | spec-0006 | 1 row: TDD-0032 | the paths listed for each under `### Files the source rows changed` | Remove the work-log validator from validate's sdd composition; Withdraw `catalog/worklog-entry.schema.md` from the shipped governed set; Remove the check that a stopped ledger owes a work-log record, with its two finding codes and the `worklogEntries.ts` module it read | The selector passes (2/2), but mutants R20 to R29 of the recorded set have no record, so the full set cannot be replayed. The CR names the row. See `### Re-run results: spec-0006` | CR-20260925-0016 |
| TDD-0067, TDD-0068, TDD-0069 | spec-0008 | 2 rows: TDD-0013..0014 | the paths listed for each under `### Files the source rows changed` | Remove the work-log validator from validate's sdd composition; Withdraw `catalog/worklog-entry.schema.md` from the shipped governed set; Remove the check that a stopped ledger owes a work-log record, with its two finding codes and the `worklogEntries.ts` module it read | Matched through the package fallback. At `03762f3cf` every selector passes. The recorded proof of TDD-0013..0014 replays as recorded (`qa-gatekeeper` PASS). `xspec-cr-c` confirmed each holds. See `### Re-run results: spec-0008` | re-reviewed |
| TDD-0067, TDD-0068, TDD-0069 | spec-0010 | 6 rows: TDD-0001, TDD-0005, TDD-0013..0015, TDD-0017 | the paths listed for each under `### Files the source rows changed` | Remove the work-log validator from validate's sdd composition; Withdraw `catalog/worklog-entry.schema.md` from the shipped governed set; Remove the check that a stopped ledger owes a work-log record, with its two finding codes and the `worklogEntries.ts` module it read | Matched through the package fallback. At `03762f3cf` every selector passes. `xspec-cr-c` confirmed each holds. See `### Re-run results: spec-0010` | re-reviewed |
| TDD-0067, TDD-0068, TDD-0069 | spec-0010 | 3 rows: TDD-0006..0008 | the paths listed for each under `### Files the source rows changed` | Remove the work-log validator from validate's sdd composition; Withdraw `catalog/worklog-entry.schema.md` from the shipped governed set; Remove the check that a stopped ledger owes a work-log record, with its two finding codes and the `worklogEntries.ts` module it read | The selector selects no test at `03762f3cf` (zero-selected capture), so no current run can confirm the obligation. The CR's `## Blocked downstream items` table names the row. See `### Re-run results: spec-0010` | CR-20260912-0003 |
| TDD-0067, TDD-0068, TDD-0069 | spec-0010 | 1 row: TDD-0016 | the paths listed for each under `### Files the source rows changed` | Remove the work-log validator from validate's sdd composition; Withdraw `catalog/worklog-entry.schema.md` from the shipped governed set; Remove the check that a stopped ledger owes a work-log record, with its two finding codes and the `worklogEntries.ts` module it read | The selector passes, but TC-0010-0012 expects pack finalization to write `currentId`. The case drives `qfai discussion use`, and no stage code writes the pointer. See `### Re-run results: spec-0010` | CR-20260913-0013 |
| TDD-0067, TDD-0068, TDD-0069 | spec-0011 | 3 rows: TDD-0021..0023 | the paths listed for each under `### Files the source rows changed` | Remove the work-log validator from validate's sdd composition; Withdraw `catalog/worklog-entry.schema.md` from the shipped governed set; Remove the check that a stopped ledger owes a work-log record, with its two finding codes and the `worklogEntries.ts` module it read | Deleted with its test by `CR-20260925-0010` (its X2 disposition), so the row certifies nothing. See `### Re-run results: spec-0011` | CR-20260925-0010 |
| TDD-0067, TDD-0068, TDD-0069 | spec-0012 | 133 rows: TDD-0286, TDD-0293..0294, TDD-0336, TDD-0338..0339, TDD-0343, TDD-0345, TDD-0350, TDD-0355, TDD-0360..0361, TDD-0363..0364, TDD-0367..0369, TDD-0371..0383, TDD-0385..0388, TDD-0403..0408, TDD-0417..0419, TDD-0425..0427, TDD-0430..0435, TDD-0439..0442, TDD-0444..0452, TDD-0454, TDD-0458, TDD-0460..0462, TDD-0466..0471, TDD-0473..0474, TDD-0476, TDD-0479, TDD-0481..0485, TDD-0487..0488, TDD-0490..0495, TDD-0497, TDD-0501..0505, TDD-0509..0515, TDD-0518..0527, TDD-0561..0577 | the paths listed for each under `### Files the source rows changed` | Remove the work-log validator from validate's sdd composition; Withdraw `catalog/worklog-entry.schema.md` from the shipped governed set; Remove the check that a stopped ledger owes a work-log record, with its two finding codes and the `worklogEntries.ts` module it read | Matched through the package fallback. At `03762f3cf` every selector passes. The recorded proof of TDD-0469, TDD-0471, TDD-0497, TDD-0514..0515, TDD-0561..0577 replays as recorded (`qa-gatekeeper` PASS). `xspec-cr-a` and `xspec-cr-b` confirmed each holds. See `### Re-run results: spec-0012` | re-reviewed |
| TDD-0067, TDD-0068, TDD-0069 | spec-0012 | 3 rows: TDD-0496, TDD-0516..0517 | the paths listed for each under `### Files the source rows changed` | Remove the work-log validator from validate's sdd composition; Withdraw `catalog/worklog-entry.schema.md` from the shipped governed set; Remove the check that a stopped ledger owes a work-log record, with its two finding codes and the `worklogEntries.ts` module it read | Not `done` at `03762f3cf`: `CR-20260923-0001`, applied on main, reset the row, and its blocked table names it. A row that is not `done` certifies nothing the change can break. See `### Re-run results: spec-0012` | CR-20260923-0001 |
| TDD-0067, TDD-0068, TDD-0069 | spec-0012 | 25 rows: TDD-0337, TDD-0366, TDD-0389..0400, TDD-0415..0416, TDD-0421..0424, TDD-0428..0429, TDD-0437..0438, TDD-0443 | the paths listed for each under `### Files the source rows changed` | Remove the work-log validator from validate's sdd composition; Withdraw `catalog/worklog-entry.schema.md` from the shipped governed set; Remove the check that a stopped ledger owes a work-log record, with its two finding codes and the `worklogEntries.ts` module it read | The selector selects no test at `03762f3cf` (zero-selected capture), so no current run can confirm the obligation. The CR's `## Blocked downstream items` table names the row. See `### Re-run results: spec-0012` | CR-20260925-0001 |
| TDD-0067, TDD-0068, TDD-0069 | spec-0012 | 23 rows: TDD-0295, TDD-0340..0341, TDD-0344, TDD-0346, TDD-0348..0349, TDD-0351, TDD-0353..0354, TDD-0356..0359, TDD-0362, TDD-0365, TDD-0370, TDD-0453, TDD-0456..0457, TDD-0459, TDD-0465, TDD-0486 | the paths listed for each under `### Files the source rows changed` | Remove the work-log validator from validate's sdd composition; Withdraw `catalog/worklog-entry.schema.md` from the shipped governed set; Remove the check that a stopped ledger owes a work-log record, with its two finding codes and the `worklogEntries.ts` module it read | The selector selects no test at `03762f3cf` (zero-selected capture), so no current run can confirm the obligation. The CR's `## Blocked downstream items` table names the row. See `### Re-run results: spec-0012` | CR-20260925-0002 |
| TDD-0067, TDD-0068, TDD-0069 | spec-0012 | 1 row: TDD-0342 | the paths listed for each under `### Files the source rows changed` | Remove the work-log validator from validate's sdd composition; Withdraw `catalog/worklog-entry.schema.md` from the shipped governed set; Remove the check that a stopped ledger owes a work-log record, with its two finding codes and the `worklogEntries.ts` module it read | The selector selects no test at `03762f3cf` (zero-selected capture), so no current run can confirm the obligation. The CR's `## Blocked downstream items` table names the row. See `### Re-run results: spec-0012` | CR-20260925-0003 |
| TDD-0067, TDD-0068, TDD-0069 | spec-0012 | 1 row: TDD-0507 | the paths listed for each under `### Files the source rows changed` | Remove the work-log validator from validate's sdd composition; Withdraw `catalog/worklog-entry.schema.md` from the shipped governed set; Remove the check that a stopped ledger owes a work-log record, with its two finding codes and the `worklogEntries.ts` module it read | The selector selects no test at `03762f3cf` (zero-selected capture), so no current run can confirm the obligation. The CR's `## Blocked downstream items` table names the row. See `### Re-run results: spec-0012` | CR-20260925-0004 |
| TDD-0067, TDD-0068, TDD-0069 | spec-0012 | 4 rows: TDD-0489, TDD-0498..0500 | the paths listed for each under `### Files the source rows changed` | Remove the work-log validator from validate's sdd composition; Withdraw `catalog/worklog-entry.schema.md` from the shipped governed set; Remove the check that a stopped ledger owes a work-log record, with its two finding codes and the `worklogEntries.ts` module it read | The selector selects no test at `03762f3cf` (zero-selected capture), so no current run can confirm the obligation. The CR's `## Blocked downstream items` table names the row. See `### Re-run results: spec-0012` | CR-20260925-0005 |
| TDD-0067, TDD-0068, TDD-0069 | spec-0012 | 8 rows: TDD-0455, TDD-0463..0464, TDD-0472, TDD-0475, TDD-0480, TDD-0506, TDD-0508 | the paths listed for each under `### Files the source rows changed` | Remove the work-log validator from validate's sdd composition; Withdraw `catalog/worklog-entry.schema.md` from the shipped governed set; Remove the check that a stopped ledger owes a work-log record, with its two finding codes and the `worklogEntries.ts` module it read | The selector selects no test at `03762f3cf` (zero-selected capture), so no current run can confirm the obligation. The CR's `## Blocked downstream items` table names the row. The CR is still `open`. See `### Re-run results: spec-0012` | CR-20260925-0007 |
| TDD-0067, TDD-0068, TDD-0069 | spec-0013 | 2 rows: TDD-0110..0111 | the paths listed for each under `### Files the source rows changed` | Remove the work-log validator from validate's sdd composition; Withdraw `catalog/worklog-entry.schema.md` from the shipped governed set; Remove the check that a stopped ledger owes a work-log record, with its two finding codes and the `worklogEntries.ts` module it read | Matched through the package fallback. At `03762f3cf` every selector passes. The recorded proof of TDD-0110..0111 replays as recorded (`qa-gatekeeper` PASS). `xspec-cr-c` confirmed each holds. See `### Re-run results: spec-0013` | re-reviewed |
| TDD-0067 | spec-0013 | 12 rows: TDD-0019..0030 | the paths listed for it under `### Files the source rows changed` | Remove the work-log validator from validate's sdd composition | Not `done` at `03762f3cf`: `CR-20260913-0009`, applied on main, reset the row, and its blocked table names it. A row that is not `done` certifies nothing the change can break. See `### Re-run results: spec-0013` | CR-20260913-0009 |
| TDD-0067, TDD-0068, TDD-0069 | spec-0014 | 3 rows: TDD-0018..0019, TDD-0034 | the paths listed for each under `### Files the source rows changed` | Remove the work-log validator from validate's sdd composition; Withdraw `catalog/worklog-entry.schema.md` from the shipped governed set; Remove the check that a stopped ledger owes a work-log record, with its two finding codes and the `worklogEntries.ts` module it read | Matched through the package fallback. At `03762f3cf` every selector passes. The recorded proof of TDD-0019 replays as recorded (`qa-gatekeeper` PASS). `xspec-cr-c` confirmed each holds. See `### Re-run results: spec-0014` | re-reviewed |
| TDD-0067, TDD-0068, TDD-0069 | spec-0014 | 1 row: TDD-0033 | the paths listed for each under `### Files the source rows changed` | Remove the work-log validator from validate's sdd composition; Withdraw `catalog/worklog-entry.schema.md` from the shipped governed set; Remove the check that a stopped ledger owes a work-log record, with its two finding codes and the `worklogEntries.ts` module it read | The selector selects no test at `03762f3cf` (zero-selected capture), so no current run can confirm the obligation. The CR's `## Blocked downstream items` table names the row. See `### Re-run results: spec-0014` | CR-20260913-0002 |
| TDD-0067, TDD-0068, TDD-0069 | spec-0014 | 1 row: TDD-0009 | the paths listed for each under `### Files the source rows changed` | Remove the work-log validator from validate's sdd composition; Withdraw `catalog/worklog-entry.schema.md` from the shipped governed set; Remove the check that a stopped ledger owes a work-log record, with its two finding codes and the `worklogEntries.ts` module it read | The selector passes, but TC-0014-0009 expects a REVISE to block verify. The selected blocks assert stale-sidecar migration errors. See `### Re-run results: spec-0014` | CR-20260913-0005 |
...
- The spec-0002 `TDD-0009` and `TDD-0010` and spec-0013 `TDD-0019` … `TDD-0030` lines
  keep rows the earlier `TDD-0067` table named. Main reset them before the re-run.
...
### Re-run results: spec-0002

- Blocked rows: 6. `re-reviewed` 2, `CR-20260912-0003` 2, `CR-20260924-0003` 2.
- Selector runs: 4 rows, run read-only with `--reporter=verbose` on the clean clone `tmp/cross-spec-mutations-qa/repo` at `03762f3cf`. 4 rows select and pass their tests (4 of 4 selected tests passed, 0 failed).
- Saved results: `tmp/xspec/selector-results2.json`, the batch logs `tmp/xspec/logs2/selector-01.log`, `tmp/xspec/logs2/selector-04.log`, `tmp/xspec/logs2/selector-12.log`, `tmp/xspec/logs2/selector-31.log`.
- Mutation replays, audited by `qa-gatekeeper`: first audit, clean clone at `25d428853` (`tmp/xspec/audit.json`): 2 rows, 1 replayed fresh (`tmp/xspec/runs-*.json`, logs in `tmp/xspec/logs/`) and 1 reusing an earlier replay whose test and source hashes it recomputed (`tmp/cross-spec-mutations-qa/runs*.json`); second audit at `03762f3cf` (`tmp/xspec/audit2.json`): 2 rows (earlier proof kept, its files unchanged: 2; runs in `tmp/xspec/runs2-new.json` and `tmp/xspec/runs2-replay.json`, logs in `tmp/xspec/logs2/`).
- Completion review: `completion-reviewer` `xspec-cr-c` (`tmp/xspec/review-R7-R9.json`), reviewed revision `03762f3cf`.

### Re-run results: spec-0003

- Blocked rows: 51. `re-reviewed` 42, `CR-20260925-0006` 3, `CR-20260925-0010` 6.
- Selector runs: 45 rows, run read-only with `--reporter=verbose` on the clean clone `tmp/cross-spec-mutations-qa/repo` at `03762f3cf`. 45 rows select and pass their tests (129 of 129 selected tests passed, 0 failed).
- Saved results: `tmp/xspec/selector-results2.json`, the batch logs `tmp/xspec/logs2/selector-01.log`, `tmp/xspec/logs2/selector-13.log`, `tmp/xspec/logs2/selector-15.log`, `tmp/xspec/logs2/selector-16.log`, `tmp/xspec/logs2/selector-17.log`.
- Mutation replays, audited by `qa-gatekeeper`: first audit, clean clone at `25d428853` (`tmp/xspec/audit.json`): 45 rows, 11 replayed fresh (`tmp/xspec/runs-*.json`, logs in `tmp/xspec/logs/`) and 34 reusing an earlier replay whose test and source hashes it recomputed (`tmp/cross-spec-mutations-qa/runs*.json`); second audit at `03762f3cf` (`tmp/xspec/audit2.json`): 40 rows (first replay of a row new since the earlier run: 1; replay after a test or source hash changed: 9; earlier proof kept, its files unchanged: 30; runs in `tmp/xspec/runs2-new.json` and `tmp/xspec/runs2-replay.json`, logs in `tmp/xspec/logs2/`).
- Completion review: `completion-reviewer` `xspec-cr-a` (`tmp/xspec/review-R1-R3.json`), `completion-reviewer` `xspec-cr-c` (`tmp/xspec/review-R7-R9.json`), reviewed revision `03762f3cf`.

### Re-run results: spec-0004

- Blocked rows: 40. `CR-20260925-0015` 11, `re-reviewed` 28, `CR-20260913-0001` 1.
- Selector runs: 40 rows, run read-only with `--reporter=verbose` on the clean clone `tmp/cross-spec-mutations-qa/repo` at `03762f3cf`. 29 rows select and pass their tests (45 of 45 selected tests passed, 0 failed); 11 rows select none (TDD-0003..0004, TDD-0006, TDD-0008..0010, TDD-0012, TDD-0015, TDD-0023, TDD-0025, TDD-0034), captured one row at a time with exit 0 and every test skipped.
- Saved results: `tmp/xspec/selector-results2.json`, the batch logs `tmp/xspec/logs2/selector-02.log`, `tmp/xspec/logs2/selector-04.log`, `tmp/xspec/logs2/selector-05.log`, `tmp/xspec/logs2/selector-12.log`, `tmp/xspec/logs2/selector-14.log`, `tmp/xspec/logs2/selector-15.log`, `tmp/xspec/logs2/selector-17.log`, `tmp/xspec/logs2/selector-23.log`, `tmp/xspec/logs2/selector-28.log`, `tmp/xspec/logs2/selector-29.log`, `tmp/xspec/logs2/selector-30.log`, `tmp/xspec/logs2/selector-31.log`, `tmp/xspec/zero-captures2.json` and `tmp/xspec/logs2/zero/`.
- Mutation replays, audited by `qa-gatekeeper`: second audit at `03762f3cf` (`tmp/xspec/audit2.json`): 1 row (first replay of a row new since the earlier run: 1; runs in `tmp/xspec/runs2-new.json` and `tmp/xspec/runs2-replay.json`, logs in `tmp/xspec/logs2/`).
- Completion review: `completion-reviewer` `xspec-cr-c` (`tmp/xspec/review-R7-R9.json`), reviewed revision `03762f3cf`.

### Re-run results: spec-0006

- Blocked rows: 25. `re-reviewed` 23, `CR-20260925-0007` 1, `CR-20260925-0016` 1.
- Selector runs: 25 rows, run read-only with `--reporter=verbose` on the clean clone `tmp/cross-spec-mutations-qa/repo` at `03762f3cf`. 24 rows select and pass their tests (37 of 37 selected tests passed, 0 failed); 1 row selects none (TDD-0020), captured one row at a time with exit 0 and every test skipped.
- Saved results: `tmp/xspec/selector-results2.json`, the batch logs `tmp/xspec/logs2/selector-05.log`, `tmp/xspec/logs2/selector-06.log`, `tmp/xspec/logs2/selector-07.log`, `tmp/xspec/logs2/selector-17.log`, `tmp/xspec/logs2/selector-18.log`, `tmp/xspec/logs2/selector-19.log`, `tmp/xspec/logs2/selector-21.log`, `tmp/xspec/logs2/selector-25.log`, `tmp/xspec/zero-captures2.json` and `tmp/xspec/logs2/zero/`.
- Mutation replays, audited by `qa-gatekeeper`: first audit, clean clone at `25d428853` (`tmp/xspec/audit.json`): 7 rows, 6 replayed fresh (`tmp/xspec/runs-*.json`, logs in `tmp/xspec/logs/`) and 1 reusing an earlier replay whose test and source hashes it recomputed (`tmp/cross-spec-mutations-qa/runs*.json`); second audit at `03762f3cf` (`tmp/xspec/audit2.json`): 7 rows (earlier proof kept, its files unchanged: 6; CR row with no replayable proof: 1; runs in `tmp/xspec/runs2-new.json` and `tmp/xspec/runs2-replay.json`, logs in `tmp/xspec/logs2/`).
- Second replay round at `03762f3cf`, for rows first classified as selector-only: TDD-0029, 9 mutant runs, the full recorded set of each, each restored byte-identically and followed by a passing GREEN (`tmp/xspec/runs3.json`, `tmp/xspec/audit3.json`, logs in `tmp/xspec/logs3/`). `completion-reviewer` `xspec-cr-a` ruled them in its round 2.
- Note: For `TDD-0029`, M2, M2a and M2b survive at the site they were recorded at, which main moved, and fail when rebuilt at the moved site under D2 of the split grilling session.
- Note: `TDD-0029`'s M1 mutant now fails the row's test at line 140, not at line 137, where it was recorded. Main's ok arm registers a check when nothing drifted, so line 137 no longer turns red. The test still fails on the predicate, and no repair is owed in this change.
- Completion review: `completion-reviewer` `xspec-cr-a` (`tmp/xspec/review-R1-R3.json`), reviewed revision `03762f3cf`.

### Re-run results: spec-0008

- Blocked rows: 2. `re-reviewed` 2.
- Selector runs: 2 rows, run read-only with `--reporter=verbose` on the clean clone `tmp/cross-spec-mutations-qa/repo` at `03762f3cf`. 2 rows select and pass their tests (12 of 12 selected tests passed, 0 failed).
- Saved results: `tmp/xspec/selector-results2.json`, the batch logs `tmp/xspec/logs2/selector-06.log`.
- Mutation replays, audited by `qa-gatekeeper`: first audit, clean clone at `25d428853` (`tmp/xspec/audit.json`): 2 rows, 0 replayed fresh (`tmp/xspec/runs-*.json`, logs in `tmp/xspec/logs/`) and 2 reusing an earlier replay whose test and source hashes it recomputed (`tmp/cross-spec-mutations-qa/runs*.json`); second audit at `03762f3cf` (`tmp/xspec/audit2.json`): 2 rows (earlier proof kept, its files unchanged: 2; runs in `tmp/xspec/runs2-new.json` and `tmp/xspec/runs2-replay.json`, logs in `tmp/xspec/logs2/`).
- Completion review: `completion-reviewer` `xspec-cr-c` (`tmp/xspec/review-R7-R9.json`), reviewed revision `03762f3cf`.

### Re-run results: spec-0010

- Blocked rows: 10. `re-reviewed` 6, `CR-20260912-0003` 3, `CR-20260913-0013` 1.
- Selector runs: 10 rows, run read-only with `--reporter=verbose` on the clean clone `tmp/cross-spec-mutations-qa/repo` at `03762f3cf`. 7 rows select and pass their tests (11 of 11 selected tests passed, 0 failed); 3 rows select none (TDD-0006..0008), captured one row at a time with exit 0 and every test skipped.
- Saved results: `tmp/xspec/selector-results2.json`, the batch logs `tmp/xspec/logs2/selector-01.log`, `tmp/xspec/logs2/selector-04.log`, `tmp/xspec/logs2/selector-06.log`, `tmp/xspec/logs2/selector-12.log`, `tmp/xspec/logs2/selector-14.log`, `tmp/xspec/logs2/selector-19.log`, `tmp/xspec/zero-captures2.json` and `tmp/xspec/logs2/zero/`.
- Completion review: `completion-reviewer` `xspec-cr-c` (`tmp/xspec/review-R7-R9.json`), reviewed revision `03762f3cf`.

### Re-run results: spec-0011

- Blocked rows: 3. `CR-20260925-0010` 3.
- Mutation replays, audited by `qa-gatekeeper`: first audit, clean clone at `25d428853` (`tmp/xspec/audit.json`): 3 rows, 3 replayed fresh (`tmp/xspec/runs-*.json`, logs in `tmp/xspec/logs/`) and 0 reusing an earlier replay whose test and source hashes it recomputed (`tmp/cross-spec-mutations-qa/runs*.json`).
- Completion review: `completion-reviewer` `xspec-cr-c` (`tmp/xspec/review-R7-R9.json`), reviewed revision `03762f3cf`.

### Re-run results: spec-0012

- Blocked rows: 198. `re-reviewed` 133, `CR-20260925-0002` 23, `CR-20260925-0001` 25, `CR-20260925-0003` 1, `CR-20260925-0007` 8, `CR-20260925-0005` 4, `CR-20260925-0004` 1, `CR-20260923-0001` 3.
- Selector runs: 195 rows, run read-only with `--reporter=verbose` on the clean clone `tmp/cross-spec-mutations-qa/repo` at `03762f3cf`. 133 rows select and pass their tests (373 of 373 selected tests passed, 0 failed); 62 rows select none (TDD-0295, TDD-0337, TDD-0340..0342, TDD-0344, TDD-0346, TDD-0348..0349, TDD-0351, TDD-0353..0354, TDD-0356..0359, TDD-0362, TDD-0365..0366, TDD-0370, TDD-0389..0400, TDD-0415..0416, TDD-0421..0424, TDD-0428..0429, TDD-0437..0438, TDD-0443, TDD-0453, TDD-0455..0457, TDD-0459, TDD-0463..0465, TDD-0472, TDD-0475, TDD-0480, TDD-0486, TDD-0489, TDD-0498..0500, TDD-0506..0508), captured one row at a time with exit 0 and every test skipped.
- Saved results: `tmp/xspec/selector-results2.json`, the batch logs `tmp/xspec/logs2/selector-01.log`, `tmp/xspec/logs2/selector-02.log`, `tmp/xspec/logs2/selector-03.log`, `tmp/xspec/logs2/selector-04.log`, `tmp/xspec/logs2/selector-05.log`, `tmp/xspec/logs2/selector-08.log`, `tmp/xspec/logs2/selector-09.log`, `tmp/xspec/logs2/selector-10.log`, `tmp/xspec/logs2/selector-11.log`, `tmp/xspec/logs2/selector-12.log`, `tmp/xspec/logs2/selector-13.log`, `tmp/xspec/logs2/selector-17.log`, `tmp/xspec/logs2/selector-21.log`, `tmp/xspec/logs2/selector-23.log`, `tmp/xspec/logs2/selector-24.log`, `tmp/xspec/logs2/selector-25.log`, `tmp/xspec/logs2/selector-26.log`, `tmp/xspec/logs2/selector-27.log`, `tmp/xspec/logs2/selector-28.log`, `tmp/xspec/logs2/selector-29.log`, `tmp/xspec/logs2/selector-30.log`, `tmp/xspec/zero-captures2.json` and `tmp/xspec/logs2/zero/`.
- Mutation replays, audited by `qa-gatekeeper`: first audit, clean clone at `25d428853` (`tmp/xspec/audit.json`): 8 rows, 0 replayed fresh (`tmp/xspec/runs-*.json`, logs in `tmp/xspec/logs/`) and 8 reusing an earlier replay whose test and source hashes it recomputed (`tmp/cross-spec-mutations-qa/runs*.json`); second audit at `03762f3cf` (`tmp/xspec/audit2.json`): 19 rows (first replay of a row new since the earlier run: 11; replay after a test or source hash changed: 1; earlier proof kept, its files unchanged: 7; runs in `tmp/xspec/runs2-new.json` and `tmp/xspec/runs2-replay.json`, logs in `tmp/xspec/logs2/`).
- Second replay round at `03762f3cf`, for rows first classified as selector-only: TDD-0497, TDD-0514..0515, 7 mutant runs, the full recorded set of each, each restored byte-identically and followed by a passing GREEN (`tmp/xspec/runs3.json`, `tmp/xspec/audit3.json`, logs in `tmp/xspec/logs3/`). `completion-reviewer` `xspec-cr-a` ruled them in its round 2.
- Note: `TDD-0372` is an older pass-immediate row with no falsifiability trio. It records no proof, so its passing selector is what the review needs. The missing trio is for the spec-0012 owner to repair.
- Completion review: `completion-reviewer` `xspec-cr-a` (`tmp/xspec/review-R1-R3.json`), `completion-reviewer` `xspec-cr-b` (`tmp/xspec/review-R4-R6.json`), `completion-reviewer` `xspec-cr-c` (`tmp/xspec/review-R7-R9.json`), reviewed revision `03762f3cf`.

### Re-run results: spec-0013

- Blocked rows: 15. `re-reviewed` 2, `CR-20260913-0009` 12, `CR-20260925-0010` 1.
- Selector runs: 2 rows, run read-only with `--reporter=verbose` on the clean clone `tmp/cross-spec-mutations-qa/repo` at `03762f3cf`. 2 rows select and pass their tests (2 of 2 selected tests passed, 0 failed).
- Saved results: `tmp/xspec/selector-results2.json`, the batch logs `tmp/xspec/logs2/selector-15.log`.
- Mutation replays, audited by `qa-gatekeeper`: second audit at `03762f3cf` (`tmp/xspec/audit2.json`): 2 rows (first replay of a row new since the earlier run: 2; runs in `tmp/xspec/runs2-new.json` and `tmp/xspec/runs2-replay.json`, logs in `tmp/xspec/logs2/`).
- Completion review: `completion-reviewer` `xspec-cr-c` (`tmp/xspec/review-R7-R9.json`), reviewed revision `03762f3cf`.

### Re-run results: spec-0014

- Blocked rows: 7. `CR-20260913-0005` 3, `re-reviewed` 3, `CR-20260913-0002` 1.
- Selector runs: 7 rows, run read-only with `--reporter=verbose` on the clean clone `tmp/cross-spec-mutations-qa/repo` at `03762f3cf`. 6 rows select and pass their tests (25 of 25 selected tests passed, 0 failed); 1 row selects none (TDD-0033), captured one row at a time with exit 0 and every test skipped.
- Saved results: `tmp/xspec/selector-results2.json`, the batch logs `tmp/xspec/logs2/selector-01.log`, `tmp/xspec/logs2/selector-08.log`, `tmp/xspec/logs2/selector-21.log`, `tmp/xspec/zero-captures2.json` and `tmp/xspec/logs2/zero/`.
- Mutation replays, audited by `qa-gatekeeper`: first audit, clean clone at `25d428853` (`tmp/xspec/audit.json`): 1 row, 0 replayed fresh (`tmp/xspec/runs-*.json`, logs in `tmp/xspec/logs/`) and 1 reusing an earlier replay whose test and source hashes it recomputed (`tmp/cross-spec-mutations-qa/runs*.json`); second audit at `03762f3cf` (`tmp/xspec/audit2.json`): 1 row (earlier proof kept, its files unchanged: 1; runs in `tmp/xspec/runs2-new.json` and `tmp/xspec/runs2-replay.json`, logs in `tmp/xspec/logs2/`).
- Completion review: `completion-reviewer` `xspec-cr-c` (`tmp/xspec/review-R7-R9.json`), reviewed revision `03762f3cf`.
...
- Blocked rows: 28. `re-reviewed` 28.
- Selector runs: 28 rows, run read-only with `--reporter=verbose` on the clean clone `tmp/cross-spec-mutations-qa/repo` at `03762f3cf`. 28 rows select and pass their tests (28 of 28 selected tests passed, 0 failed).
- Saved results: `tmp/xspec/selector-results2.json`, the batch logs `tmp/xspec/logs2/selector-22.log`.
- Completion review: `completion-reviewer` `xspec-cr-c` (`tmp/xspec/review-R7-R9.json`), reviewed revision `03762f3cf`.

### Re-run results: spec-0017

- Blocked rows: 6. `re-reviewed` 6.
- Selector runs: 6 rows, run read-only with `--reporter=verbose` on the clean clone `tmp/cross-spec-mutations-qa/repo` at `03762f3cf`. 6 rows select and pass their tests (6 of 6 selected tests passed, 0 failed).
- Saved results: `tmp/xspec/selector-results2.json`, the batch logs `tmp/xspec/logs2/selector-19.log`.
- Mutation replays, audited by `qa-gatekeeper`: first audit, clean clone at `25d428853` (`tmp/xspec/audit.json`): 6 rows, 6 replayed fresh (`tmp/xspec/runs-*.json`, logs in `tmp/xspec/logs/`) and 0 reusing an earlier replay whose test and source hashes it recomputed (`tmp/cross-spec-mutations-qa/runs*.json`); second audit at `03762f3cf` (`tmp/xspec/audit2.json`): 6 rows (earlier proof kept, its files unchanged: 6; runs in `tmp/xspec/runs2-new.json` and `tmp/xspec/runs2-replay.json`, logs in `tmp/xspec/logs2/`).
- Completion review: `completion-reviewer` `xspec-cr-c` (`tmp/xspec/review-R7-R9.json`), reviewed revision `03762f3cf`.
...
- `TDD-0067` … `TDD-0071` reached `done` after the first full CI checkpoint.
  `CR-20260925-0010` then withdrew them with their tests, and their ledger rows are
  tombstoned. `TDD-0072`, recorded in `.qfai/evidence/implement-spec-0004.md`, is the
  one spec-0004 row of this change that stays.
- Seven rows of other specs whose selector passes take a CR, on `completion-reviewer`'s
  ruling that the pass does not show the obligation holds. No user has adjudicated
  those rulings (`## Cross-spec obligations`).
...
Pending. No ATDD row of this change remains in this file. Every cross-spec entry
carries `re-reviewed` or a `CR-*`. P8 has not been reached, and no review pack is open.
