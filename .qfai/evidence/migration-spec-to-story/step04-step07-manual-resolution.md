...
EX-0003-0023's old warning outcome is retired (DEC-0723). The obligation was
rewritten to the current error outcome and placed as EX-0001-0037-01 and
EX-0001-0037-02, as `step04-ex-provenance.csv` records.
...
the active BRs. Their old text is preserved in the archive, and each was placed
only when a new authoritative contract row cited a meaningful new example. No
active BR is disposed as retired solely because automatic placement failed.
...
new-AC, and contract-reference checks. The old source of all three scopes was
removed with the old packs at cutover and is preserved in the archive. The
...
matching archived lines and SHA-256. They left the old source with the old
packs and remain in the archive. The 36 retired BR rows have explicit step 7
report lines, except pre-archived BR-0011-0007, in
`retired-rule-disposition.md`.

## Step 8 annotation follow-up

`step05-step08-tc-disposition.csv` has 18 TC-only or ambiguous TC decisions
and their step 8 real `no usable ID mapping` annotation locations. Three of the
18 IDs appear in four such annotations. `step08-unmapped-tc-other.csv` records
...
Three BF-0003 examples were coverage placeholders of the same kind: old
EX-0006-0009, EX-0007-0010 and EX-0007-0011 asserted only that an example
exists for BR-0006-0006, BR-0007-0005 and BR-0007-0010. DEC-0730 records their
replacement by observable examples.

| ID-map target   | Old sources                | Current state                                                              |
| --------------- | -------------------------- | -------------------------------------------------------------------------- |
| EX-0003-0013-05 | EX-0007-0010, TC-0007-0010 | Removed; BR-0204 cites the successor EX-0003-0014-03 under AC-0003-0014-01 |
| EX-0003-0013-06 | EX-0007-0011, TC-0007-0011 | ID reused; the row asserts normalized guardrail ordering for BR-0209       |
| EX-0003-0001-04 | EX-0006-0009, TC-0006-0011 | ID reused; the row asserts per-severity doctor summary counts for BR-0183  |

The reused IDs are not renumbered. `id-map.json` keeps all three step 4
targets, as it does for DEC-0719. `.qfai/evidence/atdd-BF-0003.md` records the
test side of the change.

Before cutover, full-profile validation stopped at `QFAI-LAYOUT-001` while old
`spec-*` entries existed under `.qfai/spec`. `cutover-old-pack-audit.csv`
verifies the archived source and exact residual sections for nine non-B packs.
The four residual files in spec-0001 and spec-0003 were formatted after the
initial audit; the audit CSV records their source hashes at that time. Archived
...
`.qfai/spec/spec-*` directories is zero.

On the tracked tree after removal, `node scripts/check-dogfood-backlog.mjs --profile full`
reports 613 inherited missing-test-annotation errors and no
