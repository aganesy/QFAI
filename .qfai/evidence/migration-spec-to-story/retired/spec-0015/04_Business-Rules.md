...
- On trigger, the skill body MUST write `.qfai/evidence/decision/<ISO8601-ts>.json` containing `{question, answer, scope, operatorIdentity, timestamp, envelopeContractClause}`.
- `.qfai/evidence/decision/` MUST be **tracked** in version control: the managed `.gitignore` block negates it after `.qfai/evidence/*` (`QFAI_GITIGNORE_GOVERNANCE_NEGATIONS`). It is a governance record carrying operator approval, not regenerable stage evidence like `.qfai/evidence/prototyping/`.
...
- Per DR-0271, `qfai audit log` (SHOULD) lists `.qfai/evidence/decision/<ts>.json` records newest-first with filters `--scope`, `--operator`, `--clause` (on `envelopeContractClause`) and `--format table|json` defaulting to `table`.
- SHOULD-level because `.qfai/evidence/decision/` is human-readable JSON; the CLI is an ergonomic improvement, not a hard requirement.
...
## BR-0015-0018: The Card Frontmatter Is the Only Agent Definition

- AC-Refs: AC-0015-0001
- An agent's card under `.qfai/assistant/agent/` is its only definition.
- The card's frontmatter carries `kind`, `domain`, `mission`, `replaces`, `owned_artifacts`, `tool_profile`, `permission_profile` and `specialization_tags`. Its `name` is the agent's ID.
- `mission` is a key of its own. `description` stays the host's one-line summary and is not read as the mission.
- Every reader of those fields, including the runtime readers of `kind`, reads them from the card frontmatter.
- The host integration files, the generated Codex agent TOML among them, are generated from the card and never authored by hand.
- No `agent-catalog.yml` is written or read, so no catalog entry carries a `developer_instructions` copy of a card body.
- The assistant tree is stated in `.qfai/contracts/cli/qfai-init.md#the-assistant-tree`.

## BR-0015-0019: Routing and Review-Profile Defaults Live in the Package

- AC-Refs: AC-0015-0023
- The default skill routing and review profiles are built into the package.
- No `agent-routing.yml` and no `review-profiles.yml` is written into a project, and none is read from one.
- A project with no override runs on the defaults, so a package upgrade reaches it without a merge step.

## BR-0015-0020: A Project Override Replaces the Matching Default Whole

- AC-Refs: AC-0015-0023
- A project's overrides live in `qfai.config.yaml`: a routing entry in the `routing:` list, keyed by its skill, and a review profile in the `reviewProfiles:` map, keyed by its name.
- An override replaces the whole default entry with the same key. Fields of the two are never merged.
- An override whose key matches no default is added.
- Every reader of routing or review profiles, `qfai validate` included, reads the defaults with the overrides applied.
- An override that names an agent with no card is an unknown agent to `qfai validate` (`QFAI-AGENT-008`).
- The optional review modes (`devils-advocate`, `pattern-doubler`) are built in only. `qfai.config.yaml` has no key that overrides them.
- The override keys are stated in `.qfai/contracts/cli/qfai-init.md#configuration`.

## BR-0015-0021: The Migration Skill's Routing Entry

- AC-Refs: AC-0015-0024
- The routing defaults QFAI ships carry an entry for `qfai-migration-spec-to-story` with three phases, in order:
  - `plan`: `requirements-analyst` and `solution-architect` mandatory; `solution-architect` blocking.
  - `execution`: `devops-ci-engineer` mandatory.
  - `review`: `completion-reviewer` and `architecture-reviewer` mandatory and blocking.
- The entry's review profile is `architecture-heavy`.
- The skill's `roles:` names every agent the entry binds and every reviewer `architecture-heavy` selects, and its `routing-profile:` is `architecture-heavy`.
- The skill's surface is stated in `.qfai/contracts/cli/qfai-migration-spec-to-story.md`.
