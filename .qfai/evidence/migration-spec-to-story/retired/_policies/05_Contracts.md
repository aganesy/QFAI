...
| CLI-ATDD         | qfai atdd scaffold (CLI) | `.qfai/contracts/cli/qfai-atdd-scaffold.md`           | spec-0008 CLI surface — placeholder skeletons per AC (`--story`) or per flow (`--flow`) in the story tree; escalation and `D-SCAFFOLD-PLACEHOLDER` keyed by AC or BF ID                                                                                                                                                         |
| CLI-MIGR         | migration scripts        | `.qfai/contracts/cli/qfai-migration-spec-to-story.md` | spec-0018 — the scripts bundled with `/qfai-migration-spec-to-story`: invocation, `--dry-run`, write set, exit codes, report sections; no `qfai` subcommand                                                                                                                                                                     |
...
| CLI-DOC          | qfai doctor              | `.qfai/contracts/cli/qfai-doctor.md`                  | Doctor checks and repair guidance.                                                                                                                                                                                                                                                                                              |
| CLI-AUDIT        | qfai audit log           | `.qfai/contracts/cli/qfai-audit.md`                   | Decision-record query surface.                                                                                                                                                                                                                                                                                                  |
| CLI-PITER        | qfai prototyping iterate | `.qfai/contracts/cli/qfai-prototyping-iterate.md`     | Iterate subcommand details.                                                                                                                                                                                                                                                                                                     |
| CLI-REPORT       | qfai report              | `.qfai/contracts/cli/qfai-report.md`                  | Report rendering and flow scope.                                                                                                                                                                                                                                                                                                |
| CLI-DISCUSSION   | qfai discussion          | `.qfai/contracts/cli/qfai-discussion.md`              | Discussion-pack listing and active pointer.                                                                                                                                                                                                                                                                                     |
| CLI-HUP          | qfai handoff upgrade     | `.qfai/contracts/cli/qfai-handoff-upgrade.md`         | Lossless legacy handoff conversion.                                                                                                                                                                                                                                                                                             |
| CLI-GUARD        | qfai guardrails          | `.qfai/contracts/cli/qfai-guardrails.md`              | Explicit decision-guardrail grammar and command surface.                                                                                                                                                                                                                                                                       |
| CLI-STORY-AUTHOR | story-tree authoring     | `.qfai/contracts/cli/story-tree-authoring.md`         | Authoring boundaries shared by SDD and migration.                                                                                                                                                                                                                                                                               |
| CLI-CONFIG       | configuration            | `.qfai/contracts/cli/configuration.md`                | Project override boundary.                                                                                                                                                                                                                                                                                                      |
| CLI-DELIVERY     | delivery workflow        | `.qfai/contracts/cli/delivery-workflow.md`            | ATDD, implementation, and verification handoffs.                                                                                                                                                                                                                                                                                |
| CLI-ROUTING      | assistant routing        | `.qfai/contracts/cli/assistant-routing.md`            | Agent cards, routing, and review profiles.                                                                                                                                                                                                                                                                                      |
| CLI-RESEARCH     | research protocol        | `.qfai/contracts/cli/research-protocol.md`            | Provider-independent research stages and source handling.                                                                                                                                                                                                                                                                       |

The command files own their public surfaces; the subject files own rules
shared by several commands or skills. The active index at
`<paths.contractsDir>/contracts.md` lists each file after cutover.
...
Triage: `## Triage (2026-09-23 spec-to-story)` in each owning spec's
`09_delta.md` and in `_policies/10_delta.md`. Phase 0 decisions:
`.qfai/evidence/sdd-batch-20260923100952585.md#phase-0-grilling-decisions`.

### Contract Index additions

CLI-ATDD and CLI-MIGR, registered once in the `### CLI Contracts` table above.

### Contract Index updates (existing rows)

Each contract below gains a `## Story-tree layout` section stating its target
state, with the Triage row and delivery phase of every commitment. The text
before that section stays in force until the REMOVE row that retires it lands.

| Short ID   | Updates                                                                                                                                                                           |
| ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| CLI-VAL    | Story-tree finding families with severity, message and profile; test obligations per layer; validator-read rows; drift gate and append-only tables; BR forms; text output grammar |
| CLI-INIT   | Spec-tree seeding, config paths, old-layout handling, migration skill install, assistant-tree destinations, `--upgrade-assistant-tree` retarget, new ID shapes                    |
| CLI-DOC    | Config-driven paths, CAP catalog check removed, singular skill path                                                                                                               |
| CLI-PROT   | The UI contract `CON-UI-NNNN` replaces the spec as the prototyping unit; renamed fields and show command; pre-change records refused                                              |
| CLI-PITER  | `--primary-ui-contract`, per-contract evidence paths, `.qfai/prototype/`                                                                                                          |
| CLI-WLOG   | Scope, promotion and links keyed on BF and DEC IDs and `decisions.md`                                                                                                             |
| EVID-PROT2 | At P7 the evidence path becomes `iter-NN/CON-UI-NNNN/<screen>.review.json` and the frozen set `uiContractsCovered[]`, as CLI-PROT's `## Story-tree layout` states                 |
| DCON-031   | At P7 the lock is frozen at the 03-contract step of `/qfai-sdd`, which has no Phase 0, and lives under `<contractsDir>/design/`                                                   |

CLI-WFSET is unchanged: no shipped workflow template names the old layout's
paths (decision N13).
...
| Pairing                       | Reconciled                                                                                                                                                                           |
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| CLI-VAL ↔ CLI-WLOG            | Link shapes, `scope` values and the promotion rule are stated once, in CLI-WLOG, and CLI-VAL points to it                                                                            |
| CLI-VAL ↔ CLI-ATDD            | Skeleton homes (`integration/<US-ID>/`, `e2e/`) are the layers the test-obligation family counts; `D-SCAFFOLD-PLACEHOLDER` keyed by AC or BF ID in both                              |
| CLI-VAL ↔ CLI-INIT            | A fresh tree holds no BF, US, AC or EX, so the default `full` run raises no obligation; the old-layout condition is the same in both                                                 |
| CLI-INIT ↔ CLI-MIGR           | Init installs the skill even on an old layout; the scripts reuse init's link and managed-block writers; step 1's renames match init's singular directories, `skill.local` included   |
| CLI-MIGR ↔ CLI-VAL            | The completion check is `qfai validate` reporting no layout or chain error; the untested list is the test-obligation family; rule forms written by step 7 are the ones CLI-VAL reads |
| CLI-INIT ↔ CLI-DOC ↔ CLI-PROT | `contractsDir` stays a key with the default `.qfai/spec/03_contract`; the design lock is read at `<contractsDir>/design/` by doctor and prototyping alike                            |
| CLI-PROT ↔ CLI-PITER          | Field, flag, config key and evidence path names are the same in both                                                                                                                 |
