...
- NFR-0001: Coverage completeness -- all required US covered by E2E, all required TC by Integration, all required CON-API by API tests; on the story tree, every BF covered by an E2E test and every AC by an integration or API test
- NFR-0002: Annotation consistency -- every generated ATDD test includes correct QFAI annotations per layer
- NFR-0003: Forbidden reference enforcement -- zero TC annotations in E2E/API test files; on the story tree, zero `QFAI:AC-`/`QFAI:EX-` annotations in E2E test files and zero `QFAI:BF-` outside E2E
...
- REQ-0002: Test Volume Estimator -- compute US/TC/CON signals with evidence table; on the story tree, BF and AC signals
- REQ-0003: Coverage obligations checklist -- E2E covers US, Integration covers TC, API covers CON-API; on the story tree, E2E covers BF, and integration or API covers AC
...
- REQ-0157: `qfai atdd scaffold --spec spec-NNNN` -- reads the spec's test cases and emits `tests/atdd/spec-NNNN/<TC-ID>.test.*` skeletons. Each skeleton imports the test-framework primitives and contains `// TODO: implement assertion for <TC-ID>` plus comment references to the related `US-*` / `CON-API-*`. `qfai validate` emits `D-SCAFFOLD-PLACEHOLDER` (severity warning) while the TODO remains and escalates it to error after 3 validate cycles (default; configurable through `qfai.config.yaml#atdd.scaffoldEscalateCycles` / DR-0272). Idempotent: a rerun does not overwrite non-TODO content. On the story tree the command takes exactly one of `--story US-NNNN-NNNN` (one skeleton per AC at `<testsDir>/integration/<US-ID>/<AC-ID>.test.<ext>`, carrying `QFAI:AC-NNNN-NNNN-NN`) and `--flow BF-NNNN` (one skeleton at `<testsDir>/e2e/<BF-ID>.test.<ext>`, carrying `QFAI:BF-NNNN`), and `D-SCAFFOLD-PLACEHOLDER` is keyed by the AC or BF ID.
...
- Discussion pack `discussion-20260923063306456` (spec-to-story restructure):
  ...
  - `discussion-20260923063306456#REQ-0009`, `discussion-20260923063306456#REQ-0010`: ATDD writes the BF (E2E) and AC (integration or API) tests
  - `discussion-20260923063306456#REQ-0015`: `qfai atdd scaffold --story US-NNNN-NNNN` and `--flow BF-NNNN`
...
- v1.9.2 Second-Wave (copy-down for execution): `qfai atdd scaffold --spec spec-NNNN` generates `tests/atdd/spec-NNNN/<TC-ID>.test.*` skeletons (a TODO marker plus `US-*` / `CON-API-*` comment references) from the spec's test cases, idempotently. A remaining TODO raises `D-SCAFFOLD-PLACEHOLDER` (warning), which escalates to error after 3 validate cycles (`atdd.scaffoldEscalateCycles`, default 3 / DR-0272). On the story tree the command takes `--story US-NNNN-NNNN` or `--flow BF-NNNN` instead, as REQ-0157 states.
