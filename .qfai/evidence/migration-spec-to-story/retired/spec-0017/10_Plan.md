...
Three edits, in the order the work is done. None introduces an architectural
element: each stays inside one script, one rule document or one shipped rule
file.

1. **P1 — the shape table, in the guards' pull request.**
   - `.agents/rules/distributed-surface.local.md` gains one row each for
     `DEC-NNNN`, `OQ-NNNN`, `BF-NNNN`, `US-NNNN-NNNN`, `AC-NNNN-NNNN-NN`,
     `EX-NNNN-NNNN-NN` and `BR-NNNN`.
   - It states the sample band a shipped template may use: `0001` to `0009` on
     every four-digit segment, `01` to `09` on a two-digit tail. Every existing
     row stays, `DEC-NNNN-NNNN` included (BR-0017-0072, EX-0017-0073).
   - The table lands in the commit that teaches the three guards the seven
     shapes (spec-0003 BR-0003-0056). The same commit extends the
     `distributed-surface.local.md` clause list in
     `packages/qfai/tests/integration/agentsRulesSurface.test.ts` with one token
     per new row and one for the sample band, as
     `.agents/rules/root-additions-policy.local.md` step 4 requires of every
     rule clause.
   - The same commit changes the smoke-test row of the table of guards in
     `distributed-surface.local.md`: the pattern set and the scan now live in
     `packages/qfai/tests/helpers/distributedSurfaceScan.ts`, which
     `distributedSurfaceLeakage.test.ts` calls (spec-0003 TC-0003-0071).
   - P1 is a pull request of its own, merged before the rest of the work. Its
     first commit removes the out-of-band IDs that shipped files already
     carry, re-spelling each to what the guard accepts (spec-0003
     BR-0003-0027). The guard commit follows, so it is green when it lands.
     Neither commit edits template content for the new layout.
2. **P6 — the mirror follows the renamed tree.** In
   `scripts/link-assistant-tree.mjs`:
   - The link plan is derived from the packaged tree's own listing, as today,
     so the root `rule/`, `skill/`, `agent/` and `prompt/` links need no name
     list in the script (BR-0017-0070, EX-0017-0071).
   - `--check` reports every entry under the root tree whose name the packaged
     tree does not have, a link as well as a real directory. `localOnlyUnder`
     skips symbolic links today, so a stale `skills` link passes; it stops
     skipping them (BR-0017-0071, EX-0017-0072).
   - `LOCAL_ONLY_ALLOWED` and its `process/migrations/` entry are removed, so a
     memo left there is drift.
   - The adopter-owned list is still read from `ADOPTER_OWNED_CATALOG_FILES` in
     `packages/qfai/src/core/assistantAssetProvenance.ts`. While `catalog/`
     holds those four files, they are the only real files kept.
   - In this repository `pnpm sync:ssot` replaces the plural links with the
     singular ones in the rename commit, and the `pnpm ci:gate:ssot` diff is
     empty after it.
3. **P6 — the lane map becomes a section.**
   - The content of `catalog/test-layers-ci-lanes.md` becomes a section of
     `packages/qfai/assets/init/.qfai/assistant/rule/test-layers.md`, and the
     sibling file is deleted from both trees (BR-0017-0062, BR-0017-0066).
   - The section opens by stating that it adds no layer token and does not
     activate per-level routing (BR-0017-0063).
   - It uses neither form `packages/qfai/src/core/layerPolicy.ts` reads as
     vocabulary: no `### L…` heading and no `layer-*` tag.
   - The width-budget key `assistant/catalog/test-layers.md` in
     `packages/qfai/src/core/doctor/assetLineBudget.ts` follows the file.
     The merged file is about 743 lines, under the 800-line ceiling in the
     same module.
   - `packages/qfai/tests/assets/layerCiLaneMapping.test.ts` is rewritten
     against the section. Its account of why the map is a sibling stops being
     true at this point.

Alternatives rejected:

- A link check that accepts both the plural and the singular names. No window
  accepts both layouts, and BR-0017-0071 makes a retired name drift.
- Keeping the lane map as a sibling file under `rule/`. The merge into the
  layer catalog is the adopted resolution, and the section is constrained
  instead.
...
- **The link check runs over planted trees at L3.** The script resolves the
  repository from its own location. Each case therefore copies it,
  `assistantAssetProvenance.ts` and a small packaged tree into a temporary
  root, outside git, where every path counts as tracked.
  `packages/qfai/tests/scripts/assistantTreeLinks.test.ts` is the home; its
  existing cases keep reading the real tree.
- **Cases that must not share one.**
  - After a sync over the renamed packaged tree: four links, `catalog/`
    holding only the four adopter-owned files, and an empty
    `pnpm ci:gate:ssot` diff (TC-0017-0093).
  - A leftover `skills` link (TC-0017-0094). It gets a case of its own because
    it is the one the script passes today.
  - A memo left under `process/migrations/` (TC-0017-0094). It gets a case of
    its own because it is the one the script allows today.
- **The shape table is read, then pinned.** TC-0017-0095 reads the table for
  the seven rows, the sample band and every base-branch row. The token block
  in `agentsRulesSurface.test.ts`, written under
  `.agents/rules/root-additions-policy.local.md` step 4, pins one token per
  new row, not one for the table: removing a single row fails the suite while
  the file still looks complete. It is a repository test of the rule file,
  not a case of this spec.
- **The lane-map cases move with the section.** The cases in
  `layerCiLaneMapping.test.ts` read the section in `rule/test-layers.md`. The
  vocabulary check reads the warning count before and after the merge rather
  than the absence of a sibling file.
...
- **NFR-0005 (guard breadth).** The commit that teaches the three guards the
  seven shapes also carries the shape table. It lands in the P1 pull request,
  which edits no template content for the new layout.
  - NFR-0005 asks for "zero template edits in the same diff". The P1 pull
    request reads that as nothing beyond re-spelling IDs: its first commit
    re-spells the out-of-band IDs shipped templates already carry, so the
    guard is green when it lands. The authority for that reading is the
    user's answer P3-C2 in the batch record, which put the guards, the shape
    table and that clean-up in one pull request of their own and kept the
    spec text as written.
  - A breach shows as any of:
    - a commit that changes `lint-shipping.ts`,
      `check-no-internal-version-leakage.sh`, the pattern set in
      `packages/qfai/tests/helpers/distributedSurfaceScan.ts` or the shape
      table without the others;
    - a commit that does so and also touches a path under
      `packages/qfai/assets/`;
    - a P1 pull request that changes a file under a `templates/` directory
      for anything other than re-spelling an ID.
  - TC-0017-0095 reads the result.

- **NFR-0015 (vocabulary does not grow).** The merged section carries no layer
  token. A breach shows as a higher layer-vocabulary warning count from
  `qfai validate` on this repository after the P6 merge than before it, or as
  a change to the built-in token set in `layerPolicy.ts`.
...
| Risk                                                                                                                                                                                    | Likelihood / impact | Mitigation                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Trigger to act                                                                                                         |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| The shape table and the guards' pattern set drift apart                                                                                                                                 | med / med           | They land in one commit; the `agentsRulesSurface.test.ts` token block pins one token per new row (`.agents/rules/root-additions-policy.local.md` step 4)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | The table and a guard pattern changing in different commits, or a guard pattern with no row in the table               |
| NFR-0005 and BR-0017-0072 name a pull request as their unit, and the P2 to P8 pull request edits shipped templates                                                                      | med / med           | The guard patterns and the shape table land in the P1 pull request, merged before the rest. Its clean-up commit only re-spells IDs to what the guard accepts and edits no template content for the new layout                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | A guard pattern for one of the seven shapes, or a shape-table row, changing in the P2 to P8 pull request               |
| The 03 and 04 edits make the `tdd` and `full` dogfood lanes report `QFAI-TRACE-001` for the ledger rows that link `07_Decisions.md`, `.github/workflows/release.yml` and `package.json` | high / med          | The findings are pinned in `scripts/dogfood-backlog.json` under `tdd` and `full` as a one-off exception to the rule that its numbers only go down (user answer Q1 in the batch record). The pin is shared with spec-0014 and holds 13 findings per profile, keyed by file: `packages/qfai/src/core/validators/prototypingEvidence.ts` 2, `packages/qfai/src/core/report.ts` 1, `packages/qfai/src/cli/commands/prototypingCertify.ts` 1, `.qfai/specs/spec-0017/07_Decisions.md` 6, `.github/workflows/release.yml` 2 and `package.json` 1. A change that touches a keyed file clears that key's findings, so it re-pins both profiles in the same commit (`node scripts/check-dogfood-backlog.mjs --profile <name> --pin`), which strikes the key. P7 retires the ledgers and strikes what is left. Every other file stays at zero | A `QFAI-TRACE-001` finding on a file the pin does not name, or the pin still present after its lifting condition holds |
| A stale plural link passes `--check`, because the check skips symbolic links today                                                                                                      | high / med          | `--check` reports every root entry the packaged tree does not have, link or not; TC-0017-0094 plants a link                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | `link-assistant-tree --check` exiting 0 over a tree that keeps a `skills` link                                         |
| Once P7 empties `catalog/`, `ADOPTER_OWNED_CATALOG_FILES` parses to nothing and the script throws, which stops `pnpm sync:ssot`                                                         | high / med          | The P7 change that moves the four adopter-owned files updates the script's reader in the same commit, so an empty list means no real files kept                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | `link-assistant-tree` throwing on `ADOPTER_OWNED_CATALOG_FILES` in the P7 change                                       |
| The merged lane-map section is read as layer vocabulary                                                                                                                                 | low / med           | No `### L…` heading and no `layer-*` tag in the section; NFR-0015's count before and after                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | The layer-vocabulary warning count rising after the merge                                                              |
