...
## 2026-09-24 — Composite examples and their criterion owners

DR-0017-0023 records three P7 EX resolutions. EX-0017-0003 is narrowed to AC-0017-0001 (old TC-0017-0002); EX-0017-0074 is appended for AC-0017-0002 (old TC-0017-0005). EX-0017-0030 moves to new AC-0017-0039, preserving old TC-0017-0033/0035 and their missing-numbers refusal. EX-0017-0068 moves to new AC-0017-0040, preserving old TC-0017-0086/0087 and their clean/change/re-pin cases. The old EX text remains represented in the split or clarified cases; no outcome is retired.
...
- Date: 2026-09-24
- Primary: spec-to-story run — the assistant-tree mirror, the lane map and the shape table
- Tags: toolchain, assistant-tree-mirror, distributed-surface
- Summary: adds US-0017-0010 and US-0017-0011, AC-0017-0037 and AC-0017-0038, BR-0017-0070 to
  BR-0017-0072, EX-0017-0071 to EX-0017-0073, TC-0017-0093 to TC-0017-0095, spec-local REQ-0016
  and REQ-0017, and ledger rows TDD-0102 to TDD-0104, TDD-0106 and TDD-0107. The lane map becomes a section of
  `rule/test-layers.md` beside its current sibling-file clauses. Details under
...
DELTA-0002. No approved Change Request ordered this run. This file has no
`## Decision Log`, so no DL entry is written.

### Change summary

- Added: US-0017-0010 and US-0017-0011; AC-0017-0037 and AC-0017-0038;
  BR-0017-0070 to BR-0017-0072; EX-0017-0071 to EX-0017-0073; TC-0017-0093 to
  TC-0017-0095; spec-local REQ-0016 and REQ-0017; ledger rows TDD-0102 to
  TDD-0104 (integration) and TDD-0106 and TDD-0107 (E2E, one per new story),
  all at `todo`.
- Withdrawn: TC-0017-0096 and its ledger row TDD-0105 (review-cycle ruling
  D9). The case read a repository test file, and no business rule of this
  spec needs it: the token block in `agentsRulesSurface.test.ts` is still
  written in P1, because `.agents/rules/root-additions-policy.local.md` step
  4 requires one for every rule clause. Both IDs are deleted and not reused.
- Changed in place, with the new-assistant-tree clause added beside the
  current one (ruling X1): `01_Spec.md` Scope In, the test-layer policy
  reference and the lane-map paragraph; US-0017-0009; AC-0017-0032;
  AC-0017-0034; BR-0017-0062; BR-0017-0063; BR-0017-0066.
- `06_Test-Cases.md` Coverage summary restated for 38 criteria, 73 examples
  and 95 cases. The ledger's `Owning module` column was widened; no existing
...
| ------------------------------------------------------------------------------------------------- | ----- | ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| REQ-0024, NFR-0004 (the shape table moves with the guards)                                        | P1    | P1                       | The shape table in `.agents/rules/distributed-surface.local.md` gains the seven rows and the sample band (BR-0017-0072), in the commit that teaches the three guards the seven shapes (spec-0003 BR-0003-0056). The same commit adds one token per new row and one for the sample band to the `distributed-surface.local.md` clause list in `packages/qfai/tests/integration/agentsRulesSurface.test.ts`, as `.agents/rules/root-additions-policy.local.md` step 4 requires, and changes the smoke-test row of the table of guards to name `packages/qfai/tests/helpers/distributedSurfaceScan.ts`, where the pattern set and the scan now live. The commit before it removes the out-of-band IDs shipped files already carry, re-spelled to what the guard accepts; it re-counts them with the guards' own patterns (the estimates are 54 in 25 files and about 55 in 24). Neither commit edits template content for the new layout                                                                            |
| REQ-0017, REQ-0018, NFR-0009 (the mirror covers the renamed tree; the lane map leaves `catalog/`) | P6    | P2 to P8                 | In `scripts/link-assistant-tree.mjs`, `--check` stops skipping symbolic links, and `LOCAL_ONLY_ALLOWED` with its `process/migrations/` entry is removed (BR-0017-0070, BR-0017-0071). `pnpm sync:ssot` replaces the plural links with the singular ones in the rename commit. The lane map becomes a section of `rule/test-layers.md`, about 743 lines against the 800-line ceiling, and the width-budget key in `packages/qfai/src/core/doctor/assetLineBudget.ts` follows the file. `packages/qfai/tests/assets/layerCiLaneMapping.test.ts` is rewritten against the section. Drops the current sibling-file and `catalog/` clauses from `01_Spec.md`, US-0017-0009, AC-0017-0032 (its cross-link and header clauses), AC-0017-0034, BR-0017-0062, BR-0017-0063 and BR-0017-0066. Rewrites EX-0017-0062, EX-0017-0063, EX-0017-0066 and TC-0017-0076, TC-0017-0077, TC-0017-0078, TC-0017-0081, TC-0017-0082 with their tests; TDD-0076 to TDD-0078, TDD-0081 and TDD-0082 go stale and are re-verified there |
| (no row of this spec)                                                                             | P6    | Migration-memo exception | The guards stop neutralising the memo's file name (spec-0003). The same pull request drops the memo exception from the exceptions list in `.agents/rules/distributed-surface.local.md` and its `cannot be renamed` token from `agentsRulesSurface.test.ts`, so the rule and the guards change together (NFR-0005)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |

No row removes an item, so nothing is retired and no ledger row is tombstoned.

### Co-changes the landing carries

- The co-change list in the P6 Triage row's Rationale.
- The `QFAI-TRACE-001` pin in `scripts/dogfood-backlog.json`, which the user
  approved as a one-off exception (Q1) and which is not there yet. It is
  shared with spec-0014 and holds 13 findings in each of `tdd` and `full`,
  keyed by file: `packages/qfai/src/core/validators/prototypingEvidence.ts`
  (two), `packages/qfai/src/core/report.ts` (one),
  `packages/qfai/src/cli/commands/prototypingCertify.ts` (one),
  `.qfai/specs/spec-0017/07_Decisions.md` (six),
  `.github/workflows/release.yml` (two) and `package.json` (one). It goes
  into the P2 to P8 pull request. A change touching a keyed file re-pins both
  profiles in the same commit, which strikes that key; P7 retires the ledgers
  and strikes the rest.
- `06_Test-Cases.md` cites
  `.qfai/assistant/catalog/test-layers.md#layer-derivation-procedure-normative`,
  and `16_Traceability-ledger.md` names `catalog/test-layers-ci-lanes.md` and
  `catalog/test-layers.md`. Both are repointed at P6.
- The NFR-0015 layer-vocabulary warning count, read before and after the P6
  merge.

### Recorded drift

- Once P7 empties `catalog/`, `ADOPTER_OWNED_CATALOG_FILES` in
  `packages/qfai/src/core/assistantAssetProvenance.ts` parses to nothing and
  `scripts/link-assistant-tree.mjs` throws, which stops `pnpm sync:ssot`. The
  plan names it as a risk for the P7 change. No item covers it. Open as
  OQ-0182.
- The Triage row states the size as 36 criteria and 92 cases, the count before
  this run. After it the spec holds 38 and 95. It still owns exactly
  `CAP-0017`, so the routing rule under
  `## Size signal and reasoned non-split` holds and no SPLIT follows; the
  ownership review is owed again at the next append.
- This run's spec-pack edits land in the P2–P8 pull request. The P1 pull
  request adds the test for TC-0017-0095 without its
  trace annotation; the P2–P8 pull request, which adds the rows, adds the
  annotations too. Settled by the orchestrator (batch record § Phase 4
  decisions).
- The Phase 2 writer's view that the new-assistant-tree clause in a Gherkin
  criterion would read better as a Scenario of its own, with its own layout
  Given. Recorded; not acted on.

### Adopted

- The lane map merges into `rule/test-layers.md` as a MODIFY (Phase 2 node
  G6 C1, option 1, griller ruling). BR-0017-0063 is restated, not removed: the
  section states that it adds no layer token and does not activate per-level
  routing, which is OC-76's own purpose. OC-76's placement clause is amended
  in `_policies`.
- Two new stories carry the mirror and the shape table (G6-14), and the
  overlay suite pins one token per new shape (G6-13). That pin is written
  under `.agents/rules/root-additions-policy.local.md` step 4, not as a case
  of this spec (ruling D9).
- NFR-0005's "zero template edits in the same diff" is read, for the P1 pull
  request, as nothing beyond re-spelling IDs. The authority is the user's
  answer P3-C2, which put the guards, the shape table and the re-spelling
  commit in one pull request of their own and kept the spec text as written.
- The `QFAI-TRACE-001` findings the 03 and 04 edits raise are pinned as a
  one-off exception (Phase 2 question Q1, answered by the user).
- The shape table lands with the guards in the P1 pull request (P3-C2).

### Rejected

- Candidate: a link check that accepts both the plural and the singular
  directory names.
- Reason: no window accepts both layouts (Phase 0 node N05), and
  BR-0017-0071 makes a retired name drift.
- DO NOT: allow a plural link to stand beside its singular replacement.
- Temptation: it keeps `--check` green on both sides of the rename commit.

- Candidate: keep the lane map as a sibling file under `rule/` (G6 C1
  option 2).
- Reason: it reverses part of OQ-0177 and departs from the approved pack,
  which abolishes or absorbs the file.
- DO NOT: recreate `test-layers-ci-lanes.md` under another directory.
- Temptation: a sibling keeps the map out of the file the layer-policy loader
  reads.

- Candidate: remove BR-0017-0063 with its example and test case.
- Reason: its obligation survives the merge as a statement inside the
  section, and a removal would need a new approved row.
- DO NOT: drop the disclaimer obligation because the header it names goes.
- Temptation: once the sibling file is gone the header rule looks vacuous.

- Candidate: write a decision record only so that `07_Decisions.md` changes
  and `QFAI-TRACE-001` stays quiet.
- Reason: a decision record states a decision; one written to satisfy a
  check records nothing.
- DO NOT: author a record to move a diff.
- Temptation: it clears the finding without a pin.

- Candidate: hold the 03 and 04 edits until P7.
- Reason: the user chose to author all fifteen specs in this run and to pin
  the finding instead.
- DO NOT: defer this spec's items to the landing change.
- Temptation: the finding disappears with no exception to record.
