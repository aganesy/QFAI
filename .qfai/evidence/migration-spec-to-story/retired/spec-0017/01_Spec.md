...
- Test-layer policy: `.qfai/assistant/catalog/test-layers.md` (with the
  `rule/ skill/ agent/ prompt/` assistant tree, `.qfai/assistant/rule/test-layers.md`). Per-level
  routing is a target state that is not enforced (DTC-9); the mapping document must not read as
  activating it.
...
- REQ-0014: Layer-to-CI mapping in a parser-invisible home — the mapping from the layer taxonomy
  ...
  belongs fails `link-assistant-tree --check` in `pnpm ci:lint` (DTC-20). With the
  `rule/ skill/ agent/ prompt/` assistant tree, the mapping is a section of the layer catalog
  `rule/test-layers.md`, authored under `packages/qfai/assets/init/.qfai/assistant/rule/`, and the
  sibling file is retired (OQ-0177). The section names no layer token or layer heading outside the
  built-in set, and states that it does not activate per-level routing.
...
- REQ-0016: The assistant-tree mirror covers the renamed tree — with the
  `rule/ skill/ agent/ prompt/` assistant tree, `pnpm sync:ssot` links the repository-root
  `rule/`, `skill/`, `agent/` and `prompt/` at the packaged assets, and `pnpm ci:gate:ssot`
  reports no diff afterwards. A retired plural or removed directory name left under the root tree
  is drift that `link-assistant-tree --check` reports.
  (upstream: `discussion-20260923063306456#REQ-0017`, `discussion-20260923063306456#REQ-0018`,
  `discussion-20260923063306456#NFR-0009`, must)
- REQ-0017: The shape table moves with the guards — `.agents/rules/distributed-surface.local.md`
  lists the new internal ID shapes `DEC-NNNN`, `OQ-NNNN`, `BF-NNNN`, `US-NNNN-NNNN`,
  `AC-NNNN-NNNN-NN`, `EX-NNNN-NNNN-NN` and `BR-NNNN` with their sample band, in the same pull
  request that changes the three guards' pattern set (NFR-0005).
  (upstream: `discussion-20260923063306456#REQ-0024`, `discussion-20260923063306456#NFR-0004`,
  must)
...
Those are the `discussion-20260804173914356` rows. REQ-0016 and REQ-0017 above descend from
`discussion-20260923063306456` instead, a pack with no `Surface` column; both concern this
repository's own tooling.

## Entry points

- US range in this spec: US-0017-0001..US-0017-0011
...
- Notes: user stories map from discussion `DUS-001`, `DUS-005` and `DUS-006`. `US-0017-0009`
  ...
  of `OQ-0010`. `US-0017-0010` and `US-0017-0011` descend from `discussion-20260923063306456`,
  spec-local REQ-0016 and REQ-0017.
