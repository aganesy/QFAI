...
- `qfai prototyping` is not an active public orchestration command (only `iterate` / `certify` / `show-spec` sub-commands are public; `show-ui-contract` replaces `show-spec` on the story tree).
...
- On the story tree, only a UI contract declaring a `CON-UI-NNNN` ID and a `screens[]` entry is UI-bearing. No spec-level marker is read.
- Validate must not over-fire UI evidence rules for non-UI work or an incomplete UI contract file.
...
- Cycle 0 records `prototyping.json#designMdSha256 = sha256(DESIGN.md)` and asserts equality with `<paths.contractsDir>/design/DESIGN.md.lock.yaml#sha256`.
...
- `<paths.contractsDir>/design/design-system.yaml` is generated post-loop as a deterministic byte-equivalent mirror of root `DESIGN.md` token tables (color / typography / radius / shadow). It is NOT extracted from the final iter HTML.
...
- On the story tree the invocation MUST resolve every UI-bearing UI contract instead. A UI contract is UI-bearing iff its file under `<paths.contractsDir>/ui/` declares a `CON-UI-NNNN` ID and at least one `screens[]` entry; the `surface_type: ui-bearing` front matter and the prototyping title read from `01_Spec.md`, and a UI contract file named after a spec, are not signals. Zero UI-bearing UI contracts is the same no-op (`.qfai/contracts/cli/qfai-prototyping.md#story-tree-layout`, "The unit is the UI contract").
...
- Per-spec lineage rule from BR-0012-0017 is preserved at `cycles 0..9` granularity under multi-spec (one lineage per UI contract on the story tree).

## BR-0012-0030: Reviewer-driven Playwright session

- AC-Refs: AC-0012-0040
- The Reviewer sub-agent MUST itself launch Playwright (or equivalent harness) per spec × screen (per UI contract × screen on the story tree) per cycle and perform human-like operation (click / type / navigate / scroll) on the live prototype.
...
- On the story tree the payload names its UI contract as `uiContractId` in place of `specId`.
...
- On the story tree the AND runs across every `(UI contract, screen)` pair in the cycle-0 frozen `uiContractsCovered[]`, with the same three empty-array conditions, and the record MUST name every lagging UI contract ID.
...
- The runtime cycle ≥ 1 license-verify gate MUST reject any `imageSources[]` entry whose `attribution` is undefined, empty string, or whitespace-only with error code `license-missing-attribution` → exit 66. This is the runtime-side counterpart of the write-side recording rule above; the gate does not silently tolerate "field present but blank".
...
- On the story tree (`.qfai/contracts/cli/qfai-prototyping.md#story-tree-layout`, "Records written before the change"):
  - (b) records `sessionStatus` on the per-`(UI contract, screen)` review payload, and the diagnostic names the `(UI contract, screen)`.
  - (d) is any added or removed UI-bearing UI contract, and the diagnostic names it.
  - A `prototyping.json` that carries `specsCovered` or `frozenSpecsCovered`, or lacks `uiContractsCovered`, MUST exit 2 on `iterate` at cycle ≥ 1, on `certify` and on `show-ui-contract`, with a message saying to re-seed with `qfai prototyping iterate --cycle 0`.
...
- On the story tree the layout MUST be `iter-NN/CON-UI-NNNN/<screen>.review.json` only, and the path helpers MUST descend into `CON-UI-NNNN` with the same cleanup semantics.
...
- On the story tree certify MUST read the cycle-0 frozen `uiContractsCovered[]`, iterate per UI contract and name the missing `(UI contract, screen)` pair. Every entry MUST be a canonical `CON-UI-NNNN` before a review path is built from it. The certificate records `uiContractsCovered`, `convergedUiContracts[]` and `laggingUiContracts[]` in place of `specsCovered`, `convergedSpecs[]` and `laggingSpecs[]`.

## BR-0012-0037: Menu reachability is qualitative (not hard-fail)

- AC-Refs: AC-0012-0048
- The Reviewer SHOULD exercise every primary menu entry point declared by the spec (by the UI contract on the story tree) at least once during its Playwright session.
...
- Cycle 0 MUST freeze and persist (a) the resolved UI-bearing UI contract set as `uiContractsCovered[]` on the story tree and (b) the stock-photo license-class catalog (allowed sources + license tiers + attribution format) in cycle-0 evidence.
...
- When `--capture` is passed, iterate drives Playwright per the Capture contract; when `htmlSourceCopy: true`, iterate MUST copy from `.qfai/prototypes/iter-NN/<screen-id>.html` (`.qfai/prototype/iter-NN/<screen-id>.html` with the `rule/ skill/ agent/ prompt/` assistant tree; not `page.content()`).
...
- On the story tree `SKILL.md` MUST name the UI contract (`CON-UI-NNNN`) as the unit rather than the spec.
...
- On the story tree a `--cycle 0` re-seed over a `prototyping.json` that carries `specsCovered` or `frozenSpecsCovered`, or lacks `uiContractsCovered`, MUST delete none of the evidence under `iter-NN/spec-NNNN/` (`.qfai/contracts/cli/qfai-prototyping.md#story-tree-layout`, "Records written before the change").
...
- Post-convergence invariant: every `frozenSurfaceUnion` screen carries ≥ 1 `evidenceRefs[]` entry per kind (`screenshot` AND `html`) cross-spec (across UI contracts on the story tree, where `frozenSurfaceUnion` holds `CON-UI-NNNN` IDs).
