...
As an AI generator, I want `qfai prototyping iterate --cycle 0` to record `sha256(DESIGN.md)` into `prototyping.json#designMdSha256` and verify it matches `<paths.contractsDir>/design/DESIGN.md.lock.yaml#sha256`.
...
As an AI developer (skill operator), I want one `/qfai-prototyping` invocation to resolve every UI-bearing spec in the consumer project so that the project-wide prototype set evolves in a single run without per-invocation primary-spec selection. On the story tree the invocation resolves every UI-bearing UI contract (`CON-UI-NNNN`) instead. (REQ-0001, CHG-002)
...
As a reviewer, I want to write short-prose impressions of `operability`, `transitionFeel`, `crossScreenContinuity`, `userStoryFeel`, `acceptanceCriteriaFeel`, and `menuReachabilityFeel` per spec × screen (per UI contract × screen on the story tree) so that design and operability evaluation is qualitative-only (not reduced to AC-pass / transition-pass percentages). (REQ-0004, REQ-0005, CHG-002)
...
As a maintainer, I want the resolved spec set frozen at cycle 0 and recorded in cycle-0 evidence so that mid-run additions of new UI-bearing specs do not restart cycle 0 — they are deferred to the next invocation. On the story tree the frozen set is the resolved UI contract set, recorded as `uiContractsCovered[]`, and a UI contract added mid-run is deferred the same way. (REQ-0011, CHG-002)
...
As a downstream consumer, I want the public `/qfai-prototyping` skill surface aligned to single-spec language (the UI contract as the unit, on the story tree) with `resolveSurfaceUnion()` removed from the public surface, so that the doc-vs-impl drift identified across SKILL.md and the contracts collapses. (REQ-0012-0065)

## US-0012-0130

As a maintainer, I want accepted-iter content mirrored to `.qfai/evidence/prototyping/screenshots/<screen-id>.png` and `.qfai/evidence/prototyping/html/<screen-id>.html` on convergence with screen-ids normalised to underscore casing end-to-end, so that the aggregate-dir and per-spec dir (per-UI-contract dir on the story tree) use the same form. (REQ-0012-0066)
