...
- BR-Ref: BR-0012-0009
- Given non-UI work and a UI contract file with no `CON-UI-NNNN` ID or no `screens[]` entry
- When prototyping execution scope is determined
- Then that file is excluded from prototyping execution without reading a spec-level marker
...
- Then cycle-0 evidence records the frozen UI contract set in `uiContractsCovered[]`; this set is the SSOT for subsequent cycles' per-contract loops and drift checks.
