...
- A clause marked "on the story tree" holds where the project's specification is the story-based `.qfai/spec/` tree, and replaces the clause beside it, which holds in the spec-pack layout. A path marked "with the `rule/ skill/ agent/ prompt/` assistant tree" holds once the assistant tree uses those directory names. `qfai validate` decides which layout a project is in.

### v1.9.2 Second-Wave behavior (copied down for implementers)

- **Cycle-0 skeleton emission (REQ-0150, DR-0261 / DR-0273):** `qfai prototyping iterate --cycle 0 --emit-skeletons` (opt-in; default behavior unchanged, no v1.9.1 regression) emits one DESIGN.md-token-styled placeholder HTML per `screens[].id` in `frozenSurfaceUnion`, with NO per-screen LLM call at cycle 0. `--skeleton-mode full|placeholder|stub` (default `placeholder`) tunes fidelity per-run. Acceptance signal: after convergence every `frozenSurfaceUnion` screen carries ≥ 1 `evidenceRefs[]` entry per kind (`screenshot` AND `html`) cross-spec (across UI contracts on the story tree, where `frozenSurfaceUnion` holds `CON-UI-NNNN` IDs).
...
- NFR-0004 (this delta): per-cycle wall-time per spec (per UI contract on the story tree) ≤ 5 min target with soft-warning on overrun
- NFR-0005 (this delta): bounded Playwright retries (N=3, exponential backoff) before recording FAIL
- NFR-0006 (this delta): all artifacts under per-spec evidence root `.qfai/evidence/prototyping/iter-NN/spec-NNNN/` (`iter-NN/CON-UI-NNNN/` on the story tree)
...
- NFR-0009 (this delta): single-command operator UX produces runnable per-spec (per UI contract on the story tree) artifacts + handoff yaml without follow-up prompts
...
- root `DESIGN.md` is brand SSOT; sha256 frozen at the SDD Phase 0 step (at the `/qfai-sdd` 03-contract step on the story tree); cycle ≥1 hash mismatch halts at exit 2.
- Non-empty `designMdViolations` blocks convergence (DESIGN.md compliance gate).
- Cycle-0 freezes both (a) the resolved UI-bearing spec set (UI contract set on the story tree) and (b) the stock-photo license-class catalog; both are the SSOT for all subsequent cycles, and mid-run drift in either is treated as lock drift (exit 2).
- Mid-run additions of new UI-bearing specs (UI contracts on the story tree) are deferred to the next `/qfai-prototyping` invocation (no cycle-0 restart).
...
- New / renamed entry points: `resolveAllUiBearingSpecs()` (replaces `resolvePrimaryPrototypingSpec()`); `licenseVerify()` (new, exit-66 gate); path helpers `iterationDirPerSpec` / `iterationReviewPathPerSpec` / `findIterationReviewFiles` / `findStaleIterDirs` / `deleteStaleIterDirs` descend into `spec-NNNN` (`CON-UI-NNNN` on the story tree)
...
- Contracts: `.qfai/contracts/cli/qfai-prototyping.md` (new CLI contract for `iterate` / `certify` / `show-spec`); its `#story-tree-layout` section and `.qfai/contracts/cli/qfai-prototyping-iterate.md#story-tree-layout` state the behaviour on the story tree and with the `rule/ skill/ agent/ prompt/` assistant tree
- Cert: `.qfai/evidence/prototyping/completion-certificate.json` aggregated per spec from cycle-0 frozen `specsCovered[]`; on the story tree it is aggregated per UI contract from `uiContractsCovered[]` and records `uiContractsCovered`, `convergedUiContracts[]` and `laggingUiContracts[]`
...
- REQ-0012-0037: P7 retires the former 130/410-line budget. Skill quality is checked against the shipped asset contract, readable instructions, and repository gates without a fixed line-count ceiling.
...
- discussion-20260923063306456#REQ-0001: on the story tree the prototyping unit is the UI contract `CON-UI-NNNN`. Resolution, the evidence namespace, `uiContractsCovered[]`, certify, the primary pin and `show-ui-contract` work per UI contract, and each per-spec requirement above reads per UI contract there. A `prototyping.json` that carries `specsCovered` or `frozenSpecsCovered`, or lacks `uiContractsCovered`, is exit 2 with a re-seed instruction, and the re-seed keeps the evidence under `iter-NN/spec-NNNN/`.
- discussion-20260923063306456#REQ-0014: on the story tree `DESIGN.md.lock` is frozen at the `/qfai-sdd` 03-contract step.
- discussion-20260923063306456#REQ-0018: with the `rule/ skill/ agent/ prompt/` assistant tree the prototype directory is `.qfai/prototype/`.
- discussion-20260923063306456#REQ-0005: the design lock and `design-system.yaml` live under `<paths.contractsDir>/design/`.
...
- REQ-0012-0059 (REQ-0106): Per `DR-0277`, QFAI-PROT-002 applies a **cap** to `proseCritique` and **selects** the unit from the text. Text carrying CJK is measured in CJK characters against the character cap; anything else is measured in whitespace-separated words against the word cap. Only the selected unit's cap applies, and neither unit has a lower bound. A Japanese fixture of 800–1500 characters and an English fixture of 200–500 words both pass, as does a shorter critique in either. Error text over the cap MUST name the count form measured, the cap and the actual count.
...
- REQ-0012-0061 (REQ-0109): `qfai prototyping iterate` MUST accept the opt-in `--capture` flag (default OFF). When passed, iterate drives Playwright per the Capture contract in `iterate-plan.json` (per-screen `viewport: { width, height }`, `deviceScaleFactor`, `waitUntil`, `htmlSourceCopy: boolean`) and writes `iter-NN/<screen-id>.{png,html}` for every `screens[]` entry. When `htmlSourceCopy: true`, iterate MUST copy source HTML from `.qfai/prototypes/iter-NN/<screen-id>.html` (`.qfai/prototype/iter-NN/<screen-id>.html` with the `rule/ skill/ agent/ prompt/` assistant tree; NOT `page.content()`) to avoid runtime style-block injection. Default-OFF posture preserves `DR-0012-0029` (no PNG / HTML / interaction capture); amendment is pinned by `DR-0012-0031` (parallel agent assignment).
- REQ-0012-0062 (REQ-0110): `qfai prototyping iterate` MUST accept the opt-in `--auto-serve` flag (default OFF). When passed, iterate MUST call its server runner once, MUST invoke the teardown the runner returns at the end of the cycle and on SIGINT, MUST continue when the runner reports a recovered prior owner, and MUST exit 2 with the runner's reason on stderr when the runner refuses. The default runner is the in-process server `REQ-0012-0076` states. Default-OFF posture preserves `DR-0012-0029`; amendment is pinned by `DR-0012-0031`.
- REQ-0012-0063 (REQ-0111): On iteration completion, `iterate` MUST write `iterations[i]` with non-null `commitSha` (sentinel `"uncommitted"` accepted when no HEAD commit applies), non-empty `proseCritique`, `scores`, `layoutAntiPatternsDetected`, `designMdViolations`, `pivotDirective`, `reviewerId`, and `evidenceRefs[]` (entries shaped `{ kind: "screenshot" | "html", path: "iter-NN/<screen-id>.<ext>" }` for every declared `screens[].id`). On convergence (exit 64), `iterate` MUST set top-level `acceptedIterationIndex` and `stopReason ∈ {"converged", "max-iterations", "license-verify-fail", "input-error"}`. `qfai validate --profile prototyping --fail-on error` MUST PASS on a converged iterate output without orchestrator post-processing.
...
- REQ-0012-0068 (REQ-0118): On every non-converged cycle, `iterate` MUST print a one-screen summary naming the top-3 categories blocking exit-64 with concrete offenders: `designMdViolations` (count + first offender `kind=color,…,path,line`), `layoutAntiPatternsDetected` (count + first lap code), `blockingFindings` (count + the first line the reviewer wrote). The summary follows the canonical form `[BLOCKED] exit-64 prevented by: N designMdViolations (top: …), M anti-patterns (top: lap-NNN), K blockingFindings (top: …)`.
- REQ-0012-0069 (REQ-0119): When `primarySpecId` validation rejects an input, the error text MUST read literally `primarySpecId must be a 4-digit zero-padded string (e.g. "0001"); received <input>`. Per OQ-0112 = **SHOULD normalize**, `iterate` SHOULD also accept `1` / `"1"` / `"01"` / `"0001"` on input and normalize internally to `spec-0001`; when the SHOULD is shipped, the error appears only when the value is wholly unparseable as a positive integer ≤ 9999. On the story tree the pin is `prototyping.primaryUiContract` or `--primary-ui-contract`, which accepts only the full `CON-UI-NNNN` form: any other input is exit 2 with an error naming that shape and the input received, and no input is normalised.
...
- REQ-0012-0074 (REQ-0128 follow-up): `tailwindContractConvergence` live-loop measurement — `packages/qfai/tests/integration/prototyping/tailwindContractConvergence.test.ts` を Phase 1 で landing した synthesised-iteration steady-state guard から、実際の `qfai prototyping iterate` subprocess を canonical Tailwind-faithful fixture pack に対して spawn し、`iterations.length <= 3 AND stopReason === "converged"` を live run から assert する dynamic-loop measurement へ昇格する。Phase 1 は post-fix steady state を pin し、本 REQ は loop-convergence semantic を pin する。Acceptance signal: 当該 integration test が child-process spawn 経由で iterate を実行し、再現可能な fixture pack で `iterations.length <= 3` を steady-state ではなく live-loop で assert して green になる。
- REQ-0012-0075 (REQ-0109 follow-up): `--capture` CLI flag wiring + default Playwright runner — CHG-005 Phase 2 で `RunPrototypingIterateOptions` に `capture: boolean` + `captureScreen?: CaptureScreenFn` の DI surface を landing したが、`cli/lib/args.ts` への flag-string parser 配線 + default Playwright capture runner は deferred。本 follow-up で operator が `qfai prototyping iterate --capture` を CLI から直接呼んで PNG/HTML capture が動作する状態に揃える。Acceptance signal: `qfai prototyping iterate --capture` invocation が `iterate-plan.json#screens[]` の各 screen に対し PNG + HTML を実書き込みし、default Playwright runner が Playwright モジュール経由でドライブ。`--capture` 未指定時の no-capture default は保持される (DR-0012-0029)。
- REQ-0012-0076 (REQ-0110 follow-up): `qfai prototyping iterate --auto-serve` MUST start a local server through a server runner. The default runner is an in-process Node HTTP server. A runner MUST be torn down at the end of the cycle and on SIGINT within the NFR-0106 bound. Without `--auto-serve`, no server is started.
- REQ-0012-0077 (REQ-0117 follow-up): `stopReason` enum widening — CHG-005 Phase 3 で `prototyping.json#stopReason` enum を {`converged`, `max-iterations`} の 2 値で landing したが、spec brief で要求された残 2 値 (`license-verify-fail`, `input-error`) の emit wiring と validator 受理は deferred。本 follow-up で 4 値 enum を完成させる。Acceptance signal: license-verify failure 経路で `stopReason: "license-verify-fail"` が emit され、入力エラー経路で `stopReason: "input-error"` が emit され、validator (`prototypingEvidence.ts`) が 4 値全て受理する。Test surface: `packages/qfai/tests/integration/cli/commands/prototypingIterate.stopReason.test.ts` で各経路の emit を pin。
- REQ-0012-0078 (REQ-0129 follow-up): `--check-convergence` CLI flag wiring. The `--cycle` out-of-range hint names `--check-convergence` as the peek mode (REQ-0012-0073 / TC-0012-0470), so `cli/lib/args.ts` MUST parse the flag: without it, an operator following the hint gets an "unknown flag" error. The flag adds a read-only invocation that peeks at the convergence state of the final cycle (cycle 9) without re-running the loop. Acceptance signal: `qfai prototyping iterate --cycle 9 --check-convergence` reads `.qfai/prototypes/iter-09/prototyping.json` (`.qfai/prototype/iter-09/prototyping.json` with the `rule/ skill/ agent/ prompt/` assistant tree) without writing to it, reports `stopReason` and `acceptedIterationIndex` without starting another iteration, and exits 0 when converged or 2 when not.
- REQ-0012-0079 (REQ-0174): Every `qfai prototyping iterate --cycle 0` run MUST move `.qfai/evidence/prototyping/screenshots/` and `html/` into `aggregate.backup-<ISO>/` before clearing, record each moved file in the mutation log, and put the move back when a later step of the same reset fails. Both backups MUST stay out of the completion certificate's digests and freshness scan.
