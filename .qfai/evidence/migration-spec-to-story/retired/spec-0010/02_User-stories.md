...
As an evaluator, I want the axes a critique is scored on fixed by the tool rather than authored per pack — design quality, originality, craft and functionality, in `<paths.skillsDir>/qfai-prototyping/references/reviewer-prompt.md` — so that critique is aligned to the exploration-first harness.

## US-0010-0005: Evaluator Calibration

As a maintainer, I want examples of good critique and of overly lenient critique carried with those fixed axes in `<paths.skillsDir>/qfai-prototyping/references/reviewer-prompt.md`, so that evaluation quality can be calibrated before prototyping.
