...
- CLI-INIT — `.qfai/contracts/cli/qfai-init.md`。CHG-007 で `## Shipped GitHub Actions workflows` セクションが追加され、create-only の `force: false` literal が所有権コントラクトの load-bearing 要素であること、`declined` name を copy **実行前**に copy set から除外すること、removal は `pruneMatchingEntries` + retired-name membership predicate のみであること（`startsWith("qfai-")` 禁止）、drift 報告は init の責務ではないことを固定する
  - § Story-tree layout: the `.qfai/spec/` seeds, the configured paths, a project still on the old layout, the migration skill, the `rule/ skill/ agent/ prompt/` assistant tree, the managed `.gitignore` block, `--upgrade-assistant-tree`, the path SSOT pattern and the new ID shapes
- CLI-MIGR — `.qfai/contracts/cli/qfai-migration-spec-to-story.md`. The migration scripts spec-0018 owns; this spec installs and links the skill, and its scripts reuse init's integration-directory and managed-block writers
...
- discussion-20260923063306456#REQ-0001: Story-based spec tree - `qfai init` writes `.qfai/spec/` and nothing under `.qfai/specs/` or `.qfai/contracts/`; `paths.specsDir` defaults to `.qfai/spec`
- discussion-20260923063306456#REQ-0002: Policy layer - `01_policy/` holds `objective.md`, `initiative.md`, `principle.md`, `glossary.md` and `constraint.md`
- discussion-20260923063306456#REQ-0005: Contract layer - `03_contract/` holds `contracts.md`, `tech.md`, `structure.md` and `api/`, `db/`, `ui/`, `cli/`, `design/`
- discussion-20260923063306456#REQ-0013: Abolished artifacts - `qfai init` writes none of the spec-pack and `_policies` templates the story tree abolishes
- discussion-20260923063306456#REQ-0016: Project settings in `qfai.config.yaml` - the routing and review-profile defaults are built into the package
- discussion-20260923063306456#REQ-0017: Assistant tree reorganisation - shared files go to `.qfai/assistant/rule/`, single-skill files to that skill's `references/`
- discussion-20260923063306456#REQ-0018: Singular directory names - `skill/`, `agent/`, `prompt/`, `skill.local/`, `steering/_template/` and `.qfai/evidence/decision/`
- discussion-20260923063306456#REQ-0019: Migration skill - `/qfai-migration-spec-to-story`, whose steps are scripts bundled with it
- discussion-20260923063306456#REQ-0022: Layout described by mdschema - every Markdown file in the new tree has one mdschema manifest entry and a paired template
- discussion-20260923063306456#REQ-0024: Leak guards learn the new ID shapes - the three guards and `.agents/rules/distributed-surface.local.md` add them, each with a sample-ID band, and keep one pattern set
- discussion-20260923063306456#NFR-0004: No internal identifiers in shipped files - 0 findings from the three guards with the extended pattern set
- discussion-20260923063306456#NFR-0010: Decision records stay tracked - no path under `.qfai/evidence/decision/` is ignored

## Entry points

- US range in this spec: US-0003-0001..US-0003-0031
