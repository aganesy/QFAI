...
the rows already in the table. A row whose oracle scans a module's source text for
calls (TC-0003-0048, TC-0003-0069) is `unit`: reading the text is setup, and
the assertion is over the text alone.
...
| TC-0003-0042 | unit        | AC-0003-0032 | EX-0003-0039 | normal   | 配布 header table の記載完全性                                                       |
...
| TC-0003-0059 | integration | AC-0003-0039               | EX-0003-0052 | normal   | story-tree seed set on a fresh project                                               |
| TC-0003-0060 | integration | AC-0003-0039               | EX-0003-0053 | normal   | a fresh story tree validates with no error                                           |
| TC-0003-0061 | integration | AC-0003-0039               | EX-0003-0054 | normal   | every story-tree seed conforms to its mdschema                                       |
| TC-0003-0062 | integration | AC-0003-0040               | EX-0003-0055 | edge     | story-tree seeds stay create-only under --force                                      |
| TC-0003-0063 | integration | AC-0003-0041               | EX-0003-0056 | normal   | init writes the story-tree spec and contract paths                                   |
| TC-0003-0064 | unit        | AC-0003-0041               | EX-0003-0056 | edge     | omitted path keys resolve to the story-tree defaults                                 |
| TC-0003-0065 | integration | AC-0003-0042               | EX-0003-0057 | edge     | spec-pack layout under specsDir: no seed, migration skill named                      |
| TC-0003-0066 | integration | AC-0003-0042               | EX-0003-0058 | boundary | `.qfai/contracts/` alone counts as the spec-pack layout                              |
| TC-0003-0067 | integration | AC-0003-0043               | EX-0003-0059 | normal   | policy and contract files replace the catalog seeds                                  |
| TC-0003-0068 | integration | AC-0003-0044               | EX-0003-0060 | normal   | migration skill installed and linked on both layouts                                 |
| TC-0003-0069 | unit        | AC-0003-0045               | EX-0003-0061 | normal   | migration link and `.gitignore` steps call init's writers                            |
| TC-0003-0070 | integration | AC-0003-0046               | EX-0003-0062 | error    | post-build guard rejects out-of-band story-tree IDs                                  |
| TC-0003-0071 | integration | AC-0003-0046               | EX-0003-0062 | error    | smoke test rejects out-of-band story-tree IDs                                        |
| TC-0003-0072 | integration | AC-0003-0046               | EX-0003-0063 | boundary | one pattern set across the three guards; `DEC-`/`OQ-` stay distinct from `NNNN-NNNN` |
| TC-0003-0073 | integration | AC-0003-0037               | EX-0003-0064 | normal   | Codex profile kind read from the card frontmatter                                    |
| TC-0003-0074 | integration | AC-0003-0015               | EX-0003-0065 | normal   | managed block negates the singular decision directory                                |
| TC-0003-0075 | integration | AC-0003-0016               | EX-0003-0066 | edge     | re-run strips the plural decision negations                                          |
| TC-0003-0076 | integration | AC-0003-0019               | EX-0003-0067 | normal   | upgrade copies named files into the new assistant tree                               |
| TC-0003-0077 | integration | AC-0003-0019               | EX-0003-0068 | edge     | upgrade leaves adopter-owned and unrecognised files in place                         |
| TC-0003-0078 | integration | AC-0003-0017               | EX-0003-0069 | edge     | overlay beside a relocated rule is left untouched                                    |
| TC-0003-0079 | unit        | AC-0003-0047               | EX-0003-0070 | boundary | distributed header respects package Node support                                     |
...
Setup: the same init output tree as TC-0003-0036.
...
## TC-0003-0056: delivered document checks: isolated legs, preserved check name

**Level:** integration
**EX Refs:** EX-0003-0050
**AC Refs:** AC-0003-0038
**Type:** normal

Setup: the document workflow a `qfai init` run delivered into an adopter tree, parsed as YAML.
Action: read the check job's matrix legs, its `fail-fast` setting and the command each leg selects, then read the aggregate job's name, dependency and run condition.
Verify:
...
Setup: the validation workflow a `qfai init` run delivered into an adopter tree, parsed as YAML.
Action: read the validation job's profile matrix and `fail-fast` setting, the invocation each profile leg selects, and the verdict job's name, dependency and run condition.
Verify:

- the validation job declares the full and drift profiles as matrix legs of one job with `fail-fast: false`
- the full invocation and the drift invocation each sit in exactly one leg, with their profile and failure threshold unchanged
- the drift leg names the pull-request event, so a push expands the matrix to the full profile alone
- the verdict job keeps the external check name unchanged, needs the validation job and runs under `always()`

## TC-0003-0058: aggregate failure protection: bad result and missing binding

**Level:** integration
**EX Refs:** EX-0003-0051
**AC Refs:** AC-0003-0038
**Type:** error

Setup: the shipped aggregate job, plus two mutated copies of it — one whose body cannot fail, and one whose dependency result binding has been removed.
Action: run the unmodified body once per dependency result (`failure`, `cancelled`, `timed_out`, `skipped`, an unknown word and the empty value), then run both mutated copies.
Verify:

- the unmodified body exits 1 for every non-success result, the empty value included
...
## TC-0003-0059: story-tree seed set on a fresh project
...
Setup: An empty temp directory that `qfai init` lays out as the story tree.
Action: Run `runInit` and list every path under `.qfai/`.
Verify:

- `.qfai/spec/` holds exactly the seed files and contract directories BR-0003-0049 names
- `decisions.md` and `open-questions.md` each hold their header row and no data row
- no `business-flow-NNNN/` or `user-story-NNNN-NNNN/` directory exists
- neither `.qfai/specs/` nor `.qfai/contracts/` exists

## TC-0003-0060: a fresh story tree validates with no error
...
Setup: An empty temp directory that `qfai init` lays out as the story tree.
Action: Run `runInit`, then `qfai validate --fail-on error` on the result.
Verify:

- validate exits 0
- no finding at error level is reported

## TC-0003-0061: every story-tree seed conforms to its mdschema
...
Setup: An empty temp directory that `qfai init` lays out as the story tree.
Action: Run `runInit`, then run the mdschema check over the Markdown files under `.qfai/spec/`.
Verify:

- every seeded Markdown file is routed to a schema by the mdschema manifest
- the check reports 0 violations
- no seed carries the mdschema ignore marker

## TC-0003-0062: story-tree seeds stay create-only under --force
...
Setup: A temp directory holding a story tree that `runInit` seeded, with an edit written into `01_policy/glossary.md`.
Action: Delete `03_contract/contracts.md`, run `runInit`; delete it again, run `runInit` with `force: true`.
Verify:

- after each run the edited `glossary.md` is byte-identical to the edit
- after each run `contracts.md` exists again with its template content
- no other seed file's bytes change on either run

## TC-0003-0063: init writes the story-tree spec and contract paths
...
Setup: An empty temp directory that `qfai init` lays out as the story tree.
Action: Run `runInit` and parse the `qfai.config.yaml` it wrote.
Verify:

- `paths.specsDir` is `.qfai/spec`
- `paths.contractsDir` is `.qfai/spec/03_contract`

## TC-0003-0064: omitted path keys resolve to the story-tree defaults
...
Setup: A configuration object whose `paths` carries neither `specsDir` nor `contractsDir`.
Action: Resolve it through the configuration loader in `packages/qfai/src/core/config.ts`.
Verify:

- `specsDir` resolves to `.qfai/spec`
- `contractsDir` resolves to `.qfai/spec/03_contract`

## TC-0003-0065: spec-pack layout under specsDir: no seed, migration skill named
...
Setup: Two temp directories: one whose configured `paths.specsDir` holds a `spec-*/` directory, one whose `paths.specsDir` holds only `_policies/`.
Action: Run `runInit` on each and capture its output and exit code.
Verify:

- nothing is written under `.qfai/spec/`
- every other output is written, the migration skill and its host links included
- exactly one output line names the detected path and `/qfai-migration-spec-to-story`
- the exit code is 0

## TC-0003-0066: `.qfai/contracts/` alone counts as the spec-pack layout
...
Setup: A temp directory with `.qfai/contracts/`, whose configured `paths.specsDir` holds neither `spec-*/` nor `_policies/`.
Action: Run `runInit` and capture its output.
Verify:

- nothing is written under `.qfai/spec/`
- the one output line names `.qfai/contracts/` and `/qfai-migration-spec-to-story`

## TC-0003-0067: policy and contract files replace the catalog seeds
...
Setup: An empty temp directory that `qfai init` lays out as the story tree.
Action: Run `runInit` and list the policy, contract and catalog files.
Verify:

- `01_policy/objective.md`, `initiative.md`, `principle.md`, `03_contract/tech.md` and `structure.md` exist under `.qfai/spec/`
- none of `catalog/product.md`, `catalog/manifest.md`, `catalog/tech.md` and `catalog/structure.md` exists under `.qfai/assistant/`

## TC-0003-0068: migration skill installed and linked on both layouts
...
Setup: Two temp directories: an empty one, and one whose `.qfai/specs/` holds a `spec-*/` directory.
Action: Run `runInit` on each.
Verify:

- `qfai-migration-spec-to-story/` holds `SKILL.md` and `scripts/` in the assistant skill tree
- its entry in each of `.claude/skills/`, `.agents/skills/`, `.codex/skills/` and `.github/skills/` is a link that resolves to it

## TC-0003-0069: migration link and `.gitignore` steps call init's writers
...
Setup: The text of migration steps 9 and 10, read as input.
Action: Inspect the calls each step makes.
Verify:

- step 9 calls init's integration-directory writer and has no link-creation or link-removal call of its own
- step 10 calls init's managed-block writer and has no `.gitignore` write call of its own

## TC-0003-0070: post-build guard rejects out-of-band story-tree IDs
...
Setup: The packed surface, and copies of it that each plant one ID of one new shape, in band or with one numeric segment out of band.
Action: Run `packages/qfai/scripts/check-no-internal-version-leakage.sh` over each.
Verify:

- each out-of-band plant exits 1 and the output names the planted file
- the clean surface and each in-band plant exit 0

## TC-0003-0071: smoke test rejects out-of-band story-tree IDs
...
Setup: The tree `qfai init` writes into a temp directory, and copies of it that each plant one ID of one new shape, in band or with one numeric segment out of band.
Action: Call `scanDistributedSurface(root)`, exported by `packages/qfai/tests/helpers/distributedSurfaceScan.ts` and called by the smoke test too, over each.
Verify:

- each out-of-band plant is reported with its file
- the clean tree and each in-band plant report nothing

## TC-0003-0072: one pattern set across the three guards; `DEC-`/`OQ-` stay distinct from `NNNN-NNNN`
...
Setup: The pattern sets of the pre-build lint and the post-build guard, read from the shipped guard files, the smoke test's set, read from the pattern set `packages/qfai/tests/helpers/distributedSurfaceScan.ts` exports, and IDs of the `DEC-NNNN-NNNN` and `OQ-NNNN-NNNN` shapes.
Action: Compare the three sets, and match the new four-digit `DEC-NNNN` and `OQ-NNNN` patterns against the two-segment IDs.
Verify:

- the three sets are identical
- the four-digit patterns match no leading part of a two-segment ID, and the existing patterns still reject those IDs

## TC-0003-0073: Codex profile kind read from the card frontmatter
...
Setup: A temp directory with the `rule/ skill/ agent/ prompt/` assistant tree and no `agent-catalog.yml`, holding a `reviewer` card and a `worker` card that declare `kind` in their frontmatter.
Action: Run `runInit` and read `.codex/agents/`.
Verify:

- a profile is generated for each card
- the reviewer's profile carries `sandbox_mode = "read-only"` and the worker's does not

## TC-0003-0074: managed block negates the singular decision directory
...
Setup: A temp directory with the `rule/ skill/ agent/ prompt/` assistant tree and no `.gitignore`.
Action: Run `runInit` and read the managed block.
Verify:

- `!.qfai/evidence/decision/` and `!.qfai/evidence/decision/**` are present
- neither `!.qfai/evidence/decisions/` nor `!.qfai/evidence/decisions/**` is present

## TC-0003-0075: re-run strips the plural decision negations
...
Setup: A temp directory with the `rule/ skill/ agent/ prompt/` assistant tree and a `.gitignore` whose managed block an earlier release wrote with the plural decision negations.
Action: Run `runInit` and read `.gitignore`.
Verify:

- the plural lines are gone and the singular pair is present
- the file holds exactly one marker line

## TC-0003-0076: upgrade copies named files into the new assistant tree

**Level:** integration
**EX Refs:** EX-0003-0067
**AC Refs:** AC-0003-0019
**Type:** normal

Setup: A temp directory with the `rule/ skill/ agent/ prompt/` assistant tree as the target, whose legacy assistant layers hold files the relocation table names.
Action: Run `runInit` with `upgradeAssistantTree: true`.
Verify:

- each named file exists at its destination in the new tree with the legacy content
- each legacy file is still present and unchanged
- nothing is written under `constitution/`, `manifest/`, `catalog/` or `process/`, and no migration memo is written

## TC-0003-0077: upgrade leaves adopter-owned and unrecognised files in place
...
Setup: The fixture of TC-0003-0076, plus `product.md`, `manifest.md`, `tech.md`, `structure.md` and a file the relocation table does not recognise in the legacy layers.
Action: Run `runInit` with `upgradeAssistantTree: true` and capture its output.
Verify:

- none of the four adopter-owned files is copied, and each stays unchanged at its legacy path
- the unrecognised file is not copied, stays unchanged, and is not listed in the output

## TC-0003-0078: overlay beside a relocated rule is left untouched
...
Setup: A temp directory initialised with the `rule/ skill/ agent/ prompt/` assistant tree, plus `.qfai/assistant/rule/drift-protocol.local.md` holding project text.
Action: Run `runInit` with `force: true`.
Verify:

- `.qfai/assistant/rule/drift-protocol.local.md` holds the same bytes as before the run
- no other `*.local.md` file exists under `.qfai/assistant/rule/`

## TC-0003-0079: Distributed header respects package Node support
...
Setup: Read the package `engines` declaration and every shipped workflow header.
Action: Compare each stated Node support floor with the declaration.
Verify: No header claims a Node support floor that the package does not declare.
