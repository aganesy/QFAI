...
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- |
| 1   | Narrow the spec to the product, and **supersede** the three decisions with a new one: the band's floor is withdrawn and `QFAI-AUD-020` is a ceiling. `DR-0267` and `DR-0013-0003` keep their `Decision` text and are marked superseded; the composite `DR-0004-0014` stays standing, with only its band entry marked superseded | Nine `spec-0013` statements and every `spec-0004` statement carrying the band rewritten, plus one `_policies` record; one new `DR-*` in `_policies`; reset `spec-0013/TDD-0027`, `TDD-0028` and `spec-0004/TDD-0050`; the band assertions of two skipped `spec-0004` suites; the ATDD stage's full passes over `spec-0004` and `spec-0013`: `E2E` tests for the ten stories of each pack no test annotates, `spec-0013`'s three `todo` `Integration` rows and the six its ledger repair seeds for `TC-0013-0014` to `TC-0013-0019`, two of them needing a test, and each pack's evidence file and coverage-depth matrix, `spec-0004`'s written for the first time | Records today's behaviour as intended. A decision nobody reviewed at the time becomes the recorded one — but it is already the shipped one, and the record says by whom and when                                                                                                                                                                                                                                  | ✅          |
| 2   | Narrow the spec to the product, and **rewrite** the three decisions in place to say `ceiling 7`                                                                                                                                                                                                                                 | The same statements, resets, suites and ATDD passes; no new record                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | The **authoritative** lineage goes: the Decision Records stop saying a floor was chosen, and the rejected options `DR-0267` lists ("1..3 minimal band") lose the thing they were rejected against. The history survives outside them — this Change Request, the changelog entry for the removal, and the delta rows that recorded the adoption — so a reader who knows to look elsewhere can still reconstruct it |             |
| 3   | Restore the floor in the product: `QFAI-AUD-020` warns below the minimum again, and the spec stands                                                                                                                                                                                                                             | A validator change, its tests, the shipped template comments, the guide and a changelog entry; no spec edit. The same three `confirm-only` invocations and delta rows as the other options, every `done` row whose test runs the changed validator re-verified in place, and the ATDD stage's full passes over `spec-0013` and `spec-0004`, with the same story tests, `Integration` rows, evidence files and matrices                                                                                                                                                                                                                                            | Reverses a deliberate removal on a rationale nobody has contradicted — one task is a screen's focus, and a floor warns against it. A CR of its own would be the place to argue that                                                                                                                                                                                                                               |             |
...
- Overlapping open CRs: `CR-20260913-0010`, which corrects the template path
  ...
  them: it assumes `CR-20260913-0010` has landed, and is restated if that
  record is rejected.
...
- Tests: `spec-0013/TDD-0027`, `spec-0013/TDD-0028`, `spec-0004/TDD-0050`; under
  ...
  whose skipped cases require the finding to name `3..7`; under every option,
  the new `E2E` tests the `/qfai-atdd spec-0004` pass writes under
  `packages/qfai/tests/e2e/**` for `US-0004-0001`, `US-0004-0016`,
  `US-0004-0020` and `US-0004-0027` to `US-0004-0033`, and that pass's
  `.qfai/evidence/atdd-spec-0004.md` and
  `.qfai/evidence/coverage-depth-spec-0004.md`; the new
  `E2E` tests the `/qfai-atdd spec-0013` pass writes for `US-0013-0001` to
  `US-0013-0010`, the tests for `spec-0013/TDD-0016` to `TDD-0018`, the
  `Integration` rows `CR-20260913-0009` seeds for `TC-0013-0014` to
  `TC-0013-0019` — a test for `TC-0013-0018` and `TC-0013-0019`, and a handoff
  and evidence for all six — and
  `.qfai/evidence/atdd-spec-0013.md` and
  `.qfai/evidence/coverage-depth-spec-0013.md`; and under option 3 the four suites
  action 4 rewrites,
...
1. **Three invocations, under every option**, because the change spans two
   ...
   **Under options 1 and 2 they run after action 2 and the hand edits.** A
   `confirm-only` rerun re-reads an artifact that already carries the approved
   change and writes only the Change Request reference, so the statements named
   below are edited by hand first, action 2 then writes the decision records,
   and only then do the three invocations run. Under option 3 nothing is edited
   first, and they confirm that every statement stands.
   ...
   **The template path those statements name is not this record's.** They
   name `templates/contracts/ui-spec.yaml`, which the package does not ship.
   That defect is `CR-20260913-0010`'s, which corrects each path and sweeps
   `TDD-0027` for it; a statement this record rewrites keeps the path that
   record left.
...
3. Downstream ledger sweep, under options 1 and 2. Reset to `todo`, recording
   ...
   **The three rows' `Selector` cells still state the band, and none of them
   resolves.** They read `primary_tasks band documented + named in warning`,
   `primary_tasks count below 3 / above 7 warns (boundary)` and
   `TC-0004-0070: 9 entries → QFAI-AUD-020 band 3..7 warn; missing acceptance → reject`,
   and no case title in their test files contains that text: the titles
   already name the ceiling. `/qfai-implement` rewrites a `Selector` while it
   does not resolve against the row's test file
   (`.qfai/assistant/constitution/drift-protocol.md`, allowed exceptions), and
   the run that takes each reset row rewrites it to the cases it runs, so no
   `confirm-only` rerun has a row cell to write. The same holds under option
   3, where the in-place re-verification re-runs each row's selector.
   ...
   **every `done` row whose test runs the changed validator is re-verified in
   place**, not only those three. Today that adds `spec-0013/TDD-0020` and
   `TDD-0021`, which run `sddPrimaryTasksLane.test.ts`, `TDD-0029` and
   `TDD-0030`, which run `primaryTasksStructured.test.ts`, and
   `spec-0004/TDD-0049`, which runs `auditProfileDualShape.test.ts`: each test
   imports `validateDesignAudit` or its `auditProfile` wrapper. The detection
   step in `.qfai/assistant/skills/qfai-implement/references/cross-spec-ownership.md`
   decides the full set at run time, and a row an approved reset has already
   returned to `todo` by then is re-executed by its own cycle instead. Each row
   is re-verified over the restored floor, by the path
   ...
   with `qa-gatekeeper` routed on the mutation run. `spec-0004/TDD-0050` and
   `TDD-0049` belong to another spec, so the run records them under
   `## Cross-spec obligations` as well
   (`.qfai/assistant/skills/qfai-implement/references/cross-spec-ownership.md`).

   **`/qfai-atdd spec-0004` runs under every option**, over the rows the
   `spec-0004` ledger repair seeds; `/qfai-implement` writes no ATDD-owned test
   or handoff, so without it those rows stay at `todo` whatever the outcome.
   Under options 1 and 2 the same pass also updates the two skipped cases
   ...
   **That invocation is a full pass over `spec-0004`, not a repair of two
   cases.** The stage has no narrower mode, so it takes up every ATDD-owned
   `spec-0004` row still owed, and it writes the pack's mandatory outputs,
   `.qfai/evidence/atdd-spec-0004.md` and
   `.qfai/evidence/coverage-depth-spec-0004.md`, neither of which exists yet,
   through its reviewer gate. **Its gate also owes an `E2E` test for every
   ...
   **`/qfai-atdd spec-0013` refreshes `.qfai/evidence/atdd-spec-0013.md` and
   ...
   matrix's `BR-0013-0019` row and its findings score the missing floor, and
   the evidence file says `TC-0013-0032` and `TC-0013-0033` specify `3..7`
   while the validator lacks the floor, beside the matrix totals it restates.
   ...
   **For `spec-0013` that record is `CR-20260913-0009`.** No record repairs the
   `spec-0004` ledger yet, and one is owed before that pack's pass, since its
   seeding and the split of `spec-0004/TDD-0050` are as much a reset as the
   `spec-0013` ones. Each pack's pass runs once its own repair has been applied.
   Until the `spec-0004` repair is raised and applied, that pack's reset rows
   stay at `todo` in this record's blocked set, its pass does not run, and this
   record's `Applied at` stays unwritten.
