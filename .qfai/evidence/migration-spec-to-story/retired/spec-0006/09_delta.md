...
- Tags: doctor, story-tree, paths
- Summary: doctor's directory checks and skill-manifest probe are worded by
  their `paths.*` keys, and the capability-catalog check goes at P7. Recorded
  under `## 2026-09-24 — spec-to-story batch record`.
...
| ------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- | --------- | ------ | ----------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------- |
| discussion-20260923063306456#REQ-0001, discussion-20260923063306456#REQ-0005, discussion-20260923063306456#REQ-0013 | Doctor directory and path checks follow `.qfai/spec` and `03_contract`, and the `spec.capCatalogSpecColumn` check and its autoremediation are removed | spec-0006     | UPDATE    | MODIFY | -           | Slice C. Adopted (N12). No spec item describes the capability-catalog check, so nothing is removed from this spec; the code and its tests go in the P7 change                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | OQ-0170                                        |
| discussion-20260923063306456#REQ-0018                                                                               | The skill-manifest probe follows `assistant/skill`                                                                                                    | spec-0006     | UPDATE    | MODIFY | -           | Slice B, lands P6. P6 co-change in this repository at `pnpm sync:ssot` time: `qfai.config.yaml` `skillsDir`; the 42 tracked links `.agents/skills/qfai-*`, `.claude/skills/*` and `.claude/agents/*.md`; and the `.qfai/assistant/{skills,agents,constitution,catalog,manifest}` citations in `AGENTS.md`, `.github/copilot-instructions.md`, `.github/PULL_REQUEST_TEMPLATE.md`, `.gitignore`, `README.md`, `.agents/rules/{distributed-surface.local,document-schema,instruction-tree,repository-language}.md`, `.instruction/02_project/agent-selection.md`, the generated `.codex/agents/*.toml`, `scripts/{check-prompt-scanner-pair,check-review-profile-consistency,gen-agent-catalog,gen-codex-agents,smoke-qfai-cli,verify-pack}.mjs` and `scripts/dogfood-backlog.json`. The P5 link-repoint script (migration step 9) runs on this repository first. | discussion-20260923063306456#REQ-0019, OQ-0177 |
...
- Rewritten in place, because the new wording names a `paths.*` key and holds
  in both layouts (ruling X1), and translated to English: US-0006-0002,
  US-0006-0010; AC-0006-0003, AC-0006-0019; BR-0006-0016; EX-0006-0008,
  EX-0006-0019.
- `01_Spec.md`: four lines citing the discussion pack.
- `10_Plan.md`: a `### Story-tree layout` subsection under each heading the
  change touches.
- No item was added. Every TC and every ledger row is unchanged (ruling X2).
...
guard exception. Both rows of this spec land in the second.

| Triage row                                                     | Operation     | Lands                                               | Items                                                                                                    |
| -------------------------------------------------------------- | ------------- | --------------------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| Directory and path checks follow the story tree; CAP check out | UPDATE:MODIFY | P3 for the paths, with no doctor change; P7 for CAP | US-0006-0002; AC-0006-0003; EX-0006-0008; TC-0006-0003 (TDD-0003)                                        |
| Skill-manifest probe follows `assistant/skill`                 | UPDATE:MODIFY | P6                                                  | US-0006-0010; AC-0006-0019; BR-0006-0016; EX-0006-0019; TC-0006-0024 (TDD-0026), TC-0006-0025 (TDD-0027) |

- Doctor resolves every directory check through `config.ts#resolvePath`, so the
  `paths.specsDir` and `paths.contractsDir` defaults that spec-0003 switches at
  P3 reach it with no doctor change.
- The capability-catalog check has no item in this spec. At P7, under
  spec-0001's REMOVE row that retires CAP (ruling N12), the change deletes
  `packages/qfai/src/core/doctor/capCatalogSpecColumn.ts` and
  `packages/qfai/tests/core/capCatalogSpecColumn.test.ts`, removes the check
  and the `capCatalogRowsDeclared` field from
  `packages/qfai/src/core/doctor.ts` and its `--autoremediate` step from
  `packages/qfai/src/core/doctor/autoremediate.ts`, and drops both from the
  doctor contract's module list.

### Co-changes the landing carries

- **P1.** Shipped doctor sources carry this spec's composite IDs, for example
  `BR-0006-0022` in `packages/qfai/src/core/doctor.ts`, `BR-0006-0018` in
  `packages/qfai/src/core/doctor/workflowsIntegrity.ts` and `AC-0006-0018` in
  `packages/qfai/src/cli/commands/doctor.ts`. The clean-up commit removes every
  one the new pattern set matches, re-counted with the guards' own patterns.
- **P6.**
  - The co-change list in this spec's skill-manifest Triage row.
  - The literals that still name the plural skill tree go in one commit: the
    `skills.integrity` titles and message in `createDoctorData`, the
    `assistant/skills/` prefix in `assetLineBudgetNextActions`, and the keys of
    the asset line budget.
  - TC-0006-0024's fixture sets `paths.skillsDir` to a directory other than the
    default, and TC-0006-0025's manifest moves under the singular tree.
- **P7.**
  - TC-0006-0003's test is replaced by a `createDoctorData` run in a `mkdtemp`
    project whose `paths.specsDir` names a missing directory.
  - spec-0007's P7 change makes `packages/qfai/src/core/doctor.ts` pass both
    story-tree directories to `loadDecisionGuardrails`, so doctor's guardrail
    check reads the same files as `qfai guardrails`. Only spec-0007's plan
    states this.
...
- AC-0006-0003 and EX-0006-0008 report a missing `paths.specsDir` directory as
  a warning. `createDoctorData` reports it at `info` while the key holds its
  default value, because `DEFAULT_SKILL_CREATED_PATH_KEYS` in
  `packages/qfai/src/core/doctor.ts` covers `specsDir`, `contractsDir` and
  `discussionDir`, and at `warning` only for another configured path. The P7
  case for TC-0006-0003 sees the warning only on a non-default path. Open.
- TC-0006-0003's current test cannot fail: it searches the CLI source for the
  word "dir". The P7 replacement above is the first test that can.

### Adoption and rejection

- Adopted (ruling N12): the capability-catalog check and its autoremediation
  step go at P7 with the CAP catalog.
- Rejected: keeping the CAP check until the story tree has shipped.
  - DO NOT: keep `spec.capCatalogSpecColumn` past P7.
  - Temptation: removing a check feels riskier than leaving it. It reads
    `_policies/03_Capabilities.md`, which the story tree abolishes, so on a
    migrated project it has nothing to read.
- Rejected: a story-tree path of doctor's own.
  - DO NOT: add a path literal to doctor.
  - Temptation: a literal is shorter than a configuration lookup. A second
    source of the path disagrees with `qfai validate` the first time a project
    moves its tree.
