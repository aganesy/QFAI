...
This spec introduces no architectural element. Each change edits a module
doctor already has. Doctor's directory checks already resolve every
`paths.*` key through `config.ts#resolvePath`, so the `specsDir` and
`contractsDir` defaults spec-0003 switches at P3 reach them with no doctor
code change.

| Phase | Change                                                                                                                                                                                                                                                                                                                                                                                       | Modules                                                                                                                                                  | Items                      |
| ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------- |
| P6    | The skill-manifest probe keeps reading `<paths.skillsDir>/<skill>/manifest.json`, whose default becomes `.qfai/assistant/skill` with spec-0003's singular-names row. The literals that still name the plural tree go: the `skills.integrity` titles and message in `createDoctorData`, the `assistant/skills/` prefix in `assetLineBudgetNextActions`, and the keys of the asset line budget | `packages/qfai/src/core/doctor.ts`, `packages/qfai/src/core/doctor/skillManifestProbe.ts`, `packages/qfai/src/core/doctor/assetLineBudget.ts`            | BR-0006-0016; TC-0006-0024 |
| P7    | The `spec.capCatalogSpecColumn` check, its `--autoremediate` step and the `capCatalogRowsDeclared` field go, under spec-0001's REMOVE row that retires CAP (N12). The directory-check tests start naming the configured keys                                                                                                                                                                 | `packages/qfai/src/core/doctor.ts`, `packages/qfai/src/core/doctor/autoremediate.ts`; `packages/qfai/src/core/doctor/capCatalogSpecColumn.ts` is deleted | BR-0006-0002; TC-0006-0003 |

Alternatives considered:

- **Keep the CAP check until the story tree has shipped.** Rejected by N12: it
  reads `_policies/03_Capabilities.md`, which the story tree abolishes, so on a
  migrated project it has nothing to read.
- **Give doctor a story-tree path of its own.** Rejected by the doctor
  contract: doctor adds no path literal, and a second source of the path would
  disagree with `qfai validate` the first time a project moves its tree.
...
No ledger row is added. Existing cases change at the phase their code does:

| Layer          | Case                                                                                          | Change                                                                                                                                                                                                                                          |
| -------------- | --------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| L3 Integration | TC-0006-0024, `packages/qfai/tests/integration/cli/commands/doctorSkillProfile.probe.test.ts` | P6: the fixture sets `paths.skillsDir` to a directory other than the default, so the case proves the key is read rather than a literal                                                                                                          |
| L1 Unit        | TC-0006-0025, `packages/qfai/tests/unit/core/doctor/skillManifestProbe.empty.test.ts`         | P6: the manifest moves under the singular tree                                                                                                                                                                                                  |
| L3 Integration | TC-0006-0003, `packages/qfai/tests/integration/doctorSpec0006.test.ts`                        | P7: the case runs `createDoctorData` in a `mkdtemp` project whose `paths.specsDir` names a missing directory, and asserts the warning names that directory. Its current test only searches the CLI source for the word "dir", so it cannot fail |
| L1 Unit        | `packages/qfai/tests/core/capCatalogSpecColumn.test.ts`                                       | P7: deleted with the module                                                                                                                                                                                                                     |

The boundary that needs its own case is a relocated tree: a `paths.skillsDir`
or `paths.specsDir` other than the default. A case on the default path passes
for an implementation that still reads a literal.
...
- **NFR-0040 (message quality).** A directory or manifest finding names the
  path the configuration resolved, never a default the project does not use.
  Breach: TC-0006-0003 or TC-0006-0024, run on a relocated tree, finds a
  message naming `.qfai/specs` or `.qfai/assistant/skills`.
...
| Risk                                                                                                                            | Likelihood / impact | Mitigation                                                                                                                                                                    | Trigger to act                                                                              |
| ------------------------------------------------------------------------------------------------------------------------------- | ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| A literal naming the plural skill tree is left in doctor after P6, so a finding points at a directory the project no longer has | med / low           | The P6 commit replaces the `skills.integrity` titles and message, the `assetLineBudgetNextActions` prefix and the budget keys together; TC-0006-0024 runs on a relocated tree | `assistant/skills` still appears under `packages/qfai/src/core/doctor*` after the P6 commit |
| Removing `capCatalogRowsDeclared` changes the `--autoremediate` output a script may read                                        | low / med           | The field goes in the P7 commit with the check, and the doctor contract's module list drops both in the same commit                                                           | The doctor contract or a test still names the field after P7                                |
