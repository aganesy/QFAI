...
## TC-0013-0036: Concrete-First Order On The Story Tree

- EX-Ref: EX-0013-0021
- AC-Refs: AC-0013-0028
- Type: normal
- Level: integration
- Verify that, on the story tree, the `qfai-sdd` skill writes `01_policy/`, then `02_business-flow/`, then the stories with their US, AC and EX, then `03_contract/` with its BRs; that it states a BR cites only EXs already written; and that it runs no Contracts-first phase there.

## TC-0013-0037: Story-Tree Order Rejects Early Citations And Contracts-First

- EX-Ref: EX-0013-0021
- AC-Refs: AC-0013-0028
- Type: error
- Level: integration
- Verify two failures on the story tree: a BR citing an EX that no story holds yet fails the gate, and a statement in the skill that makes Contracts-first mandatory outside the spec-pack layout fails the check.

## TC-0013-0038: Story-Tree Files Follow Their Templates

- EX-Ref: EX-0013-0022
- AC-Refs: AC-0013-0029
- Type: normal
- Level: integration
- Verify that, on the story tree, each file the skill writes follows its paired `qfai-sdd` template, a story directory holds exactly `01_User-story.md`, `02_Acceptance-Criteria.md` and `03_Example.md`, each `business-flow.md` carries a Mermaid `flowchart` or `sequenceDiagram`, and each contract file has a row in `<paths.contractsDir>/contracts.md`.

## TC-0013-0039: Extra Entry In A Story Directory Fails

- EX-Ref: EX-0013-0022
- AC-Refs: AC-0013-0029
- Type: error
- Level: integration
- Verify that a `user-story-NNNN-NNNN/` directory holding a fourth file, or holding a subdirectory, fails the story-directory family, naming the directory and the extra entry.

## TC-0013-0040: Business Flow Without A Mermaid Diagram Fails

- EX-Ref: EX-0013-0023
- AC-Refs: AC-0013-0029
- Type: error
- Level: integration
- Verify that a `business-flow.md` holding no Mermaid `flowchart` or `sequenceDiagram` block fails the check, naming the file.

## TC-0013-0041: Contract File Without A contracts.md Row Fails

- EX-Ref: EX-0013-0024
- AC-Refs: AC-0013-0029
- Type: error
- Level: integration
- Verify that a contract file under `<paths.contractsDir>` with no row in `contracts.md` is reported as an unlisted contract at error, naming the file.

## TC-0013-0042: Records Land As Rows Of The Two Tables

- EX-Ref: EX-0013-0025
- AC-Refs: AC-0013-0030
- Type: normal
- Level: integration
- Verify that, on the story tree, the skill records triage decisions, change requests, retired stories and rejected options as `decisions.md` rows and open questions as `open-questions.md` rows; that a triage row needing approval is appended at TODO, moves to WIP when approved and to REJECTED when declined; that a change request's Content opens `Change request:`; and that nothing is written under `.qfai/decisions/` and no `01_Spec-retired` file is written.

## TC-0013-0043: Table Shape Violations Fail

- EX-Ref: EX-0013-0026
- AC-Refs: AC-0013-0030
- Type: error
- Level: integration
- Verify that a `decisions.md` or `open-questions.md` carrying a fifth column, or a row whose Status is outside its table's vocabulary, fails the two-tables family, naming the file and the column or row ID.

## TC-0013-0044: Rewritten Or Removed Rows And Record Files Fail

- EX-Ref: EX-0013-0026
- AC-Refs: AC-0013-0030
- Type: error
- Level: integration
- Verify three failures on the story tree: a row whose ID, Content or Approach differs from the base is reported by the row-rewritten family; a row the base holds that is gone is reported by the same family; and a record written as a file under `.qfai/decisions/` fails the check.

## TC-0013-0045: Merged Files State Each Fact Once

- EX-Ref: EX-0013-0027
- AC-Refs: AC-0013-0031
- Type: normal
- Level: integration
- Verify that, on the story tree, `objective.md`, `initiative.md`, `principle.md`, `tech.md` and `structure.md` state no fact twice, and that the quality-gate commands appear only in the Standard commands section of `<paths.contractsDir>/tech.md`.

## TC-0013-0046: Gate Commands Outside tech.md Fail

- EX-Ref: EX-0013-0027
- AC-Refs: AC-0013-0031
- Type: error
- Level: integration
- Verify that a quality-gate command listed in any of the other four merged files, or in `tech.md` outside its Standard commands section, fails the check, naming the file.

## TC-0013-0047: New IDs Are The Highest In Scope Plus One

- EX-Ref: EX-0013-0028
- AC-Refs: AC-0013-0032
- Type: normal
- Level: integration
- Verify that, on the story tree, the skill allocates each BF, US, AC, EX, BR, DEC and OQ ID as the highest number its scope already names plus one, for the scope BR-0013-0028 gives each kind, and names a new flow or story directory after its ID.

## TC-0013-0048: Empty Scope And Retired IDs

- EX-Ref: EX-0013-0028
- AC-Refs: AC-0013-0032
- Type: boundary
- Level: integration
- Verify two boundaries: an empty scope starts at `0001`, and at `01` for the AC and EX tail; and when the highest ID in a scope belongs to a retired item that only a `decisions.md` row still names, the next ID is one above it, so the retired ID is not reissued.

## TC-0013-0049: Rules Are Written Inside Their Contracts

- EX-Ref: EX-0013-0029
- AC-Refs: AC-0013-0033
- Type: normal
- Level: integration
- Verify that, on the story tree, each rule sits in the contract file that enforces it, in that file type's form (`x-qfai-rules` in YAML or JSON, `-- Rule` and `-- Examples:` lines in SQL, a `## Rules` table in Markdown), with an ID, a statement and at least one full EX ID, and that BR IDs are numbered across all contracts.

## TC-0013-0050: Rule Without Examples Or A Separate Rules File Fails

- EX-Ref: EX-0013-0029
- AC-Refs: AC-0013-0033
- Type: error
- Level: integration
- Verify two failures: a rule with no examples fails the BR-to-EX family, naming the BR and its file; and a rules file written apart from the contracts fails the check.

## TC-0013-0051: A Shared Rule Restated In A Second Contract Fails

- EX-Ref: EX-0013-0030
- AC-Refs: AC-0013-0033
- Type: error
- Level: integration
- Verify that a rule several contracts rely on, restated in a second contract instead of listed in its rule refs, fails the check, naming both files.

## TC-0013-0052: Story-Tree Required Edges Pass The Gate

- EX-Ref: EX-0013-0031
- AC-Refs: AC-0013-0034
- Type: normal
- Level: integration
- Verify that `qfai validate --profile sdd --fail-on error` passes a story tree where every EX names exactly one existing AC, every AC has at least one EX, every BR cites at least one existing EX, and every EX is cited by at least one BR.

## TC-0013-0053: Broken EX-To-AC Edges Fail The Gate

- EX-Ref: EX-0013-0031
- AC-Refs: AC-0013-0034
- Type: error
- Level: integration
- Verify three failures of the EX-to-AC family, each naming the ID and its file: an EX whose `AC-Ref` cell names no AC, an EX naming two ACs, and an AC that no EX names.

## TC-0013-0054: Broken BR-To-EX Edges Fail The Gate

- EX-Ref: EX-0013-0032
- AC-Refs: AC-0013-0034
- Type: error
- Level: integration
- Verify three failures of the BR-to-EX family, each naming the ID and its file: a BR citing no EX, an EX that no BR cites, and a BR citing an EX that does not exist.

## TC-0013-0055: qfai-sdd Citations Resolve

- EX-Ref: EX-0013-0033
- AC-Refs: AC-0013-0035
- Type: normal
- Level: integration
- Verify that, with the `rule/ skill/ agent/ prompt/` assistant tree, every citation of `change-classification.md` and `requirements-decomposition.md` in the `qfai-sdd` skill's `SKILL.md`, `references/` and `templates/` reads `.qfai/assistant/rule/change-classification.md` or `<paths.skillsDir>/qfai-sdd/references/requirements-decomposition.md`, and resolves to a file that exists.

## TC-0013-0056: A Stale constitution/ Citation Fails

- EX-Ref: EX-0013-0033
- AC-Refs: AC-0013-0035
- Type: error
- Level: integration
- Verify that, with the `rule/ skill/ agent/ prompt/` assistant tree, a citation in the `qfai-sdd` skill still reading `.qfai/assistant/constitution/change-classification.md` or `.qfai/assistant/constitution/requirements-decomposition.md` fails the check, naming the citing file.

## TC-0013-0057: The Completion Gate Runs Per Flow

- EX-Ref: EX-0013-0034
- AC-Refs: AC-0013-0036
- Type: normal
- Level: integration
- Verify that, on the story tree, the skill gates each business flow it wrote or changed with `qfai validate --profile sdd --fail-on error --flow BF-NNNN`, once per flow.

## TC-0013-0058: The Per-Spec Gate Is Not Used On The Story Tree

- EX-Ref: EX-0013-0034
- AC-Refs: AC-0013-0036
- Type: error
- Level: integration
- Verify that, on the story tree, the skill does not pass `--spec <spec-id>` to its completion gate, and that `qfai validate --profile sdd --spec <spec-id>` on that tree exits 2 and names `--flow`.
