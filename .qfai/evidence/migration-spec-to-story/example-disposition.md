...
The source audit is `br-review-multi-ex.csv`, an unchanged copy of the P7
audit. These 15 old examples
...
All twelve active examples have one criterion. The step 4 and step 6 reports
and `step04-ex-provenance.csv` record the old-to-new provenance. Each new
example has its own ID, no old EX ID has two parents, and the pre-split source
is archived under `retired/`.
