...
| `TC-0012-0486` | Unit | `packages/qfai/tests/unit/cli/commands/prototypingIterate.composeCaptureUrl.test.ts` | D3 | Calls `composeCaptureUrl` on each pairing of a screen URL that is absolute, route-relative or absent with a `--target-url` that is set or absent, and on a pair `new URL` rejects. Checks the exact result, or a reason naming `--target-url` and not the internal field. Partitions ✅, normal path ✅, error path ✅ (both rejections), edge cases ✅ (a bare origin a join would normalise, a base whose last segment a join replaces), boundary values ✅ (a route with and without a leading slash), combinatorial ✅ (every URL kind with and without `--target-url`), state transitions n/a (a pure function), oracle strength ✅ (exact `toEqual` results, and a named mutation fails each ledger row). Special values ⚠️: an empty-string URL, an upper-case scheme and a protocol-relative `//host` URL are not tested; each takes a branch the tests already reach. One row per boundary: `TDD-0516`, `TDD-0578` to `TDD-0581` |
| `TC-0012-0487` | Unit | `packages/qfai/tests/unit/core/prototyping/defaultCaptureScreen.responseStatus.test.ts` | D2 | Runs `defaultCaptureScreen` against a stubbed Playwright page answering 200, 204, 399, 404, 500 or no response, and checks the result, the reason and whether a screenshot was taken. Partitions ✅ (2xx, 3xx, 4xx, 5xx, no response), normal path ✅ (200 writes the PNG and the HTML), error path ✅ (404, 500 and no response are each refused with the reason and no screenshot), edge cases ✅ (204), special values ✅ (no response), state transitions n/a, combinatorial n/a (one input), oracle strength ✅ (inverting the comparison fails the five status tests, and skipping the missing-response check fails the sixth). Boundary values ❌: `BR-0012-0066` refuses 400 or above, and no test answers 400, so `status > 400` in place of `status >= 400` passes all six tests. Not justified: no DR or CR accepts the gap, and the test case lists no 400 status, so closing it amends `TC-0012-0487` through `/qfai-sdd`. The 204 and 399 tests check `ok` alone, not the screenshot. One row per boundary: `TDD-0517`, `TDD-0582` |
| `BR-0012-0066` | Unit | `packages/qfai/tests/unit/cli/commands/prototypingIterate.composeCaptureUrl.test.ts`, `packages/qfai/tests/unit/core/prototyping/defaultCaptureScreen.responseStatus.test.ts` | D2 | Covered by `TC-0012-0486` and `TC-0012-0487`. Positive case ✅: an absolute URL opened as written, a route-relative one joined with `new URL`, the fallback to `--target-url` or to none, and 200, 204 and 399 captured. Negative case ⚠️: both rejections of the second clause are checked for `--target-url` in the reason, and 404, 500 and no response are refused without a screenshot. Three parts are not: 400 itself (the ❌ on `TC-0012-0487`), a reason naming the screen, and iterate exiting 2. No traced case checks the last two. The unannotated case `returns exit 2 with an explanatory message when a route-relative URL has no targetUrl` in `prototypingIterate.cliCapture.test.ts` checks exit 2 for a missing `--target-url` but not the screen id, and nothing runs iterate on a pair that does not compose. Conditional branches ✅: every branch of the three clauses has a case |
