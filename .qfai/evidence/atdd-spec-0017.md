...
- `.qfai/specs/spec-0017/tdd/test-list.md` — read, never written. 92 rows: 72 `Integration`,
  11 `Unit`; **74 `refactor`, 6 `blocked`, 12 `todo`**. The other nine rows are `E2E`, one per story,
  seeded at `todo`. Three `todo` rows are `Integration` and are therefore this stage's to route — see
...
e2e callsites at this tree: 2360 (packages/qfai/tests/assets 2183, packages/qfai/tests/e2e 177)
