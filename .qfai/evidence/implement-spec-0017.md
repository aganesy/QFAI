...
- `record:unchecked`, `TDD-0070`, Round 1: the review pack `review-20260923120009000` holding the qa-gatekeeper#1 `REVISE` was written after that gate's Round 2 `PASS`, so the round that Round 2 follows carries the verdict that opened it. Its content copies the attempts entry; it is not the gate's original output.
