...
[coverage-depth-BF-0002.md](coverage-depth-BF-0002.md) — ✅ 0 / ⚠️ 64 / ❌ 522. The counts cover scored cells; `—` means the row has no declared case in that dimension.
...
- Static inspection: annotations and nearby `expect(...)` or `assert(...)` calls in `packages/qfai/tests/**`. EX-0002-0021-01–04/06 now have rule-section, vocabulary, and link assertions aligned with the accepted assistant-tree layout. A candidate is partial until its CI outcome and reviewer assessment are checked.
