...
| Layered Spec Architecture | specsを_policies/（共有ポリシー層）とspec-XXXX/（Capability固有層）の2層に分離する設計構造。1 CAP = 1 spec directory。 | Traceability & Spec Architecture | SRC-0019 |
| Escalation Hook | spec-XXXX/01_Spec.mdに記載される_policiesへの参照委譲メカニズム。NFR・policy・requirements・evidenceのcopy-downを行う。 | Traceability & Spec Architecture | SRC-0019 |
