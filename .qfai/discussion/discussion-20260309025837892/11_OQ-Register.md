...
| OQ-0005 | 既存\_policies/03_Capabilities.mdのCAP追加フォーマット              | sdd  | resolved    | agent | 既存フォーマットにCAP-0007〜0010をどう追記するか                                        | A) 既存フォーマットに準拠して追記（recommended） / B) CLIコマンドCAPと分離して別セクションを作る                                                      | Option A: 既存フォーマットに準拠し、CLIコマンドCAPの後に連番で追記する。ただし「フレームワーク設計仕様」カテゴリを明記 | N/A                 | N/A | 09_Constraints.md TC-03: \_policiesは追記のみ                                                   |
