...
| Implementation Strategy | A YAML artifact defining the chosen UI/UX implementation approach (framework, component library, design system) | uiux/10_implementation_strategy.yaml |
| Scoring Axes | Three-layer evaluation framework: invariant (universal UX principles), trend-derived (current design trends), product-specific (project-unique criteria) | uiux/20-23_design_eval_*.yaml |
| Aggregate Scoring | Combined scoring rules that weight and merge the three scoring axes into a single evaluation framework | uiux/23_design_eval_aggregate.yaml |
| Anchor Screen | The selected reference screen design chosen from option comparison as the primary design direction | uiux/31_selected_anchor_screen.md |
| Screen Contract | A structured YAML definition of screen-level UI obligations (elements, actions, states, data bindings) | uiux/40_screen_contracts.yaml |
| Option Comparison | A structured comparison of 2+ design alternatives with trade-off analysis | uiux/30_option_comparison.md |
...
| Review Input Bundle | A consolidated artifact packaging sidecar outputs for reviewer consumption | uiux/50_review_input_bundle.md |
