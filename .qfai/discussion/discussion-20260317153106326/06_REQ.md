...
| REQ-0004 | Duplicate TDD-ID Check | Validator MUST emit TDDLIST_DUPLICATE_ID error when the same TDD-ID (case-insensitive) appears more than once in a spec's test-list.md | SRC-0001 §4.1D | Must |
| REQ-0005 | TDD-ID Format Check | Validator MUST emit TDDLIST_INVALID_ID error when TDD-ID does not match the TDD-NNNN pattern | SRC-0001 §6.1 | Must |
| REQ-0006 | Coverage Summary in Report | Report MUST display per-spec unit/component TC coverage: total, done, exception, open, missing TC refs, exception rows, latest evidence refs | SRC-0001 §4.2 | Must |
| REQ-0007 | Actionable Guidance in Report | Report MUST include "what to edit" guidance for each detected issue type | SRC-0001 §7.2 | Should |
...
| REQ-0009 | Specs README Update | specs/README.md MUST document test-list.md as execution ledger, coverage definition, exception/DR-ID contract, and test file existence requirement | SRC-0001 §4.3 | Must |
...
| TDDLIST_INVALID_ID added in v1.6.1 | Catches malformed IDs early before they propagate through the framework (REQ-0005) |
...
| F-6104 | TDDLIST_DUPLICATE_ID | Same TDD-ID appears more than once in a single spec's test-list.md |
| F-6105 | TDDLIST_INVALID_ID | TDD-ID does not match the TDD-NNNN pattern |
