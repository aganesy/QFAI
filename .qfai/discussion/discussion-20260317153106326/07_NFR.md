...
| NFR-0005 | Single PR Coherence         | The downstream v1.6.1 implementation PR MUST ship release changes in a single PR with no half-migrated state | Implementation PR contains validator + report + docs + tests + verify-pack | SRC-0001 §2.1 |
| NFR-0006 | No Scope Creep              | The downstream v1.6.1 implementation PR MUST NOT include any v1.6.2 items                                    | 0 v1.6.2 features in implementation PR diff                                | SRC-0001 §2.3 |
...
| NFR-0005 | verify-pack runs on the downstream implementation PR branch and passes; manual review confirms no partial migration |
| NFR-0006 | downstream implementation PR diff reviewed against the v1.6.2 anti-goal list; no overlap found                      |
