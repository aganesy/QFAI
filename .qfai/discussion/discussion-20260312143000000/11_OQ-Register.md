...
| OQ-0004 | Windows symlink 失敗時の挙動           | discussion | resolved    | user  | Windows で Developer Mode OFF の場合 symlink 作成に失敗する。ユーザー確認で「エラーメッセージを表示して処理続行しない」方針に決定 | A) エラーメッセージ表示のみで中断 / B) junction（ディレクトリ用）+ テキストファイル（ファイル用）にフォールバック / C) エラーメッセージ + 処理続行せず（recommended: C） | C: エラー表示 + 処理続行せず            | N/A                 | 2026-03-12 | ユーザー確認（conversation log） |
