...
```gherkin
# AC-0001-0038
Scenario: カノニカルテンプレート生成
  Given 空のプロジェクトディレクトリが存在する
  When `qfai init` を実行する
  Then templates/uiux/ 配下に 6 種のカノニカルテンプレートファイルが生成される
  And 00_index.md がカノニカルファミリーを参照している
```

```gherkin
# AC-0001-0039
Scenario: 旧テンプレートのデフォルト除外
  Given `qfai init` を実行する
  When テンプレート生成が完了する
  Then 旧 4-axis ファミリーファイルはデフォルトテンプレートに含まれない
  And 旧 4-axis ファミリーファイルには非推奨マーキングが付与されている
```
...
| AC-0001-0038 | カノニカルテンプレート生成             | REQ-0004           | P1       |
| AC-0001-0039 | 旧テンプレートのデフォルト除外         | REQ-0006, DR-0102  | P1       |
