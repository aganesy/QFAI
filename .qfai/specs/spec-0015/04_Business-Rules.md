...
- What the three buckets classify is the **operations the skill performs** — what it may do without asking, what it asks before doing, and what it may not do at all without a value. A question put inside a grilling session is not one of those: it settles a design, an approach, a scope boundary or a trade-off before anything is performed, and `.agents/rules/grilling.md` owns which of those are asked and in what order. Reading the buckets as a classification of every question an invocation can utter is what made the closed `ask-user` list contradict that rule. For a skill whose own operation is the interview, the asking is what it performs and stays in `ask-user`.
