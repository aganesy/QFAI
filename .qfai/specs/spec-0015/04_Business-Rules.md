...
- Per DR-0269, every SKILL.md MUST carry a `## Default Autopilot Policy` section populated with three named buckets:
  ...
  - **ask-user** (AI prompts via `AskUserQuestion` with the bucket's prompt template): approval-required governance operations, destructive operations, version-pin changes, scope expansions, decisions a grilling session puts to the user. The first and last entries are **categories**: each skill instantiates it with the operations its own run cannot authorize for itself — the triage ops (CREATE / DELETE / SPLIT / MERGE / SUPERSEDE / UPDATE:REMOVE) in `/qfai-sdd`, and the `TDDLIST-001` accepted-risk waiver, the Drift-Protocol Change-Request escalation and consent to item-level parallelism in `/qfai-implement`. A skill's instantiation must cover every decision its own body already gates on a user: `/qfai-implement`'s Parallelization Policy requires explicit user approval on top of the `delivery-planner` technical gate, so omitting it left a user-gated decision in no bucket while the section claims to classify every decision. The last entry is the category a skill running a grilling session instantiates, with its own frontier and the confirmation that closes the session; a frontier decision fits neither other bucket, so without it the claim fails the moment a session runs.
