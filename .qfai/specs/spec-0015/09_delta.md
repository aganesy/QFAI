...
- Keep existing init preservation and catalog emission. No new parser, validator, role or artifact format is needed. Catalog adoption remains necessary for an already-initialized project whose catalog is old.
- Final validation and independent architecture, completion and QA reviews remain pending. Baseline failures are not waived; CR Applied at remains `-`.
...
- id: VFY-001
  ...
  method: Existing agentDelegationSpec0015 suite, including normal and force reinit
  owner: dev
...
- id: VFY-002
  ...
  method: HEAD-to-worktree payload comparison and the narrowed TDD-0007 selector
  owner: dev
