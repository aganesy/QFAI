...
- REQ-0013: Full-Harness Review Profile (v1.7.14) — `review-profiles.yml` に full-harness プロファイルを追加。always_required: [completion-reviewer, product-surface-reviewer, qa-gatekeeper]。product-experience-architect は `kind: worker` のため review-profiles.yml ではなく `agent-routing.yml` の prototyping evidence phase conditional_agents に配置
