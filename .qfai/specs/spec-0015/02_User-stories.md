...
As a QFAI operator, I want every SKILL.md to carry a `## Default Autopilot Policy` section with three named buckets (auto-decide / ask-user / hard-required) and a Reviewer Gate that emits `R-AUTOPILOT-POLICY-MISSING` (severity error) when the section is absent OR is present but missing one or more required buckets (heading-only / partial population), so that avoidable per-session `AskUserQuestion` prompts collapse to 0–1 while approval-required governance operations / destructive / version-pin / scope-expansion decisions still require human authorization, and a grilling session's decisions still reach the user — the first and last being categories each skill instantiates, with the operations its own run cannot authorize for itself and with its own frontier, so a skill may narrow a bucket to what it can reach but may not add an entry outside the prototype's categories (REQ-0160, DR-0269 Amendments 2 and 4).
