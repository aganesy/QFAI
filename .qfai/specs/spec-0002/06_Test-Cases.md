...
- v1.8.9 (spec-0017 → spec-0012 absorb): the legacy `discussionDesignHardening` validator and its proving tests were retired together with the exploration-sidecar family. TC-0002-0002..0007 (the validator's behavior catalog) were superseded by DESIGN.md-driven equivalents now owned by spec-0012, and have been removed from this active table together with the corresponding TDD ledger rows. The remaining TC entries cover the validator-agnostic discussion surfaces (planner-first posture, non-UI safe skip, canonical wording).
