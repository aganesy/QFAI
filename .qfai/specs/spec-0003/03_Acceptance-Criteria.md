...
```gherkin
# AC-0003-0015
Scenario: The QFAI managed block is appended to the root .gitignore
  Given an empty project directory
  When `qfai init` runs
  Then the root `.gitignore` contains the marker line `# ── QFAI managed (generated by qfai init) ──` exactly once
  And it contains `.qfai/report/*` and `!.qfai/report/README.md`
  And it contains `.qfai/evidence/*` and `!.qfai/evidence/README.md`
  And it contains `.qfai/review/*` and `!.qfai/review/README.md`
  And it contains `.qfai/discussion/discussion-*/`
  ...
  And the lines the project already had are preserved
```
