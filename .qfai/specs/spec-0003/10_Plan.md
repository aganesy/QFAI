...
## validate.json スキーマバージョニング

### 概要

validate.json のルートオブジェクトに `schemaVersion` フィールドを追加し、スキーマの互換性管理を行う。
...
| 区分  | 条件                                                     | 例                                                 |
| ----- | -------------------------------------------------------- | -------------------------------------------------- |
| MAJOR | 破壊的変更（フィールド削除、型変更、必須フィールド変更） | `issues[].severity` の型を string から enum に変更 |
| MINOR | 後方互換のフィールド追加                                 | `summary` に新しい集計フィールドを追加             |
| PATCH | スキーマドキュメントのバグ修正                           | フィールド説明文の誤記修正                         |

### 運用方針

- validate.json は内部契約である（OC-02: `_policies/07_Constraints.md` 参照）が、スキーマバージョニングにより破壊的変更を検出可能にする
- report コマンド（spec-0003）は `schemaVersion` を読み取り、サポート対象バージョンとの互換性を確認する
- サポート対象外のメジャーバージョンを検出した場合、明確なエラーメッセージを出力する
