...
2. Implement `Planner` in `packages/qfai/src/core/harness/planner.ts`
   - Generation strategy creation from spec inputs
   - Pivot handling (revised plan on evaluator signal)
3. Implement `Generator` in `packages/qfai/src/core/harness/generator.ts`
   - Output generation from plan
   - Refinement handling (incorporate evaluator feedback)
4. Implement `Evaluator` in `packages/qfai/src/core/harness/evaluator.ts`
...
5. Implement `HarnessLoop` in `packages/qfai/src/core/harness/loop.ts`
...
6. Implement evidence generation in `packages/qfai/src/core/harness/evidence.ts`
...
7. Verify standard path isolation
