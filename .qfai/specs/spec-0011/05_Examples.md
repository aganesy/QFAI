...
## EX-0011-0010: Record Homes In The Shipped Skill Text

- BR-Ref: BR-0011-0009
- Given the shipped `SKILL.md` and `references/execution-ledger.md`
- When the `-> blocked` edge, the `blocked -> todo` edge and the recording constraint are read
- Then the stop is carried by `Blocked-By`, and a decision, a consultation or an out-of-scope discovery goes to `/qfai-sdd` as a Change Request
- And the `blocked -> todo` bullet contains no `archived` and no instruction to close a record
- And neither file contains `.qfai/steering/`, `worklog-entry.schema.md` or "work-log entry"
