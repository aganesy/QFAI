...
| OC-03 | QFAI 生成成果物（`.qfai/report/*`, `.qfai/evidence/*`, `.qfai/review/review-*/`, `.qfai/discussion/discussion-*/`）はデフォルトで gitignore。`qfai init` が管理ブロックをルート `.gitignore` に冪等追記する。review-pack を共有したい場合はプロジェクト側で明示的に negation を追加する（v1.7.18 拡張） | ローカル成果物であり、リポジトリ肥大化を防ぐ | 証跡管理の制約 |
