...
| -------- | --------------------- | --------------------- | --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| DCON-001 | Exploration Brief     | exploration-brief     | `.qfai/contracts/design/exploration-brief.yaml`     | (REMOVED — UX-loop redesign / spec-0017 OP-0011b: brand SSOT moved to root `DESIGN.md`. History-only.)                                      |
...
| DCON-005 | Design System         | design-system         | `.qfai/contracts/design/design-system.yaml`         | (UX-loop redesign / spec-0017 OP-0004: 最終 iter HTML からの抽出ではなく `DESIGN.md` token の deterministic mirror。validator は DCON-032。) |
| DCON-006 | Reference Pool        | reference-pool        | `.qfai/contracts/design/reference-pool.yaml`        | (REMOVED — UX-loop redesign / spec-0017 OP-0011b: deviate-from framing 廃止、DESIGN.md compliance gate に統合。History-only.)               |
| DCON-007 | Brand Design          | brand-design          | `.qfai/contracts/design/brand-design.yaml`          | (REMOVED — UX-loop redesign / spec-0017 OP-0011b: brand SSOT は root `DESIGN.md` に統合。History-only.)                                     |
...
| DCON-030 | DESIGN.md             | design-md             | `DESIGN.md` (repo root)                             | (UX-loop redesign / spec-0017 OP-0012) brand vision / visual identity (color / font / radius / shadow tokens) の SSOT。markdown 直接編集。 |
| DCON-031 | DESIGN.md Lock        | design-md-lock        | `.qfai/contracts/design/DESIGN.md.lock.yaml`        | (UX-loop redesign / spec-0017 OP-0012) `DESIGN.md` の sha256 hash を `/qfai-sdd` Phase 0 で凍結。cycle ≥1 hash mismatch を fail-closed で検出。 |
| DCON-032 | Design System Mirror  | design-system-mirror  | (validator on `.qfai/contracts/design/design-system.yaml` ↔ DESIGN.md) | (UX-loop redesign / spec-0017 OP-0012) `design-system.yaml` が `DESIGN.md` token と byte-equivalent mirror であることを検証。            |
...
- (UX-loop redesign / spec-0017 OP-0011c) discussion sidecars `uiux/30_exploration_brief.md`, `uiux/31_reference_pool.md`, `uiux/32_design_anti_goals.md` は廃止。`/qfai-discussion` はこれらを生成しない。
...
- (UX-loop redesign / spec-0017 OP-0012) `DCON-030` (root `DESIGN.md`) は `/qfai-discussion` の brand 出力。`/qfai-sdd` Phase 0 で sha256 凍結 (`DCON-031`) を行う。
- (UX-loop redesign / spec-0017 OP-0004) `DCON-005` (design-system) は `/qfai-prototyping` の post-loop で `DESIGN.md` token の deterministic mirror として生成される。`DCON-032` validator が byte-equivalent を検証。
...
- (UX-loop redesign / spec-0017 OP-0011b) `DCON-001` (exploration-brief), `DCON-006` (reference-pool), `DCON-007` (brand-design) は active surface から削除。validator codepath からも除外。history-only。
...
- (UX-loop redesign / spec-0017 OP-0011b) 削除対象 contracts: `DCON-001`, `DCON-006`, `DCON-007`
