...
- Amendment 3 (2026-09-07): the hard-required bucket is machine-checked as well. `QFAI-AUTOPILOT-001` reads every shipped `qfai-*` SKILL.md and reports a bucket naming an entry this decision has retired, or one no skill declares — a warning until the 1.12.0 release and an error from there. The statement above says only auto-decide widening is detectable because its entry set is closed; hard-required's is closed too, by the common entries plus a per-skill declaration, so the same reading applies. `companyName` is the retired entry: no shipped file read it, so the bucket paid a guaranteed prompt out of a 0-1 budget and read nothing back. ask-user remains enforced by review — its first entry is a category each skill instantiates, which no closed set describes.
- Amendment 4 (2026-09-12): a grilling session puts design decisions to the user — a design, an approach, a scope boundary, a trade-off — none of which is a governance authorization, a destructive operation, a version-pin change or a scope expansion. They are `ask-user` all the same, because nothing else describes them: `auto-decide` is a pick among demonstrably-equivalent alternatives and `hard-required` an input with no default, and a frontier decision is neither — it has a recommended answer the user may reject. **A fifth ask-user entry is therefore added, itself a category: _decisions a grilling session puts to the user_, which a skill that runs a session instantiates with every decision its session puts there — the frontier, the confirmation that closes the session, and everything a session between agents escalates — including a decision whose prerequisites never resolved, so it never reached a frontier.** Without it the closed list left a skill that grills two illegal moves and no legal one: widen the bucket, or leave a user-settled decision in no bucket while the section claims to classify every decision. A skill that runs no session narrows the entry away.
