...
- Amendment 4 (2026-09-12): a grilling session puts design decisions to the user — a design, an approach, a scope boundary, a trade-off — and none of them is a governance authorization, a destructive operation, a version-pin change or a scope expansion. They are `ask-user` all the same, because nothing else describes them: `auto-decide` is defined by a pick among demonstrably-equivalent alternatives and `hard-required` by no default being possible, and a frontier decision is neither — it has a recommended answer, and the recommendation is not the user's choice. **A fifth ask-user entry is therefore added, itself a category: _decisions a grilling session puts to the user_, which a skill that runs a session instantiates with the decisions on its own frontier and the confirmation that closes the session.** Without it the closed list left a skill that grills two illegal moves and no legal one: widen the bucket, or leave a user-settled decision in no bucket while the section claims to classify every decision. `/qfai-grilling` and `/qfai-grill` instantiate it; a skill that runs no session narrows it away.
