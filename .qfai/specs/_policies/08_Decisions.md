...
- Status: RE-OPENED — spec-0003 DR-0003-0008 が本決定を置き換える。配布 asset に `.codex/agents/` が存在せず、`qfai init` を実行したプロジェクトの Codex には agent が 1 件も届かないという前提誤りが根拠。以降 `.codex/agents/*.toml` は `qfai init` の生成物として扱う（上記 Rejected / DO NOT はこの spec-0003 の範囲では失効）
