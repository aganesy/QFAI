...
- Then the run exits non-zero deterministically with the documented exit code per class:
  ...
  - (h) certify-side present-but-malformed `prototyping.json#frozenSpecsCovered` field (key IS on the record, but value fails the string-array validation contract — non-array, empty array, non-string entry, empty-string entry) → exit `2` with a "present but malformed" diagnostic naming the rejection reason (33rd-wave addition per codex r3270861808 P1 — chatgpt-codex-connector. Pre-fix the `readFrozenSpecsCoveredMultiSpec(...) ?? readFrozenSpecsCovered(...)` null-coalesce collapsed "missing" and "present-but-invalid" into one branch, which on a partially-corrupt multi-spec record would silently fall back to legacy single-spec `specsCovered` and downgrade certification scope to the resolved primary spec — letting missing secondary-spec review evidence ship a sealed certificate. The new SSOT classifier `classifyFrozenSpecsCoveredMultiSpec()` returns `{kind: "absent" | "malformed" | "ok"}` so the absent case still legitimately falls back for pre-Wave-3 evidence compatibility but `malformed` fails closed),
    AND no user prompt is emitted,
    AND **(ordering invariant — 34th-wave addition per codex r3270889168 MINOR — requirements-reviewer)** the hard-stop classes (a)-(h) MUST be evaluated BEFORE convergence / budget-exhaustion signals (i.e. before `shouldStop()` in iterate's cycle ≥ 1 path, and before any per-(spec × screen) coverage-rejection class on the certify side). When a hard-stop class AND a convergence / coverage signal both fire in the same invocation, the hard-stop class wins and the convergence / coverage signal MUST be suppressed. Rationale: a partial / corrupt lock or a mid-loop drift cannot be "resolved" by satisfying convergence axes or exhausting the iteration budget; honouring `shouldStop`-first would let a mid-loop UI marker removal ship as a successful exit-64 / exit-65 outcome and bypass the lock-drift remediation path entirely (pinned by `EX-0012-0153` + `TC-0012-0424` / `TDD-0444` for the cycle ≥ 1 drift case; this clause is the normative AC text the test pins).
