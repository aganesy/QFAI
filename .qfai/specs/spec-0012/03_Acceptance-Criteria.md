...
- **Cross-class postcondition: no user prompt is emitted** (39th-wave restructure per codex r3271039011 MINOR — requirements-reviewer; restored to its original CHG-002 scope of binding ALL hard-stop classes (a)-(h)). The hard-stop catalog runs in fully autonomous mode: when ANY class triggers, the run exits non-zero deterministically WITHOUT emitting a user prompt. Previously the `AND no user prompt is emitted` clause was indented under class (h)'s Then block via the 33rd-wave / 34th-wave / 37th-wave cascade of edits, which could be misread as a class-(h)-specific postcondition. This clause applies cross-class to the full hard-stop catalog (a)-(h); the autonomous-run semantic is further reinforced by AC-0012-0044.
...
- And **(40th-wave addition per codex r3271093350 MINOR — requirements-reviewer)** a present-but-malformed `prototyping.json#frozenSpecsCovered` field (the SSOT classifier `classifyFrozenSpecsCoveredMultiSpec()` returns `{kind: "malformed", reason}` — value is non-array, empty array, non-string entry, empty-string entry, OR explicit `null` / `undefined` on a present key) → `show-spec` exits `2` with a "present but malformed" diagnostic naming the rejection reason. show-spec MUST NOT silently fall back to the legacy `specsCovered` field on this branch: iterate and certify both treat the same input as a hard error, so reporting a downgraded single-spec scope from show-spec would mislead recovery decisions made by operators / automation that read the show-spec output. This sub-clause mirrors AC-0012-0045 class (h) onto the show-spec surface so the absent-vs-malformed discrimination contract holds across all three CLI surfaces that read `frozenSpecsCovered`. Absent key (record has no `frozenSpecsCovered` field) still legitimately falls back to legacy `specsCovered` for pre-Wave-3 evidence compatibility.
