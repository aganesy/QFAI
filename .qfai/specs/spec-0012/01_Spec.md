...
- REQ-0012-0077 (REQ-0117 follow-up): `stopReason` enum widening — CHG-005 Phase 3 で `prototyping.json#stopReason` enum を {`converged`, `max-iterations`} の 2 値で landing したが、spec brief で要求された残 2 値 (`license-verify-fail`, `input-error`) の emit wiring と validator 受理は deferred。本 follow-up で 4 値 enum を完成させる。Acceptance signal: license-verify failure 経路で `stopReason: "license-verify-fail"` が emit され、入力エラー経路で `stopReason: "input-error"` が emit され、validator (`prototypingEvidence.ts`) が 4 値全て受理する。Test surface: `prototypingIterate.stopReasonEnum.test.ts` (新規) で各経路の emit を pin。
