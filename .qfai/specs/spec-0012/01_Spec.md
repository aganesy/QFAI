...
- In:
  ...
  - NOTE: v1.7.15 rev11 adds 3 workstreams (WS-1 through WS-3) — semantic closure completion: (WS-1) public API surface closure: `runMeasurement()`/`validatePanelScore()` removed from `src/core/index.ts` exports; `runMeasurement()` validates all 8 category refs (renderRefs/browserQaRefs/runtimeGateRefs/uiObservationRefs/specCoverageRefs/discussionRefs/trendRefs/screenContractRefs) non-empty+concrete, `screenContractRefs` canonical form only, `l1.axes`/`l2.axes` non-empty pre-check, mandatory `validatePanelScore()` call; `validatePanelScore()` validates axes non-empty + per-axis evidenceRefs non-empty+concrete; (WS-2) declaredRef semantic closure: `isSpecDeclarationRef()` restricted to `.qfai/specs/<specId>/01_Spec.md#L<positive integer>` only — anchor fragments/notes.md/appendix.md/#L0/abs paths rejected; `specCoverage.ts` scan restricted to `01_Spec.md` only (other .md files in spec dir ignored); (WS-3) test synchronization: `measurement.test.ts`/`panelScore.test.ts` updated to current DTO/panel score shape with rev11 negative cases; `specCoverage.test.ts`/`refSemantics.test.ts` created or extended with semantic boundary coverage
  ...
  - Coverage Matrix for all specs (uiRoutes only; apiEndpoints/dbObjects retired as dead fields per DR-0012-0057, rev11)
