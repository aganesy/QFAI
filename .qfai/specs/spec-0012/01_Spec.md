...
- In:
  ...
  - NOTE: v1.7.16 adds QFAI パッケージ側デザイン品質パイプライン構造改善（discussion-20260418093755100）— (1) prototyping SKILL.md に Delegation Scope Table（UI実装/スクリーンショット/評価L1-L2/ビルドの4カテゴリ × frontend-engineer/devops-ci-engineer/product-surface-reviewer 等ロール）、(2) 反復ゲート（full-harness 時 iterationCount==1 && converged=true を ERROR、最低2反復強制）、(3) Required Process Step 0（executionPlan: targetIterations / evaluationAxesSource / delegationMap / plannedAt を prototyping.json に必須化）、(4) `qfai.config.yaml` prototyping.calibration.overrides（perAxisMinimum / maxIterationsByMode プロジェクトオーバーライド）、(5) 共有 `capture-screenshots.js`（QFAI パッケージ配置、入力 URL/port+出力 dir、出力 タイムスタンプ付きパス一覧）、(6) 反復サイクル 5 ステップ化（Capture→Evaluate→Identify→Fix→Re-evaluate、scoringTrace.screenshotDir 追加）、(7) 評価者入力準備プロトコル（スクリーンショットパス／評価軸定義テキスト／前回スコア／DESIGN.md 準拠チェックリスト の 4 要素 MUST）、(8) Visual Quality Structural Checklist（カラー／タイポグラフィ／スペーシング／角丸／シャドウ／Do's&Don'ts の 6 カテゴリ、Lighthouse Gate を full-harness && web surface 時 MUST に格上げ）、(9) Evaluate ステップに uiux/12_design_system.md 存在時の designSystemCompliance チェック（CSS 実装値 ↔ design system 仕様値一致率、80% 未満を L1 finding / 即時修正対象）
  - Delegation Scope Table（`packages/qfai/assets/init/.qfai/assistant/skills/qfai-prototyping/SKILL.md`）: UI実装 → frontend-engineer、スクリーンショット → devops-ci-engineer（`capture-screenshots.js` 経由）、評価 L1/L2 → product-surface-reviewer / product-experience-architect、ビルド → devops-ci-engineer。定義外ロールへの移譲は reviewer が検出
  - 反復ゲート（prototyping SKILL.md）: full-harness 実行時は terminationCondition 充足まで次フェーズ移行を禁止。`iterationCount === 1 && converged === true` は常に ERROR として処理し、最低 2 回の反復を強制
  - Step 0 Execution Plan（SKILL.md Required Process 先頭 MUST）: `prototyping.json.executionPlan` = `{ targetIterations, evaluationAxesSource, delegationMap, plannedAt }`。full-harness 時は存在必須、欠落はバリデーション error
  - `qfai.config.yaml` の `prototyping.calibration.overrides`（任意拡張）: `perAxisMinimum`（全軸最低スコア閾値）、`maxIterationsByMode`（モード別最大反復数）。プロジェクト側での system default 上書き用。existence-based precedence（project override > system default）
  - `capture-screenshots.js`（QFAI パッケージ配置、`packages/qfai/assets/scripts/capture-screenshots.js` 想定）: 入力=target URL/port + 出力ディレクトリ、出力=タイムスタンプ付きファイル名を持つスクリーンショットパス一覧。prototyping SKILL.md から参照
  - 反復サイクル 5 ステップ化: Capture → Evaluate → Identify → Fix → Re-evaluate。`prototyping.json.scoringTrace[]` に `screenshotDir`（反復ごとに使用したスクリーンショットディレクトリ）フィールドを追加
  - 評価者入力準備プロトコル（L1/L2 サブエージェント起動前 MUST）: (a) スクリーンショットパス、(b) 評価軸定義テキスト展開、(c) 前回スコア、(d) DESIGN.md 準拠チェックリスト。4 要素すべてを評価者コンテキストに渡すこと
  - Visual Quality Structural Checklist（SKILL.md、reviewer/evaluator 参照）: カラー / タイポグラフィ / スペーシング / 角丸 / シャドウ / Do's&Don'ts の 6 カテゴリ。Lighthouse Gate は full-harness && web surface 時 MUST（従前 SHOULD から格上げ）
  - designSystemCompliance チェック（Evaluate ステップ）: `uiux/12_design_system.md` が discussion pack に存在する場合、CSS 実装値（color, typography, spacing, radius, shadow 等）と design system 仕様値の一致率をスコア化。80% 未満を L1 finding として即時修正対象に分類
...
- NFR-0046 (v1.7.16): 後方互換性（Compatibility）— 既存の discussion pack（12 ファイル構造）および prototyping.json（既存スキーマ）を破壊しないこと。新フィールド（`executionPlan`, `scoringTrace[].screenshotDir`, `calibration.overrides`, `designSystemCompliance` 等）は任意拡張として追加し、当該フィールドが存在しない既存パック/json もパース可能であること。新バリデータルールは段階的移行（原則 WARNING 導入→次バージョン ERROR 昇格）
- NFR-0047 (v1.7.16): AI-only 操作性（Operability）— Delegation Scope Table に基づく移譲／反復ゲート／Step 0 executionPlan 記録／capture-screenshots.js 実行／評価者入力準備／designSystemCompliance スコア計算 の全工程が AI エージェント単独で実行可能であり、人間への確認待ちを必須フロー内に含めないこと
- NFR-0048 (v1.7.16): パッケージ独立性（Modularity）— v1.7.16 改善はすべて `packages/qfai/` 配下（src/, assets/init/.qfai/assistant/skills/qfai-prototyping/SKILL.md, assets/scripts/capture-screenshots.js 等）に閉じ、運用ディレクトリ `.qfai/` への変更を含まないこと。個別プロジェクト（qfai-virtual-product-5 等）への side effect 0 件
- NFR-0049 (v1.7.16): 検証速度（Performance）— executionPlan / screenshotDir / designSystemCompliance / calibration.overrides 検証追加による `qfai validate` 実行時間増加が、v1.7.15 baseline の 20% 以内に収まること。逐次 I/O を避け必要最小限で完結させる
- NFR-0050 (v1.7.16): オンライン前提（Environment）— capture-screenshots.js やその他外部依存コンポーネントはオンライン環境を前提とし、オフラインフォールバック実装を含めない。自動テストでは外部依存のモックで代替可
...
- REQ-0123 (v1.7.16; discussion REQ-0001): prototyping SKILL.md Delegation Scope Table — UI 実装 / スクリーンショット / 評価 L1-L2 / ビルド の 4 カテゴリごとに対応ロール（frontend-engineer, devops-ci-engineer, product-surface-reviewer, product-experience-architect 等）を定義する。SKILL.md にテーブルを配置し、定義外ロールへの移譲を reviewer が検出可能な記述を含める
- REQ-0124 (v1.7.16; discussion REQ-0002): prototyping SKILL.md 反復ゲート — full-harness 実行時、`terminationCondition` 充足まで次フェーズ禁止。`iterationCount === 1 && converged === true` を ERROR 扱いとし、最低 2 反復を強制。テストで当該条件が検出可能であること
- REQ-0125 (v1.7.16; discussion REQ-0003): prototyping SKILL.md Step 0（実行計画 MUST）— Required Process の先頭に Step 0 を追加し、`prototyping.json.executionPlan = { targetIterations, evaluationAxesSource, delegationMap, plannedAt }` ブロックを定義。full-harness 時は必須とし、欠落をバリデーションで検出
- REQ-0126 (v1.7.16; discussion REQ-0004): qfai.config.yaml prototyping.calibration.overrides — `perAxisMinimum`（全軸最低スコア閾値）と `maxIterationsByMode`（モード別最大反復数）をプロジェクトオーバーライド可能にする。既存 calibration 設定への破壊的変更を含まないこと（NFR-0046）
- REQ-0127 (v1.7.16; discussion REQ-0010): 共有キャプチャスクリプト `capture-screenshots.js` — QFAI パッケージに配置（`packages/qfai/assets/scripts/` 配下を想定）。入力=ターゲット URL/port + 出力ディレクトリ、出力=タイムスタンプ付きファイル名を持つスクリーンショットパス一覧。prototyping SKILL.md から参照
- REQ-0128 (v1.7.16; discussion REQ-0011): 反復サイクル 5 ステップ化 — Capture → Evaluate → Identify → Fix → Re-evaluate を SKILL.md に明記。`prototyping.json.scoringTrace[]` に `screenshotDir` フィールドを追加し、反復ごとに使用したスクリーンショットディレクトリを記録
- REQ-0129 (v1.7.16; discussion REQ-0012): 評価者入力準備プロトコル — L1/L2 評価者（サブエージェント）起動時、(a) スクリーンショットパス / (b) 評価軸定義テキスト展開 / (c) 前回スコア / (d) DESIGN.md 準拠チェックリスト の 4 要素を MUST 入力として SKILL.md に明記
- REQ-0130 (v1.7.16; discussion REQ-0013): Visual Quality Structural Checklist + Lighthouse MUST — カラー / タイポグラフィ / スペーシング / 角丸 / シャドウ / Do's&Don'ts の 6 カテゴリを SKILL.md に定義。Lighthouse Gate を full-harness && web surface 時 MUST に格上げ（SHOULD から MUST）
- REQ-0131 (v1.7.16; discussion REQ-0017): prototyping SKILL.md Evaluate ステップ designSystemCompliance チェック — `uiux/12_design_system.md` が discussion pack に存在する場合、CSS 実装値 ↔ design system 仕様値の一致率をスコア化し、80% 未満を L1 finding（即時修正対象）として分類する処理を SKILL.md に記述
