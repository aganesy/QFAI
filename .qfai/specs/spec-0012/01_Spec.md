...
- In:
  ...
  - global convergence judged **per cycle by AND across all spec × screen pairs**: `blockingFindings` empty AND `layoutAntiPatternsDetected` empty AND `designMdViolations` empty; the four UX axes are reported and do not gate, and quantitative AC-pass% / transition-pass% thresholds are not used
...
- REQ-0012-0062 (REQ-0110): `qfai prototyping iterate` MUST accept the opt-in `--auto-serve` flag (default OFF). When passed, iterate MUST call its server runner once, MUST invoke the teardown the runner returns at the end of the cycle and on SIGINT, MUST continue when the runner reports a recovered prior owner, and MUST exit 2 with the runner's reason on stderr when the runner refuses. When that teardown rejects, iterate MUST print a line on stdout naming the `--auto-serve` teardown as what failed, with the rejection's reason, and MUST return the exit code the cycle would have returned had the teardown resolved. The default runner is the in-process server `REQ-0012-0076` states. Default-OFF posture preserves `DR-0012-0029`; amendment is pinned by `DR-0012-0031`.
...
- REQ-0012-0078 (REQ-0129 follow-up): `--check-convergence` CLI flag wiring — CHG-005 Phase 3 で `--cycle` out-of-range peek-mode hint 文字列 (literal `--check-convergence` を含む) を pin した (REQ-0012-0073 / TC-0012-0470) が、`--check-convergence` flag 自体は `cli/lib/args.ts` で parse されていない documentation-only contract。operator が hint に従って `qfai prototyping iterate --cycle 9 --check-convergence` を実行すると "unknown flag" エラーが返る。本 follow-up で `--check-convergence` flag を CLI parser に追加し、最終 cycle (cycle=9) の収束状態を peek (re-run なし) する read-only invocation path を実装する。Acceptance signal: `qfai prototyping iterate --cycle 9 --check-convergence` invocation が `.qfai/prototypes/iter-09/prototyping.json` を read-only mode で読み、再 iterate を起こさずに `stopReason` + `acceptedIterationIndex` を report して exit 0 (収束済) または exit 2 (未収束) を返す。
