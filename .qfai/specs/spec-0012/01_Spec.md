...
- REQ-0012-0076 (REQ-0110 follow-up): `--auto-serve` CLI flag wiring + default spawn/tree-kill runner — CHG-005 Phase 2 で `RunPrototypingIterateOptions` に `autoServe: boolean` + `serverRunner?: ServerRunnerFn` の DI surface を landing したが、`cli/lib/args.ts` への flag-string parser 配線 + default spawn + `tree-kill` (Linux/macOS) / `taskkill /F /T` (Windows) teardown runner は deferred。本 follow-up で operator が `qfai prototyping iterate --auto-serve` を CLI から直接呼んで HTTP server の spawn / SIGINT teardown が動作する状態に揃える。Acceptance signal: `qfai prototyping iterate --auto-serve` invocation が default port で local HTTP server を spawn し、SIGINT で 2 秒以内に tree-kill + foreign-process refusal を honor する。`--auto-serve` 未指定時の no-server default は保持される。
