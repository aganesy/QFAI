...
- US-0012-0102: As a reviewer, I match the iter against the declared layout anti-patterns, each naming the authority that…
...
As a reviewer, I match the iter against the identifiers declared in `packages/qfai/assets/validators/layoutAntiPatterns.json`, each carrying the authority that makes it a defect: a published heuristic, an accessibility criterion, or the screen contract the screen is built to. A detection is a blocking finding.
