...
- Mitigation while deferred: the contradiction requires a MULTI-SPEC frozen set — `certify` hard-fails only that case on the flat layout — so the single-spec freeze in `prototypingIterate.ts` is what keeps it unreachable, and **`TC-0012-0388` already pins that freeze**: it seeds a second UI-bearing spec and asserts the frozen set stays one entry. Measured by lifting the freeze: `TC-0012-0388` goes red, while a purpose-built reachability test watching for the per-spec layout being produced stayed green. So no additional guard is warranted, and none was added; `CR-20260904-0002` records the four structural drafts and one behavioural draft that were tried and why each was weaker or wrong. Because `TC-0012-0388` is a spec-0012 test case, lifting the freeze cannot be done quietly — it reddens that test, and amending the test is an upstream SSOT edit needing its own Change Request, which forces this decision to be made rather than merely failing.
