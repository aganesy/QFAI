...
- Mitigation while deferred: the single-spec freeze in `prototypingIterate.ts` keeps the contradiction unreachable, and `TC-0012-0388` pins it by seeding a second UI-bearing spec and asserting the frozen set stays one entry. The trigger itself — `iterate` PRODUCING the per-spec layout — is guarded behaviourally in `packages/qfai/tests/cli/commands/prototypingIterate.test.ts`: a cycle-0 run with two UI-bearing specs available must leave no `iter-NN/spec-NNNN/` directory under `.qfai/evidence/prototyping/`, which is the exact shape `certify`'s `hasPerSpecSubdir` decides on. Its failure message names this open question and `CR-20260904-0002` and says the decision now has to be made, rather than asking the reader to revert. The guard is behavioural on purpose: `cli/commands/prototypingCertify.ts` already composes per-spec paths from a template string with no import of `core/prototyping/iterationPaths.ts`, so any structural check over module edges or call sites would report the layout unreachable while it is being written.
