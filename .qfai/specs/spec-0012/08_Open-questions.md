...
- Mitigation while deferred: the single-spec freeze in `prototypingIterate.ts` keeps the contradiction unreachable, and `TC-0012-0388` pins it by seeding a second UI-bearing spec and asserting the frozen set stays one entry. The reachability trigger itself is guarded by `packages/qfai/tests/unit/reviewLayoutContradiction.test.ts`, in three rows: no production module takes a runtime edge (static import, re-export, namespace import or a dynamic `import()`) into `core/prototyping/iterationPaths.ts` or `core/prototyping/reviewerDispatch.ts` (beyond the layout-neutral cleanup helpers `findStaleIterDirs` / `deleteStaleIterDirs`); those modules' exported surface is pinned, so a forwarding `export const x = iterationReviewPathPerSpec` is caught where it is written rather than only where it is used; and `validateIterationReviewArtifacts` still reads the flat layout, resolved through the import binding so an alias is still the same helper. All three failure messages name this open question and `CR-20260904-0002` and say the decision now has to be made, rather than asking the reader to revert. Naming a trigger is not detecting it: before that guard existed, a wire-in would have made this contradiction live on real projects with nothing failing.
