...
- Question: `validate` requires the flat `iter-NN/review.json` for every recorded non-seed iteration. `certify` does NOT demand the per-spec layout unconditionally — it checks whether a per-spec subdirectory exists, and only then requires a file per declared `(spec, screen)` pair (`prototypingCertify.ts:633-666`, existence only; a flat-only single-spec project takes the skip and passes). So the failure is per-spec WITHOUT flat: `validate` reports `QFAI-PROT-002` and `certify` refuses to seal while it does. `.qfai/contracts/cli/qfai-prototyping-iterate.md` nonetheless calls the per-spec file "the sole per-cycle Reviewer artifact", so one layout has to be named canonical or the wire-in has to dual-write.
