...
- Mitigation while deferred: the trigger is guarded behaviourally in `packages/qfai/tests/cli/commands/prototypingIterate.test.ts`, on the condition that actually holds — **an iteration must not carry per-spec review artifacts without the flat `review.json` beside them**, asserted over every `iter-NN` directory after a cycle-0 and a cycle-1 run. `certify` branches on `hasPerSpecSubdir` before it looks at the frozen set, so a per-spec-only writer contradicts `validate` immediately even for a SINGLE-spec frozen set; `TC-0012-0388` does not cover this, because the condition does not depend on `frozenSpecsCovered`. A dual-write migration (flat kept, per-spec added) satisfies both gates and is deliberately allowed. Verified in all three directions: per-spec-only red, dual-write green, flat-only green. `CR-20260904-0002` records the five drafts that were wrong first and what refuted each.
