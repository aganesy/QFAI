...
- Due: 2026-06-30
- Severity: low
- Source: CHG-002 review (raised 2026-05-19).
...
- Resolves: blocks legacy-to-per-spec layout cutover in `prototypingIterate.ts`. Couples with the TDD-0384 deferral.
...
- Couples: TDD-0436 / TC-0012-0416 / AC-0012-0038 (cycle-9 idempotency Then clause; landed on AC-0012-0044 and moved to AC-0012-0038 because terminator routing is a 10-cycle iteration-budget concern, not an autonomous-run concern)
- Due: 2026-06-30
- Severity: low
- Source: CHG-002 review (original deferred follow-up; traceability stitch).
...
- Resolves: closes the OP-APPEND-079 pattern asymmetry (deferred-followup OQ + tdd/test-list / 16_Traceability-ledger / 06_Test-Cases triad now mirrored in 08_Open-questions). The OP-APPEND register entry in `09_delta.md` records it.
...
- Couples: AC-0012-0037 / AC-0012-0047 / BR-0012-0030 (paths-related contracts); the `specDirExists` / `readPerSpecScreens` / `readUiContractScreenContracts` fixes
- Due: 2026-06-30
- Severity: low (architectural / consistency)
- Source: the `path.join` vs `path.resolve` bug class — accepted with deferral; the fix targets the two same-responsibility helpers (`readPerSpecScreens` and `readUiContractScreenContracts`) that directly partner with each other on the certify path, but ~11 other call sites carry the same pattern.
...
- Due: 2026-10-31
- Severity: medium
- Source: hardening the reviewer-deliverable gate.
...
- Measured on `main` at `aa7bcd23` (2026-09-04), so the deferral rests on facts rather than on the issue's framing:
  ...
  - Not derivable: `proseCritique` is one capped string, `impressions` is `Record<FeelField, string>` over six bounded fields; composing one from the other fabricates a critique.
