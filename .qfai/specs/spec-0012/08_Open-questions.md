...
- Mitigation while deferred: the single-spec freeze in `prototypingIterate.ts` keeps the contradiction unreachable, and `TC-0012-0388` pins it by seeding a second UI-bearing spec and asserting the frozen set stays one entry. The reachability trigger itself is guarded by `packages/qfai/tests/unit/reviewLayoutContradiction.test.ts`: one row fails when any production module calls `iterationReviewPathPerSpec` or `dispatchReviewerToPair`, and one fails if the reviewer-deliverable gate stops reading the flat layout. Both failure messages name this open question and `CR-20260904-0002` and say the decision now has to be made, rather than asking the reader to revert. Naming a trigger is not detecting it; before that guard existed, a wire-in would have made this contradiction live on real projects with nothing failing.
