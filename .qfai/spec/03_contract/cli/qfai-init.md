...
- Owning flow: `BF-0001`
- Used-by: story-tree initialization, assistant-tree upgrade, and delivery workflows
- SSOT modules:
...
- Companion contracts:
  - `.qfai/spec/03_contract/cli/shipped-workflows.md` — the ownership boundary,
  ...
  - `.qfai/spec/03_contract/cli/qfai-workflow.md` and
    `.qfai/spec/03_contract/cli/workflow-files.md` — the command the free-text
    entry drives and the files it reads, see [Workflow entry](#workflow-entry)
...
An old assistant `README.md` is removed only when its entire body still matches the former generated signature.
...
`--force` refreshes shipped `assistant/skill/**` and `assistant/agent/**`, generated wrappers and files that the provenance record still identifies as unedited. It preserves the story tree, rule overlays and project routing or review-profile overrides. It does not write an assistant manifest or merge routing YAML.
...
`.qfai/spec/03_contract/cli/shipped-workflows.md` and is not restated here.
...
(`.qfai/spec/03_contract/cli/qfai-doctor.md` §`workflows.integrity`). `qfai init`
...
fresh install and on an upgrade alike. The command is
`.qfai/spec/03_contract/cli/qfai-workflow.md`, and the files it reads are
`.qfai/spec/03_contract/cli/workflow-files.md`.

### What it installs

- `qfai-run` and `qfai-maintain` under `.qfai/assistant/skill/`, and each stage
  skill a built-in plan names with its `references/orchestrated-mode.md`.
- Host links for the two skills, made by the same integration step as every
  other shipped skill. A link carries no policy of its own.
- No `agents/openai.yaml` for any skill, on a plain run or under `--force`.

The built-in plans and the workflow schemas stay in the installed package.
Init writes neither into the project, as it writes no `process/` tree. The two
skills' routing entries are package defaults, read the way
[Configuration](#configuration) describes, so an upgrade needs no routing step.
...
`AGENTS.md` and `CLAUDE.md` when no operative copy exists. It tells the agent
to send a first free-text change request to `qfai-run`.

- It does not depend on `REVIEW.md`.
- It is not written into `.github/copilot-instructions.md`. The directive would
  send Copilot to a skill that fails closed there.
- Existing text and line endings are preserved, and the refusals of that
  section apply.
- A rerun adds nothing.

The seeded `AGENTS.md` and `CLAUDE.md` open with the directive above their
level-1 heading. The shipped Markdown lint configuration therefore lets the
first-line-heading rule accept a preamble, and still requires the first
heading to be level 1.

### Mode line

Init writes no `workflow.mode` key and asks no mode question. Its summary gains
one line naming the mode in force: the configured value, or `active` when the
key is absent. A value other than `active`, `shadow` or `off` is named as
invalid on that line. The line changes no exit code. The key itself is stated
in `.qfai/spec/03_contract/cli/configuration.md#workflow-mode`.

### Skipped skills

A plain run leaves a shipped skill whose project copy differs from the template,
compared ignoring line endings. The summary counts those skills on one line. It
names `qfai init --force` as the command that updates them, and says that
command replaces them with the shipped versions, overwriting local edits. Under
`--force` no count is printed, because that run replaces them.

### Windows parity

Init and upgrade behave the same on Windows as on Linux, CRLF checkouts and
paths with spaces included:

- provenance compares text after CRLF normalization, so an unmodified file with
  CRLF endings counts as unmodified;
- every key of `.qfai/assistant/.assets.lock.json` is a project-relative path
  separated by `/`;
- a managed `.gitignore` block written with CRLF endings is replaced, not
  duplicated.
...
3. prints one line naming the detected path and `/qfai-migration-v1-to-v2`;
...
`/qfai-migration-v1-to-v2` is installed and linked like
...
`qfai-migration-v1-to-v2.md`.

`qfai-migration-spec-to-story`, the skill's name in earlier 2.0 releases, is a
retired skill ID. A project that upgrades keeps
`.qfai/assistant/skill/qfai-migration-spec-to-story/` and its host links until
it runs `qfai init --force`. That run removes the host links init made for the
old name, as it does for every retired skill. Afterwards `qfai validate`
reports no finding naming the old name, and a copy the project changed there is
...
- `.qfai/assistant/rule/**`: `constitution.md`, `communication.md`,
  ...
  `research-first-protocol.md`, `ui-definition-protocol.md` and
  `ui-procurement.md`.
...
It also carries two lines for `npx qfai workflow`:

- `.qfai/run/`, so run state is ignored;
- `!.qfai/evidence/workflow/`, after the `.qfai/evidence/*` ignore line, so run
  evidence stays tracked.

After init, `git check-ignore` reports `.qfai/run/x` ignored and
...
Rule refs: BR-0631, BR-0654

| BR-ID   | Statement                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Examples                                                                            |
| ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------- |
| BR-0035 | With the `rule/ skill/ agent/ prompt/` assistant tree, `.qfai/assistant/` holds exactly `rule/`, `skill/`, `agent/` and `prompt/`, plus the project's own `skill.local/` where it exists. `constitution/`, `manifest/`, `catalog/` and `process/` are absent. No period accepts both the old and the new directory names (`.qfai/spec/03_contract/cli/qfai-validate.md#assistant-tree`, `.qfai/spec/03_contract/cli/qfai-init.md#the-assistant-tree`)                                                                                                                                                                                                                                                                                                                                                                                                                    | EX-0001-0012-01, EX-0001-0012-02, EX-0001-0012-05                                   |
| BR-0036 | With the `rule/ skill/ agent/ prompt/` assistant tree, a file read by several skills or by the CLI goes to `.qfai/assistant/rule/`, and a file read by one skill goes to that skill's `references/` (`.qfai/spec/03_contract/cli/qfai-init.md#the-assistant-tree`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | EX-0001-0012-03, EX-0001-0012-04                                                    |
| BR-0041 | Without `--force`, the `root/` and `.qfai/` templates are placed create-only (an existing file is skipped)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | EX-0001-0021-01, EX-0001-0203-12                                                    |
...
| BR-0043 | `--dry-run` 時はファイル書き込み・symlink 作成・削除を一切行わない                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | EX-0001-0023-01, EX-0001-0023-02, EX-0001-0025-03                                   |
| BR-0044 | symlink ターゲットは相対パスで指定し、リポジトリの絶対パスに依存しない                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | EX-0001-0024-01                                                                     |
| BR-0045 | skills ディレクトリの symlink は `type: 'dir'` で作成する                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | EX-0001-0024-02                                                                     |
| BR-0046 | Agent file symlinks are created with `type: 'file'`. `qfai init` writes no `README.md` into `.agents/`, `.codex/`, `.claude/agents/` or `.github/agents/`, neither as a symlink nor as a regular file.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | EX-0001-0025-01                                                                     |
| BR-0047 | Git リポジトリ内の場合のみ `git config core.symlinks true` を実行する。Git リポジトリ外では skip する                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | EX-0001-0028-02, EX-0001-0028-03                                                    |
| BR-0048 | Windows で EPERM エラー時は Developer Mode 有効化の案内 URL を含むエラーメッセージを表示して処理を中断する                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | EX-0001-0028-01                                                                     |
| BR-0049 | instructions ファイルは `--force` なしでは create-only。`--force` 時は shipped テンプレートで再生成する (a symlink at the entry itself is replaced as an entry, whatever it points to, and its target is left unchanged; an entry whose parent directory resolves outside the project through a symlinked ancestor is not written, even under `--force`)。0バイトの空ファイルも「存在する」として扱う                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | EX-0001-0030-01, EX-0001-0030-02, EX-0001-0031-01, EX-0001-0031-02, EX-0001-0031-03 |
...
| BR-0054 | On a rerun, init removes the managed block from the marker through its last known line (a current block line or a `QFAI_GITIGNORE_LEGACY_LINES` entry), looking no further than the next blank line, comment or end of file, and writes the rebuilt block in its place. A project line written directly under the block, after its last known line, stays outside the block and below it. The rebuilt block keeps the ignore lines the existing block holds, drops the legacy lines and re-appends the governance negations; it does not re-add an ignore line the project removed from the block.                                                                                                                                                                                                                                                                       | EX-0001-0033-02, EX-0001-0033-08, EX-0001-0033-09                                   |
| BR-0055 | Init seeds `.qfai/assistant/` with `rule/`, `skill/`, `agent/` and `prompt/` from the shipped assets. It writes none of `constitution/`, `manifest/`, `catalog/`, `process/` and `steering/`. A project's overlay of a master that moved to `rule/` sits beside it as `rule/<name>.local.md`, and init neither writes nor overwrites it (CLI-INIT § Story-tree layout, "The assistant tree")                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | EX-0001-0034-01, EX-0001-0034-03                                                    |
...
| BR-0088 | Where init lays out the story tree, it writes from the `qfai-sdd` templates `.qfai/spec/decisions.md` and `.qfai/spec/open-questions.md` with header rows only, `01_policy/glossary.md`, `01_policy/constraint.md`, `02_business-flow/business-flows.md`, `03_contract/contracts.md`, and the directories `03_contract/api/`, `db/`, `ui/`, `cli/` and `design/`. It writes no `business-flow-NNNN/` and no `user-story-NNNN-NNNN/`, so a fresh tree carries no test obligation, and it creates none of `.qfai/specs/` and `.qfai/contracts/`, which are retired, nor `.qfai/discussion/`, `.qfai/evidence/`, `.qfai/review/` and `.qfai/report/`, which the stages that write into them create. Each seeded Markdown file conforms to the schema its mdschema manifest entry names (CLI-INIT § Story-tree layout, "The spec tree")                                      | EX-0001-0020-01, EX-0001-0038-01, EX-0001-0038-02, EX-0001-0038-03                  |
| BR-0089 | The story-tree seeds are create-only on every run, `--force` included: an existing file is never rewritten and an absent one is written. The tree is project content (CLI-INIT § Story-tree layout, "The spec tree")                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | EX-0001-0038-04                                                                     |
...
| BR-0096 | Init generates one `.codex/agents/<name>.toml` profile per canonical card from its frontmatter name, description and kind and its body. Only kind: reviewer gets sandbox_mode = read-only. A plain run preserves an existing profile; --force regenerates it; --dry-run writes no profile. If kind cannot be determined, init skips the profile and reports why.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | EX-0001-0025-02, EX-0001-0025-03, EX-0001-0025-04                                   |
...
| BR-0530 | The `.github/copilot-instructions.md` that `qfai init` writes describes the legacy `.qfai/assistant/steering/` and `.qfai/assistant/instructions/` layout as the product treats it: past its compatibility window, and reported by `qfai init` on stderr as a `D-DEPRECATED-PATH` error. It names `qfai init --upgrade-assistant-tree` as the migration, and it calls the layout neither read-compatible nor the finding a warning.                                                                                                                                                                                                                                                                                                                                                                                                                                      | EX-0001-0029-03                                                                     |
| BR-0684 | The managed `.gitignore` block ignores `.qfai/run/` and negates `.qfai/evidence/workflow/` after the `.qfai/evidence/*` ignore line, so run state is ignored and run evidence stays tracked. A rerun over a block an earlier release wrote without them replaces the block, so the marker and each of the two lines occur once, and a further rerun writes nothing (CLI-INIT § Story-tree layout, "The managed `.gitignore` block")                                                                                                                                                                                                                                                                                                                                                                                                                                      | EX-0001-0033-05, EX-0001-0033-06                                                    |
| BR-0685 | A fresh install and an upgrade install `qfai-run`, `qfai-maintain` and each plan-named stage skill's `references/orchestrated-mode.md` through the ordinary asset copy, and link the two skills into every host skills directory from their one source. No built-in plan and no workflow schema is written into the project, and an upgrade leaves every other skill directory as it was (CLI-INIT § Workflow entry, "What it installs")                                                                                                                                                                                                                                                                                                                                                                                                                                 | EX-0001-0203-01, EX-0001-0203-02, EX-0001-0203-03                                   |
| BR-0686 | `qfai init` writes no `agents/openai.yaml` for any skill, on a plain run or under `--force` (CLI-INIT § Workflow entry, "What it installs")                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | EX-0001-0203-04                                                                     |
| BR-0687 | Init prepends the entry directive, which sends a first free-text change request to `qfai-run`, to `AGENTS.md` and `CLAUDE.md` through the review directive's mechanism and refusals when no operative copy exists, whether or not `REVIEW.md` exists. It never writes it into `.github/copilot-instructions.md`, a fenced copy is not operative, and a rerun adds nothing (CLI-INIT § Workflow entry, "Entry directive")                                                                                                                                                                                                                                                                                                                                                                                                                                                 | EX-0001-0203-05, EX-0001-0203-06, EX-0001-0203-07, EX-0001-0203-08, EX-0001-0203-09 |
| BR-0688 | The shipped Markdown lint configuration lets the first-line-heading rule accept a preamble above the first heading and still requires that heading to be level 1, so the seeded `AGENTS.md` and `CLAUDE.md` can open with the entry directive (CLI-INIT § Workflow entry, "Entry directive")                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | EX-0001-0203-10                                                                     |
| BR-0689 | Init writes no `workflow.mode` key and asks no mode question. Its summary has one line naming the mode in force, `active` for an absent key, and names a value other than `active`, `shadow` or `off` as invalid. The line changes no exit code (CLI-INIT § Workflow entry, "Mode line")                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | EX-0001-0203-11, EX-0001-0203-12, EX-0001-0203-13                                   |
| BR-0692 | Init and upgrade give the same tree, summary and exit code on a CRLF checkout and under a path with a space as on Linux. Provenance compares text after CRLF normalization, lock keys are `/`-separated project-relative paths, and a managed `.gitignore` block with CRLF endings is replaced rather than duplicated (CLI-INIT § Workflow entry, "Windows parity")                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | EX-0001-0203-16, EX-0001-0203-17, EX-0001-0203-18, EX-0001-0033-07                  |
| BR-0693 | A plain run counts, on one summary line, the shipped skills it left because the project's copy differs from the template ignoring line endings. The line names `qfai init --force` as the command that updates them and says that command replaces them with the shipped versions, overwriting local edits. A copy that differs only in line endings is not counted, and `--force` prints no count (CLI-INIT § Workflow entry, "Skipped skills")                                                                                                                                                                                                                                                                                                                                                                                                                         | EX-0001-0203-19, EX-0001-0203-20                                                    |
| BR-0764 | `qfai-migration-spec-to-story` is a retired skill ID. `qfai init --force` removes each host skill link init made for it, as for every retired skill. After that run, `qfai validate` reports no finding naming `qfai-migration-spec-to-story`, and a copy of `.qfai/assistant/skill/qfai-migration-spec-to-story/` the project changed is kept, not deleted (CLI-INIT § Story-tree layout, "The migration skill")                                                                                                                                                                                                                                                                                                                                                                                                                                                        | EX-0004-0041-14                                                                     |
