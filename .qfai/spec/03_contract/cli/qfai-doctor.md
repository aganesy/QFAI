...
with its resolved path. A configured or present deprecated `paths.promptsDir`
produces the `paths.promptsDirDeprecated` warning and directs migration to
`skillsDir`; a default path that was never created is not itself drift.

## Rules

| BR-ID   | Statement                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Examples                         |
| ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------- |
| BR-0178 | --format 未指定時はデフォルト text としてテキスト形式で診断結果を出力する                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | EX-0003-0001-01, EX-0003-0005-01 |
| BR-0179 | CLI コマンドは createDoctorData() に診断ロジックを委譲し、結果をフォーマットする                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | EX-0003-0001-03, EX-0003-0002-01 |
| BR-0180 | --root が明示指定されない場合は startDir から qfai.config.yaml を探索して root を決定する                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | EX-0003-0001-02                  |
| BR-0181 | --fail-on 未指定時は常に exit 0。error: error > 0 で exit 1。warning: warning + error > 0 で exit 1                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | EX-0003-0012-01, EX-0003-0012-02 |
| BR-0182 | --out 指定時はファイルに出力し、stdout には書き出さない。ディレクトリは自動作成する                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | EX-0003-0005-02                  |
| BR-0183 | summary は ok, info, warning, error のカウントを含む                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | EX-0003-0001-04                  |
| BR-0184 | `qfai doctor --profile prototyping` の probe 順序は (1) `node_modules/.bin/playwright` (Windows では `playwright.cmd` / `playwright.bat` / `playwright.ps1` variants) → (2) `npx --no-install playwright --version` fallback。順序は Flow C 相当の doctor reference (`03_Story-Workshop.md` 系) に記載する                                                                                                                                                                                                                                                                                                                                                   | EX-0003-0006-01                  |
| BR-0185 | `playwright-cli` (`.cmd` / `.bat` variants 含む) は deprecation window 中 accepted。検出時は `D-DEPRECATED-PROBE` を severity warning (during window) で fire。本 BR の sunset SSOT は `1.10.0`。sunset 到達時に同 finding は error にエスカレートする                                                                                                                                                                                                                                                                                                                                                                                                       | EX-0003-0006-02                  |
...
| BR-0187 | `qfai doctor` の `skills.integrity` check は finding severity を既定で `warning` とする (`error` ではない)。`--fail-on error` でも skills.integrity 単独では exit 0 を維持する (advisory)。`--fail-on warning` では従来通り exit 1 になる                                                                                                                                                                                                                                                                                                                                                                                                                    | EX-0003-0007-01                  |
| BR-0188 | doctor summary 出力は findings を "errors blocking the active profile" / "warnings advisory of drift" の 2 group に明示的に分割する。`skills.integrity` は finding message wording にかかわらず必ず後者の group に属する。group ヘッダ文字列はテキスト/JSON 両 format で安定識別子として扱う                                                                                                                                                                                                                                                                                                                                                                 | EX-0003-0007-02                  |
| BR-0189 | `qfai doctor --clean` は `.qfai/review/<ts>/` の mtime が TTL を超過した pack を `.qfai/review/_archive/<ts>/` へ move する。TTL 既定値は 14 日 (DR-0264)、`qfai.config.yaml#review.staleTtlDays` で project ごとに設定可能。TTL 内 pack は残置                                                                                                                                                                                                                                                                                                                                                                                                              | EX-0003-0008-01                  |
...
| BR-0191 | `qfai doctor --autoremediate` は (a) manifest runtimeDependencies (CLI-MANIFEST / BR-0006-0016) の `npm install`、(b) `--clean` TTL-archive (BR-0006-0012)、(c) qfai.config.yaml の default-keyed 欠落フィールド書き込み の 3 種を実行する。既定で interactive `--yes` 確認必須、`--yes` flag で skip。user-authored 値は上書きしない                                                                                                                                                                                                                                                                                                                        | EX-0003-0009-01                  |
| BR-0192 | 標準 CI env vars 検出時は `--autoremediate=off` を既定とし "autoremediate disabled in CI" line を明示出力する。`--dry-run` は修復計画を preview するが install / archive / config write のいずれの副作用も発生させない                                                                                                                                                                                                                                                                                                                                                                                                                                       | EX-0003-0009-02                  |
| BR-0193 | `qfai doctor --profile <skill>` reads `<paths.skillsDir>/<skill>/manifest.json` (CLI-MANIFEST), probes `node_modules/.bin/...` / `node_modules/<name>/` for each `runtimeDependencies` entry, and reports a missing dependency with its install command. The path comes from `paths.skillsDir`, so a project that relocates its skill tree, or renames it to the singular `skill/`, keeps its manifest resolved (`.qfai/spec/03_contract/cli/qfai-doctor.md#story-tree-layout`)                                                                                                                                                                              | EX-0003-0010-01                  |
| BR-0194 | `runtimeDependencies` が空配列の manifest は probe を 1 件も emit しない (false positive 禁止)。manifest 宣言と probe 結果の drift は `R-SKILL-MANIFEST-DRIFT` (SSOT-sync Pair III) を emit する。manifest schema 著作 / 配布 lint 側は spec-0015 owned (本 spec は probe 側のみ)                                                                                                                                                                                                                                                                                                                                                                            | EX-0003-0010-02                  |
| BR-0195 | `qfai doctor` は adopter tree の `.github/workflows/` 配下の shipped workflow を install 済み package 同梱 copy と改行正規化後の内容一致で比較する (`skillsIntegrity` と同型の diff)。比較対象は `.qfai/install-provenance.json` に entry を持つ name のみ — entry を持たない name は `adopter-owned` として、shipped name と衝突していても報告しない。drift は `changed` bucket が非空のときだけで、`missing` (= `declined`) と `extra` は drift ではない。provenance record の書き込み / schema 所有は spec-0003 / REQ-0020、本 spec は読むだけ。check id は `workflows.integrity`、内容一致なら severity `ok`。根拠と却下案は DR-0006-0005                | EX-0003-0011-01, EX-0003-0011-04 |
| BR-0196 | `workflows.integrity` finding の既定 severity は `info`。`info` は `--fail-on` のどの値でも exit code を変えない唯一の severity であり、本 finding は `skills.integrity` / BR-0006-0010 の「`--fail-on error` 前提の advisory」より強いこの不変条件を要求する。表示は "warnings advisory of drift" group (BR-0006-0011)。本 drift は doctor の diagnostic surface のみに出し、`qfai validate` には finding を追加しない。契約 SSOT は `.qfai/spec/03_contract/cli/qfai-doctor.md`、severity 選択の根拠・却下案・`validate` に置かない理由は DR-0006-0004。識別的な falsification leg は BR-0006-0021                                                         | EX-0003-0011-02, EX-0003-0011-09 |
| BR-0197 | finding message body は stale file path と手動 repair (「install 済み package 内の copy で当該ファイルを置き換える」) を名指しする。refresh command / CLI verb / flag は 1 つも名指ししない — refresh half は OQ-0021 (deferred) に blocked であり、command 名指しは refresh を ship する release で初めて行う。adopter tree に当該 file が無い場合は drift finding を emit しない (不在の declined / missing 分類は spec-0003 / REQ-0020 の ownership contract 側)。package 同梱 copy を解決できない場合は severity `info` で skip する                                                                                                                     | EX-0003-0011-03                  |
| BR-0198 | `workflows.integrity` の drift finding は `summary.warning` を加算しない。したがって本 finding 以外に warning / error が無い tree では `qfai doctor --fail-on warning` の exit code は 0 のままでなければならない。`shouldFailDoctor` は `--fail-on warning` で `warning + error > 0` を返すため、severity が `warning` ならここで 1 になる — BR-0006-0019 の `info` 選択を識別できる唯一の leg である (`--fail-on error` leg は `error > 0` しか見ないので両 severity を区別しない)。同時に、同じ tree へ本 finding と無関係な warning を 1 件足すと exit 1 になることも要求する (oracle が vacuous でないことの対照)                                       | EX-0003-0011-05, EX-0003-0011-06 |
| BR-0199 | drift finding の `details` は `{ workflowsDir, modified, declined, packagedDir }` を運ぶ。`declined` (provenance entry を持つが adopter tree に存在しない name) は severity にも exit code にも一切寄与しないが、operator が「QFAI は不在を認識した上で意図的に放置している」と読めるよう `details.declined` に列挙する。message body は declined file を stale として名指ししない。`declined` は finding のトリガではないため、`modified` が 0 件なら finding 自体が emit されず `details` も現れない。state 語彙と `declined` の定義は `.qfai/spec/03_contract/cli/shipped-workflows.md` §3 / spec-0003 REQ-0020 owned — 本 spec は読んで surface するだけ | EX-0003-0011-07, EX-0003-0011-08 |
| BR-0525 | Doctor resolves configured paths from `qfai.config.yaml`. An absent configured `paths.testsDir` produces a `paths.testsDir` warning with `details.path` naming the resolved path and guidance to configure or create it. A configured or present deprecated `paths.promptsDir` produces a `paths.promptsDirDeprecated` warning with the legacy path and migration guidance to `skillsDir`; a missing default path that init never created is not drift.                                                                                                                                                                                                      | EX-0003-0003-01, EX-0003-0004-01 |
