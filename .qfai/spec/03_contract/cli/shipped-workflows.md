...
| ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------- |
| BR-0060 | Every job of a shipped workflow declares a job-reachable `permissions:` block with the least scope it needs (the orchestrator's verdict job has an empty permission map) and a `timeout-minutes`. Every shipped file declares a ref-scoped `concurrency:` group with `cancel-in-progress: true`. An artifact upload, where one exists, is skipped on cancellation, tolerates a missing file and keeps retention at seven days or less. This is the shipped form of one property of NFR-C0007 (bounded and least-privileged); CLI-WFSET §5 dimension 3 is its operand on the gate side                                                                              | EX-0002-0001-01                  |
...
| BR-0078 | The write set comes from the in-binary `SHIPPED_WORKFLOW_NAMES`, and the prune set from the in-binary `RETIRED_WORKFLOW_NAMES`: the names an earlier version shipped and the current one does not. A name leaves the first list and enters the second in the same change, so the two share no name, and a name in neither is not QFAI's. Computing either set by globbing `qfai-*` on the adopter's disk is forbidden. The workflows directory is not a generated wrapper directory whose contents QFAI owns; it is an adopter-authored directory into which QFAI writes a few named files                                                                         | EX-0002-0007-01                  |
