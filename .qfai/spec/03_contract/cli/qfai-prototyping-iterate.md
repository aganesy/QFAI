...
| BR-0272 | Convergence is AND across spec × screen - Global convergence is the AND, across every `(spec, screen)` pair in the cycle-0 frozen spec set, of: `blockingFindings[] empty AND layoutAntiPatternsDetected[] empty AND designMdViolations[] empty`. - Quantitative AC-pass% / transition-pass% thresholds are explicitly NOT consulted (user direction 2026-05-18: design and operability are qualitative-only). - On hard-fail at cycle 9, the aggregator record MUST name every lagging spec ID. - On the story tree the AND runs across every `(UI contract, screen)` pair in the cycle-0 frozen `uiContractsCovered[]`, with the same three empty-array conditions, and the record MUST name every lagging UI contract ID.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | EX-0001-0120-03, EX-0001-0120-04, EX-0001-0116-01                                                                                                                                                                                            |
...
| BR-0274 | Autonomy and deterministic hard-stop catalog - The run MUST be fully autonomous from cycle 0 through cycle 9 with no per-cycle user prompts (NFR-0005). - Hard-stop classes (deterministic non-zero exits, no prompts): - (a) lock drift (exit 2; BR-0012-0026 governs `DESIGN.md` hash mismatch case) - (b) Reviewer Playwright-session failure across all reviewers for a spec × screen (exit 64 with `sessionStatus ∈ {retryExhausted, launchFailed}` recorded on the per-`(spec, screen)` review payload so the orchestrator can disambiguate from converged-exit-64; diagnostic names the `(spec, screen)`) - (c) license-verify failure (exit 66) - (d) mid-run spec-set change detection (exit 2; same class as lock drift — diagnostic names the new spec; deferred to next invocation per BR-0012-0038) - On the story tree (`.qfai/contracts/cli/qfai-prototyping.md#story-tree-layout`, "Records written before the change"): - (b) records `sessionStatus` on the per-`(UI contract, screen)` review payload, and the diagnostic names the `(UI contract, screen)`. - (d) is any added or removed UI-bearing UI contract, and the diagnostic names it. - A `prototyping.json` that carries `specsCovered` or `frozenSpecsCovered`, or lacks `uiContractsCovered`, MUST exit 2 on `iterate` at cycle ≥ 1, on `certify` and on `show-ui-contract`, with a message saying to re-seed with `qfai prototyping iterate --cycle 0`. | EX-0001-0122-01, EX-0001-0115-02, EX-0001-0122-05, EX-0001-0127-04, EX-0001-0122-06                                                                                                                                                          |
