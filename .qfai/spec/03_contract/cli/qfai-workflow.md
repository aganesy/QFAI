...
`delivery-workflow.md` states for a stage invoked by name.
...
| `sdd_append`                                                             | The `03_Example.md` of the one story the diagnosis's first matched ID names; the one contract whose rule cites that criterion's examples, derived at issue; `decisions.md`; `.qfai/evidence/sdd-<BF>.md`                   |
| `sdd`, `sdd_delta`                                                       | `decisions.md`; `open-questions.md`; `.qfai/evidence/sdd-<BF>.md`. For a slot that creates a flow, the two tables only: the flow's ID is not known at issue. Story and contract files come from the proposal's write scope |
| `prototype`                                                              | None                                                                                                                                                                                                                       |
...
- For `sdd_append` this refuses a change to a criterion or a rule statement
  with `write-scope`, which is how "the criterion and the rule do not change"
  is enforced.
...
| `debts`                                                         | `{ findingCode, path, cause, owningFlow, detectingCommand, resolvingOwner, blockingExtent }` each. `resolvingOwner` is a skill a plan names, or `operator`                                                                                                                                      |
...
| `test-fix-meaning`         | `citedAfter` differs from `citedBefore`. The fix belongs to `qfai-sdd`, and the run is unchanged. The `test_fix` stage reaches story authoring by returning `needs_repair` with a finding whose `resolvingOwner` is `qfai-sdd`                    |
...
| `blocked-repairable`       | A `blocked` result lists a finding the run can repair: one inside the checked write scope whose `resolvingOwner` is a skill, or one whose `owningFlow` names no flow, or claims another flow while its `path` lies inside the checked write scope |
...
exists, and one naming another flow must have its `path` outside the checked
write scope, or the result is refused the same way. How the core names the
blocker is in [State machine](#state-machine).
...
- A `Change request:` row names every story-tree and contract file the stage
  ...
  change authorized by the drift gate. The stage's first attempt changes no such
  file: it returns `awaiting_input` with one `decision` question showing the
  change target and the proposal. The attempt that holds the answer makes the
  change and appends the row citing that `human_decision`, at WIP. The
  `request_scope` authorization never raises it.
...
- `qfai-run/SKILL.md` stays within 150 lines. Every shipped asset the workflow
  adds or changes stays within 800 lines and 400 characters per line.
...
| BR-0570 | A `regression` diagnosis on an example a test annotates selects `regression_fix` under `regression_found`, then a full verify, and the example stays annotated.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | EX-0001-0193-03                                                                                                                                                                                                                                                                                                  |
...
| BR-0581 | A `test_fix` result whose cited criterion or rule changed is refused `test-fix-meaning`, naming `qfai-sdd` as the owner of the fix, and the run is unchanged. The `test_fix` stage reaches story authoring by returning `needs_repair` with that finding owned by `qfai-sdd`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | EX-0001-0194-04                                                                                                                                                                                                                                                                                                  |
...
| BR-0613 | Reaching a replan or automatic-repair budget leaves the run `blocked`, never counts as a pass, and the strictest applicable budget applies.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | EX-0001-0196-21                                                                                                                                                                                                                                                                                                  |
...
| BR-0622 | A semantic effect found during a `direct` change reclassifies the run before the edit.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | EX-0001-0198-04                                                                                                                                                                                                                                                                                                  |
...
| BR-0648 | Every shipped asset the workflow adds or changes stays within 800 lines and 400 characters per line.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | EX-0001-0201-14                                                                                                                                                                                                                                                                                                  |
...
| BR-0665 | `accept` refuses, with reason `record-unauthorized`, a story-authoring result that changed a story-tree or contract file unless it appended one `Change request:` row naming every such file, whose Approach cites a `human_decision` this run recorded in answer to that stage's change question. A result with outcome `awaiting_input` that changed such a file is refused the same way, and a `request_scope` authorization never stands in for the answer.                                                                                                                                                                                                                                                                                                             | EX-0001-0195-13, EX-0001-0195-14                                                                                                                                                                                                                                                                                 |
| BR-0666 | A `missing-test` diagnosis whose first matched ID is an example that states the case selects no story-authoring stage: the plan's acceptance or implement stage writes that example's test.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | EX-0001-0193-11                                                                                                                                                                                                                                                                                                  |
...
| BR-0669 | A stage skill handed a work order checks its run, stage-instance and work-order IDs against those the run issued. On a match it does only that work order and says nothing to the operator; on no match it edits nothing and returns the refusal to the harness.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | EX-0001-0202-03, EX-0001-0202-04                                                                                                                                                                                                                                                                                 |
