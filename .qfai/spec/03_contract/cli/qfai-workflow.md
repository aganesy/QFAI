...
| `sdd_append`                                                             | The `03_Example.md` of the one story the diagnosis's first matched ID names; the one contract whose rule cites that criterion's examples, derived at issue (pending OQ-0193 when rules in several contracts cite them); `decisions.md`; `.qfai/evidence/sdd-<BF>.md`                                    |
| `sdd`, `sdd_delta`                                                       | `decisions.md`; `open-questions.md`; `.qfai/evidence/sdd-<BF>.md`. For a slot that creates a flow, the two tables only: the flow's ID is not known at issue, and the proposal's write scope names `.qfai/evidence/sdd-BF-*.md` (OQ-0192). Story and contract files come from the proposal's write scope |
| `prototype`                                                              | None                                                                                                                                                                                                                                                                                                    |
...
- A `Change request:` row names every story-tree and contract file the stage
  ...
  change and appends the row already at WIP, citing that answer. A row citing
  only `request_scope` is refused `record-unauthorized`.
- An approval-free row, such as the UPDATE:APPEND a seeded example records,
  cites no answer.

The story-authoring stage that appended a row moves it to DONE once every
change the row names is written, in that attempt or a later attempt of the
same stage. The core never writes a Status. A row a run appends carries no
options or recommendation of its own, as BR-0005 asks of a change request
raised outside a run: they are the ones the stage's question showed, and the
authorization record the row cites holds them
