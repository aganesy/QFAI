...
As an operator, I want `primarySpecId` to surface a single deterministic error message naming the canonical 4-digit shape AND (SHOULD) to accept `1` / `"1"` / `"01"` / `"0001"` with internal normalisation, so that the input-validation surface is unambiguous. On the story tree the pin is `primaryUiContract`, which accepts only the full `CON-UI-NNNN` form and normalises nothing, so the error names that one shape. (REQ-0012-0069)
...
- Story block: `.qfai/evidence/migration-spec-to-story/retired/spec-0012/02_User-stories.md#us-0012-0133`
