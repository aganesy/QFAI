...
- Parent: CAP-0017
- Source: discussion-20260923063306456#REQ-0024, discussion-20260923063306456#NFR-0004
- Goal: As a QFAI maintainer I want the shape table in `.agents/rules/distributed-surface.local.md`
  to list every internal ID shape the three distributed-surface guards reject, with the sample
  band a shipped template may use, and to change in the same pull request as the guards — so
  that the rule a contributor reads and the patterns the guards enforce never disagree
- Non-goals: choosing the guards' patterns, which belong to the specs that own the guards;
  editing a shipped template in the same pull request; dropping a shape the table already lists
- Notes: covers spec-local REQ-0017. NFR-0005 is the obligation this story applies to the new
  shapes.
...
- Story block: `.qfai/evidence/migration-spec-to-story/retired/spec-0017/02_User-stories.md#us-0017-0011`
