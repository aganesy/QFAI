# US-0002-0021: Layer-to-CI-lane mapping in the layer rule

## User Story

- Parent: CAP-0017
- Source: discussion-20260804173914356#DSC-016
- Goal: As a contributor deciding which CI lane a test belongs to, I want the mapping in the
  shipped `rule/test-layers.md` so I can find it beside the layer taxonomy. The section adds no
  layer token or heading and does not activate per-level routing.
- Non-goals: a separate mapping file; extending the built-in token set to legalize CI vocabulary;
  adding a layer, layer token or layer heading; placing annotations outside the validator's test
  scan; authoring the repository-root link directly.
- Notes: covers spec-local REQ-0014, whose upstream row is `both`-surface; this spec owns the
  mapping. No `DUS-` story covers it — provenance is the success criterion `DSC-016` and the
  resolution of `OQ-0010`. The mapping is a section of
  `packages/qfai/assets/init/.qfai/assistant/rule/test-layers.md`. The repository-root rule is a
  link to that asset, and the former sibling file is retired (OQ-0177). The layer-policy loader
  reads the rule, so the section states its vocabulary and routing boundary explicitly.
...
- The layer-to-CI-lane mapping section in
  `packages/qfai/assets/init/.qfai/assistant/rule/test-layers.md`, linked into the repository-root
  assistant tree. The former sibling mapping file is absent.
