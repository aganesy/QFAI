...
**An `exception` item is outside this requirement, at either gate.** It is
reached only when neither a mutation nor an `equivalent-mutant` was available,
so an `Oracle proof` is the one thing it cannot produce. It never reaches GREEN,
and a user-approved `TDDLIST-001` waiver can carry it to the spec-level
completion gate still holding no proof — do not REVISE it there for the absence.
Judge it by "Branch 3 gets its own verdict" below, on its `DR-*`.
