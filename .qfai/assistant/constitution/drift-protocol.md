...
- `.qfai/specs/<spec-id>/tdd/test-list.md` — the `Status`, `DR-ID`, `Evidence`
  and `Blocked-By` cells unconditionally, append/update by `/qfai-implement`,
  plus two cells that are writable **only** while a stated condition holds:
  ...
  `Blocked-By` is unconditional alongside the first three because
  `todo -> blocked` is an edge that skill owns and
  ...
  fills a cell rather than adding a column.
...
satisfying it was in drift. The entry above names the file and the four cells
...
micro-cycle authors it. So the writing stage cannot hold the status the
unconditional cells **do** authorise without also writing a cell they do
**not** — and
