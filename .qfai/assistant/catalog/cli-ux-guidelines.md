...
移行中の日本語メッセージは一覧に記録され、その一覧は**縮むだけ**である。
翻訳したら同じ変更でその項目を削る。項目を増やすのは、開いている branch が
base の新しいメッセージを取り込む merge のときだけで、その場合も件数の pin を
同じ変更で動かす — 増えたことがレビューで見えるようにするため。
