...
TCs declaring `Level` `L1`/`Unit` or `L2`/`Component` are **skipped**, and the
command names them on stderr. Their skeleton would land in
`tests/integration/**`, which is the duplication the Coverage obligations
section forbids, and `QFAI-ATDD-112` would not count it either — so filling one
in would discharge nothing. Their ledger row under `/qfai-implement` is where
they are owed.
