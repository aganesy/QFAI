...
- Coverage obligations are mandatory:
  - `tests/e2e/**` must cover all required `US-*`. A story outside the current
    ...
    meta line in its own `## US-XXXX` block — the same token both contract kinds
    use — and is named at `info` by `QFAI-ATDD-118`. It is not left uncovered,
    and it is not covered by a test that asserts nothing. `exception` is not the
    alternative here: that branch belongs to a ledger row, and a `US-*` owns
    none (`references/red-provenance.md#a-spec-with-no-atdd-owned-rows`).
...
- All required `US` are covered by E2E tests (`QFAI-ATDD-111`); a story outside
  the current slice is deferred with `- x-qfai-status: planned` in its
  `02_User-stories.md` block and reported by `QFAI-ATDD-118` (`info`), not left
  uncovered.
