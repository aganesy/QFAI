...
| A no-question mode is active        | Do not invoke, and do not continue a session already open     |
| An ambiguity met while implementing | Not a session; an ordinary clarification under its own budget |

**A no-question mode is decisive.** An invocation told not to ask — `--auto`, or
whatever the host spells it as — proceeds on labelled assumptions, or stops and
names what it cannot assume, exactly as it would without this skill. Declaring a
session is not a way to ask.
...
- A no-question mode neither enters a session nor continues one: such a run proceeds on labelled assumptions, or stops and names what it cannot assume.
