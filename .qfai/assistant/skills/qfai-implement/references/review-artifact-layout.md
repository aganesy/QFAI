...
- Minimum `summary.json` shape (`version: "2.0"`):
  ...
  **`revision_form: "content-hash"`** and **`revision`** — the state these verdicts describe, as a
  ...
  `evidence-revision.md`. Both are required: a pack that declares the contract and names no tree
  cannot be re-checked, and one that declares neither is judged as if it predated the form.
