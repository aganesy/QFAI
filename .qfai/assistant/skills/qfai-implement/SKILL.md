...
9. UI-affecting items have prototype parity PASS from `product-surface-reviewer`. Whether an item is UI-affecting is not the implementer's judgement call — it is decided by `references/ui-affecting.md`, and the clause that decided it is recorded either way
...
- `Prototype parity` — product-surface-reviewer result for UI-affecting items (PASS or REVISE), or `n/a (not UI-affecting)` when no clause of `references/ui-affecting.md` selects the row. Never blank: a blank cell cannot tell the two apart
...
- UI-affecting items require product-surface-reviewer prototype-parity PASS before the item can transition to done; `UI-affecting` is decided mechanically by `references/ui-affecting.md`, and a row that no clause selects records `Prototype parity: n/a` rather than nothing.
