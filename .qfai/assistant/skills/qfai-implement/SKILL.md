...
When no explicit argument is given, detect the candidate specs — **active specs only**. A spec is retired only by a **complete** declaration in its `01_Spec.md` header block: a top-level `Status: superseded` whose `Superseded-by:` names a spec that exists and itself declares `Status: active`, or `Status: deprecated` / `Status: removed` with a `Deprecated-at:` that is a real calendar date. That is the same resolution `validate` performs, and the two must agree in both directions. An **incomplete** declaration retires nothing — `Status: superseded` with no `Superseded-by`, a successor that is itself retired, `Deprecated-at: 2026-02-30` — so its ledger still gates at full severity: keep working it, and report the incomplete declaration instead of sending the user to a successor that does not exist. For a spec that is retired: `/qfai-sdd` migrated its live rows to the inheritor before rewriting that bullet, `validate` demotes every remaining finding on its ledger to `info`, and the rows left behind are the historical record of what it delivered. Never let one supply a candidate row — an unfiltered scan picks the retired ledger's first `todo` / `review-fix` and re-implements a requirement nobody owes. Execution is constrained to one spec at a time. Auto-discovery MAY present several specs as a queue to be processed sequentially (see `references/volume-policy.md#multi-spec-queue`); this protocol does NOT enable multi-spec parallel execution.
...
- Owning the repository-wide quality gate and its report artifacts (use `/qfai-verify`). The scoped `npx qfai validate --profile tdd --fail-on error --spec <spec-id>` is **not** a non-goal: checkpoint verification requires it at every boundary (gate item 12, `references/checkpoint-verification.md`), and `## Completion Message & Next Actions (MUST)` routes to `/qfai-verify` for full-scan approval only after it has run.
...
1. Read `test-list.md` and select the next row to work by the precedence below. Each iteration works **exactly one** row; **branches 1 and 2 are the exceptions in extent, not in rank** — both are invoked with a set of rows rather than a ledger scan, and each works them all, one per iteration, in the order given, before the ledger is re-read. Branch 2 is named by `TDD-ID`; branch 1 carries its inputs **per affected row**, because one shared artifact is read by every completed row that uses it. Stopping either after the first row is what the "exactly one" rule must not be read to allow: on a branch-1 request the unworked rows get no `Shared-artifact re-verify` line, so their stale `RED test hash` is never cleared and the requesting stage still cannot complete. **The first branch that matches wins, and nothing under it applies.**
   1. **A mutation-only request wins over all three below, and moves no row.** `/qfai-atdd` sends one when a stage that owns no ledger row has edited a shared test artifact that one or more completed rows read (`../qfai-atdd/references/shared-test-artifacts.md`); work **every** row it names. Its inputs, its execution sequence and its no-write rule are in `references/mutation-only-request.md`: **write nothing to `test-list.md` and nothing to that row's evidence**. This is the only entry that takes a row already at a terminal status, and it takes it read-only — every branch below selects `todo`, a named row or `review-fix`, none of which a `done` row can be, so without this the request had no receiver and the requesting stage could never complete.
   2. **A named row wins** over the two below: when the invocation names one or more `TDD-ID`s — `/qfai-atdd` stage gate P1c hands over by id — process exactly those, in the order given, and select nothing else. Row order in the ledger is not a priority: a branch-2 (`falsifiability`) row sitting above the named one would otherwise be picked up first, and its checkpoint runs the full suite against a tree that still holds the named row's deliberate RED, so it fails and lands at `refactor`, which this step does not re-select.
   3. Otherwise, **rework first**: if any row is at `review-fix`, select the first such row and resume its rework (`references/round-evidence.md`) before any `todo` row — one left by an interrupted session is otherwise never picked up.
   4. Otherwise select the first row with `Status = todo`, skipping any `blocked` row — it cannot be started, and re-issuing it is what made the determination get re-derived every pass (`references/execution-ledger.md#blocked-rows`).
...
1. Write the **minimum production code** to make the failing test pass. **When it goes into a file that already exists, run the cross-spec check of `references/cross-spec-ownership.md` before writing it** — the ordinary implementation edits a shared production file here and never touches it again in Refactor, so the copy of the check in Refactor step 1 would read a file this step had already changed. On the _RED not observable_ path there is none to write — the `Satisfied-by` row already implements the predicate — so go straight to step 2.
...
1. Improve code quality (naming, structure, duplication removal) while keeping all tests green. **Before editing a production file or a test file — here or in any other phase**, run the cross-spec check of `references/cross-spec-ownership.md` against every other spec's `tdd/test-list.md`, over its `Status = done` rows only: the file is named in that row's `Owning module` — the only column that holds a production path, matched on the normalized form so a dotted module path counts — or, when the file is itself a test, in its `Test file`; and every `done` row that did not match directly is still checked through the reverse dependency closure, so a row that declares an unrelated module does not pass silently either. **A match either way makes the edit cross-spec** — named directly in one of those two columns, or reached only through that closure, which names it in neither — so record it in this item's evidence and re-run `completion-reviewer` against that spec's obligations too (`references/cross-spec-ownership.md`).
...
4. Submit for completion review (`completion-reviewer`) and code quality review
   ...
   `refactor` until the group closes (`references/volume-policy.md#group-formation-states-and-transitions`), which re-verifies every member on the closed tree and refreshes its `Refactor verify` fields before these two reviews are requested.
...
2. Implementation agent submits the RED run to `qa-gatekeeper` **while no implementation makes that assertion pass — neither the Phase Red step 3a seam nor a surface that already exists and implements the row's predicate wrongly does** (routing phase `red`), then the GREEN run after it (routing phase `build`). Read as "while no production code exists" it contradicted step 3a, which puts the seam in the production tree before the RED is taken: a new-surface row could then either never submit its RED or had to skip the seam and submit a load error, which `references/red-admissibility.md` rejects. Read as "before any code implementing the row's predicate exists" it still shut out the existing-but-wrong surface `../qfai-atdd/references/red-provenance.md` routes to its branch 1 and requires this gate to PASS. One combined post-hoc submission is not a substitute: RED is unrecoverable once Green begins, so it leaves nothing but the implementer's own account of a destroyed state — the self-attestation this gate exists to prevent. It **returns it to the orchestrator in the per-item evidence contract's form** so it can be written to the ledger. The implementation agent never writes `test-list.md` itself.
...
- `parallel_groups` in `agent-routing.yml` describes **role fan-out within a phase**, not item dispatch — whatever value it holds. The shipped `build` phase may fan out `frontend-engineer` and `backend-engineer` over **one** ledger row, and only when both roles apply to that row and the planner selects both: the row keeps one `Owning module`, hands each role a non-overlapping write range inside it — a row that cannot be split that way runs its roles one at a time, and work that does not fit the module needs an existing second row or a Change Request to `/qfai-sdd`, never a second writer here — clears the external-runtime-resource condition over the two roles as it would over two items, returns one per-item evidence block, is judged on one GREEN observation covering both roles' output — **taken after the merge by the role that owns the row's `Selector`**, because neither worktree holds the whole behaviour and neither the orchestrator nor `qa-gatekeeper` may supply an observation nobody made — and has each role's head diffed against its assigned range at seam reconciliation, **over a scoped commit whose worktree `git status --porcelain` comes back empty**, since a two-commit range diff cannot see uncommitted or untracked role edits (`references/parallelization-policy.md#role-fan-out-inside-one-row-build-phase`).

### Post-parallel integration verify

- **Reconcile the ledger first.** Under worktree separation each worker holds a private copy of `test-list.md`, so the merged trunk carries none of their transitions. Write Status + Evidence for every merged item from the worker reports **before** integration verify, and fail the verify if any merged item's row is still `todo` — an unreconciled ledger reports finished work as unstarted. **That write replays the row's own path one listed edge at a time** (`todo -> red -> green -> refactor`), because the trunk row is still `todo` and assigning the returned status directly would be the unlisted `todo -> refactor` / `todo -> done` — the successful run's own write. **A worker's returned `done` is written as `refactor`**, the replay stopping one edge short, and becomes `done` only after the re-take below passes: this write precedes both gates, and `done -> refactor` / `done -> review-fix` are not listed edges, so a `done` written here could not be walked back when the re-verify fails (`references/parallelization-policy.md#ledger-ownership`).
- After parallel slices complete and merge, run integration verify on the merged result — then, before any merged item goes `done`, re-take each item's item 6 on the integrated tree and re-request its items 7-8 reviews there — **plus item 9's `product-surface-reviewer` review on a UI-affecting item** (prototype parity, or on a cli-only target the captured-output surface review item 9 puts in its place), whose reviewed output a merged sibling can change without moving this item's suite — because a worker took them all inside its own worktree and item 10 wants them equal on the tree the item ships on (`references/parallelization-policy.md#re-verify-each-merged-item-on-the-integrated-tree`)
- Then reconcile the seams: diff each slice's touched paths under the production roots `catalog/structure.md` declares against its declared `Owning module` and report undeclared or overlapping paths as a deny-condition breach — **independently of whether the merged suite is green** (`references/parallelization-policy.md#seam-reconciliation-after-a-parallel-run`)
...
Follow `.qfai/assistant/constitution/shared-skill-operating-baseline.md#completion-contract-shared`. **Smallest applicable smoke check** (this skill's override): the Test command from `catalog/tech.md#standard-commands-copy-paste`, run over the items this session moved to `done` — a suite that did not execute is UNRUN, not a pass. A run that moves no item still has a target: **every exit that leaves the ledger terminal without a `done` transition of its own** takes the spec-level checkpoint verification that terminal boundary already owes as this bullet's outcome (`references/checkpoint-verification.md#spec-level-boundary-on-an-already-complete-ledger`). That is not only the "nothing to do" / "All items complete" exits of a ledger that was already terminal on entry — a session that terminalises the ledger itself by writing its remaining rows `todo` -> `exception` under user-approved accepted-risk waivers (Completion step 3) also moved nothing to `done`, and the last row reaching `exception` ends the loop exactly as a `done` does, so the boundary is owed there too and the suite has no `done` set to run over. Reading the branch as "already terminal on entry" made that exit an unconditional UNRUN and blocked a completion the skill otherwise permits. In all of them, run the checkpoint's command set when the recorded entries are absent, when they predate the last ledger change, **or when the `Revision` recorded beside them is not this tree's**: a terminal ledger stops moving while the code under it does not, so a PASS recorded before a later source or dependency edit is stale however still the ledger is, and recomputing the seal catches an edited record, never code that moved underneath an honest one. Only entries fresh by all three become this bullet's outcome, with the `Checkpoint verification seal` recomputed here; anything else re-runs the command set against the current tree. A legitimate no-op is never UNRUN for want of a target.
...
- Checkpoint verification passed at the spec-level boundary (see `#checkpoint-verification`), and its `Checkpoint verification seal` is **recomputed** here over the recorded command, result, **any `## Shared-artifact re-verify` block that boundary wrote** and revision — the same four inputs `references/checkpoint-verification.md` seals it over, because a recomputation over three of them can never match a seal taken over four and every spec that used the per-spec repair path would be uncompletable by construction. Where no repair ran the boundary wrote no block, and the subject is the other three. That boundary has no row, so gate item 12 never runs for it — without this recomputation the full-suite result on a terminal ledger could be edited from FAIL to PASS afterwards with no revision, no `Audited evidence hash` and no pack seal moving
...
- A `## Cross-spec obligations` entry of the **code-ownership** kind in this spec's evidence file is still open — this run changed a file another spec's ledger names in `Test file` and the change it names has not landed, or the blocked spec's obligation is still untested. A clean completion here would certify an obligation this run knowingly left unmet (`references/cross-spec-ownership.md`). **Not the contract-residue kind.** An `E2E` / `API` / `Integration` row keeps its evidence in `.qfai/evidence/atdd-<spec-id>.md` (item 10), so a `/qfai-atdd` run that ended `PASS with cross-spec obligations` leaves rows in this very section for a **sibling** spec's uncovered `CON-API` / `CON-DB`: recorded, attributed to a named owning spec, closed by that spec's run, and non-blocking by construction (`../qfai-atdd/references/cross-spec-obligations.md`). Reading those as this condition re-blocks what that stage already discharged and moves the deadlock here instead of ending it. Tell the kinds apart by the row's own fields — a code-ownership row names a `Test file` and a blocked `TDD-ID`, a contract-residue row names a `Contract ID` and an `Owning spec` — and apply the same floor as the stage that wrote it: a row naming no owning spec, naming **this** spec as the owner, or omitting the contract ID is not attributed residue and is still this condition
- Items with `todo`, `red`, `green`, `refactor`, or `review-fix` status still exist (for spec-level completion)
- Items with `exception` status still exist, **unless** the row's `DR-ID` names a Decision Record explicitly recorded as a **user-approved accepted-risk waiver** (a `TDDLIST-001` entry in `.qfai/waivers.yml`). An `exception` whose DR only describes the anomaly is a parked defect, not a completed item.
  **A waived branch-3 `exception` is outside the RED, GREEN, reviewer-verdict and `Oracle proof` conditions above as well** — including item 5 and the per-item evidence contract. Releasing only this bullet released nothing: branch 3 is reached only when neither a mutation nor an `equivalent-mutant` was available, so it never enters Phase Green and produces none of that evidence, and the conditions above would go on blocking the very row the waiver just admitted. Judge it as `agents/qa-gatekeeper.md` does — on its `DR-*`, under "Branch 3 gets its own verdict" — and require nothing else of it here.
  **That exclusion is keyed on the `DR-*` of the _current_ exception, never on the status.** `exception` is reachable from any active status, `exception -> todo` keeps the old anomaly's `DR-*`, and a re-entry appends a new one (`references/execution-ledger.md`), so a row that reached `red`, proved its oracle and was parked later carries a stale branch-3 record beside the current one. Read the last appended `DR-*`, and exempt only when **that** one records both proof forms unavailable.
...
- `TDD-ID`, `Layer`, `Test file`, `Selector` — the row identity, copied from `test-list.md`; on a Phase 2b-seeded acceptance row those two cells are still `-` there, so the copy comes from the handoff entry's identity — the acceptance test `/qfai-atdd` authored — which Phase Red step 3b writes into the ledger in the same edit. The reviewers hash these and gate item 10 checks the copy against the ledger, so a row recorded without them has no identity record to hash and cannot pass that check (`constitution/shared-skill-delegation-baseline.md#reviewer-response-template`)
...
- `Refactor verify result` — the output confirming GREEN is maintained (likewise once per item), and `Refactor verify revision` beside it: the state Phase: Refactor step 2's re-run was observed against, taken by the same procedure as `Revision` and prefixed like its pair rather than with a `Round N:`. **That is the address items 6, 7 and 8 share**, that run being the only observation of the refactored tree taken before the reviews: the reviews are requested from `refactor` (Phase: Refactor step 4) while item 5's GREEN is observed before the refactor, so requiring the GREEN's `Revision` to match the two verdicts left this gate and item 6 jointly satisfiable only by a refactor that changed nothing (`references/evidence-revision.md`)
...
- `Checkpoint verification result` — the outcome of that command set (PASS only when every command exits 0) — plus `Checkpoint verification revision`, **this run's own address** and never a round block's `Revision` (that one addresses the pre-refactor tree, and this boundary runs after every reviewer PASS), and `Checkpoint verification seal`, the audit hash over the command, the result and that revision, taken by whoever ran it the moment the run ends. The four are appended after every reviewer has hashed, so they are in no audit subject by construction; the revision excludes `.qfai/evidence/**`, and the review pack seal covers only the pack. Without a seal of their own, a row already at `done` could have its checkpoint result edited from FAIL to PASS with no revision, no `Audited evidence hash` and no pack seal moving, and item 12 would accept it (`references/checkpoint-verification.md`)
