...
- Empty evidence entries are rejected: minimum evidence per TDD item must be met — `TDDLIST_EVIDENCE_EMPTY` (error). The cell is a **pointer**, in one legal shape capped at 240 characters — `RED:<fail|falsifiability|n-a> GREEN:pass ORACLE:<proved|equivalent-mutant> [TIER:<T1|T2|T3>] REV:<revision> -> .qfai/evidence/<implement|atdd>-<spec-id>.md#<heading>`, where `RED:n-a` is **not** available on an `E2E` / `API` / `Integration` row, the anchor's stage and `<spec-id>` are checked against the row's `Layer` and spec — an `implement-` anchor on an ATDD-owned row needs the `Pre-split-evidence: implement` marker item 10 writes on `E2E` / `API` rows only — and `REV:` takes only the two spellings `references/evidence-revision.md` defines; commands and output live in the evidence file the anchor names — `TDDLIST_EVIDENCE_CELL_MALFORMED` / `TDDLIST_EVIDENCE_CELL_OVERSIZE` (warning, waivable as `TDDLIST-007` / `TDDLIST-008`), grammar in `references/execution-ledger.md`
