...
a pass — and **step 1 is not settled by its exit code** in either form: file-scoped it can execute only the other rows sharing the `Test file`, so its output must name every `Selector` entry of the row being checkpointed; narrowed it is emitted one command per `Selector` entry, and each run passes only when its recorded output names the entry it ran — or, for a narrowed run whose runner has no option that prints names, reports a positive selected/run count instead — because a run that selected zero tests exits 0 as well. The boundary definition, command set, pass criteria and evidence
