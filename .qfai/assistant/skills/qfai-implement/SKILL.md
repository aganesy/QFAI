...
1. Read `test-list.md` and select the next row to work by the precedence below. Each iteration works **exactly one** row; branch 2 is the exception in extent, not in rank — an invocation naming several `TDD-ID`s works them all, one per iteration, in the order given, before the ledger is re-read. **The first branch that matches wins, and nothing under it applies.**
   1. **A mutation-only request wins over all three below, and moves no row.** `/qfai-atdd` sends one when a stage that owns no ledger row has edited a shared test artifact a completed row reads (`../qfai-atdd/references/shared-test-artifacts.md`). Its inputs, its execution sequence and its no-write rule are in `references/mutation-only-request.md`: **write nothing to `test-list.md` and nothing to that row's evidence**. This is the only entry that takes a row already at a terminal status, and it takes it read-only — every branch below selects `todo`, a named row or `review-fix`, none of which a `done` row can be, so without this the request had no receiver and the requesting stage could never complete.
