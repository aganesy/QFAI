...
- The item's evidence file (item 10) does not exist, or does not record **every routed reviewer's** verdict for the item — including `Prototype parity` on a UI-affecting item — or a UI-affecting item's entry carries no `Surface artifacts` manifest, or one with no path under it: the parity verdict is then hashed over fields alone, and the rendered evidence it PASSed on can be replaced with neither `Reviewed revision` (which excludes `.qfai/evidence/**`) nor `Audited evidence hash` moving, which is the one thing that verdict exists to prevent (this is the single blocking statement about the evidence file; its absence of _verdicts_ is never blocking before items 7-8, while the manifest is phase-authored and is expected to be there when the review is requested)
...
- `Revision` — the state the observation was made against: `git rev-parse HEAD`, or `working-tree+<content hash>` for an uncommitted tree. **A UI-affecting row carries `Surface artifacts` beside it** — the repo-relative path of every rendered artifact under `.qfai/evidence/**` the parity review reads (the runtime screenshot, the HTML capture of this row's own output), one per path, recorded as each is captured and therefore phase-authored: it is inside the subject the reviewer hashes, not appended after them. `Revision` excludes `.qfai/evidence/**` by construction, so this manifest is the only thing that carries the rendered output into a hash at all — with no field naming them, a fully compliant UI row named no artifact, item 9's subject was fields alone, and a screenshot replaced after the PASS moved neither of the two values item 10 checks. It names this row's captures only; the prototype artifacts the reviewer also reads sit inside the revision and contribute no record (`constitution/shared-skill-delegation-baseline.md`). **Empty is not a value here**: a parity verdict with nothing recorded under it is refused at the completion gate (`Completion prohibition conditions`), not hashed as a row that happens to have none
...
- `Prototype parity` — product-surface-reviewer result for UI-affecting items (PASS or REVISE) — on a cli-only target the field records the captured-command-output surface review item 9 substitutes for parity, and the field name does not change, so nothing downstream has to learn a second key — with its `Reviewed revision` and `Audited evidence hash`, the same two the other verdicts carry, so gate item 10 recomputes this verdict's hash as it does theirs. **What that hash is taken over is named, not inferred from the siblings**: `.qfai/assistant/constitution/shared-skill-delegation-baseline.md`'s _Completion review_ subject lists `product-surface-reviewer` beside the other two and adds the surface artifacts this row's phase-authored `Surface artifacts` manifest names under `.qfai/evidence/**` — the screenshots and HTML captures the revision excludes. One named field, read by the reviewer and by the gate: "the artifacts the entry names" with no field naming them let a compliant entry name none. A field that only looked like its siblings left the reviewer and the gate free to choose different extents, and two honest parties computing different hashes fail the row exactly as tampering would. Its `Reviewed revision` is held to the same freshness rule as items 5, 7 and 8 (item 10)
