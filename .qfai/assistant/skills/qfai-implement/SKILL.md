...
3. Write a **failing test** from the row's obligation column, selected by `Layer`: `TC-Refs` for `Unit` / `Component` / `Integration`, `US-Refs` for `E2E`, `CON-API-Refs` for `API`. An `E2E` / `API` / `Integration` row's test is authored by `/qfai-atdd` (Non-goals); this skill only drives that row's status and evidence once the acceptance test exists, and stops with a handoff note if it does not.
...
- Additional implement-specific overrides:
  ...
  - update `test-list.md` **Status and Evidence** after each phase completes, recording the delegated agent's one-word RED/GREEN outcome plus the anchor into the evidence file the row's `Layer` owns — `.qfai/evidence/implement-<spec-id>.md`, or `.qfai/evidence/atdd-<spec-id>.md` for an `E2E` / `API` / `Integration` row, whose RED was produced by the stage that authored its test (gate item 10). **A row carrying `Pre-split-evidence: implement` keeps `implement-<spec-id>.md`** (gate item 10): the marker is what makes a legacy row's implement anchor the one that item accepts, so read the marker first, then the `Layer` — the same order `references/checkpoint-verification.md` and `references/cross-spec-ownership.md` state for the entries they own, and this is the rule those two are the exception to, so branching on `Layer` alone here left the ordinary phase evidence as the one part of such a row still sent to the wrong file. Resuming a marked row from `red` / `green` / `refactor` and writing its phase evidence to the ATDD side splits the record in two: the implement entry item 10 selects is left without the GREEN, refactor and reviewer evidence the completion gate reads there, while pointing the `Evidence` cell at the ATDD anchor instead contradicts both the marker and item 10's implement selection — so the row could not reach `done` either way. Write that agent's command+result verbatim in the evidence file itself — a GFM cell cannot hold either a newline or a bare `|` (`references/execution-ledger.md#evidence-cell-contract`). Gate item 10 requires both columns, the protocol permits both, and the orchestrator is the only role permitted to write this file (`references/parallelization-policy.md#ledger-ownership`).
...
10. `test-list.md` Status is current and its Evidence cell's anchor resolves to a fresh per-item entry in the evidence file its `Layer` owns — `.qfai/evidence/implement-<spec-id>.md`, or `.qfai/evidence/atdd-<spec-id>.md` for an `E2E` / `API` / `Integration` row, whose RED provenance was produced by the stage that authored its test — `/qfai-atdd` owns all three, because `QFAI-ATDD-112` puts every `L3` and undeclared-`Level` TC in `tests/integration/**` and its P4 writes those tests (`references/execution-ledger.md#atdd-owned-rows`). **Compatibility:** an `E2E` / `API` / `Integration` row that reached `done` or `review-fix` before its `Layer` moved into the ATDD-owned set has its evidence and anchor in `implement-<spec-id>.md`, which was the contract at the time. **`Integration` moved later than `E2E` / `API`** — the Orchestrator Protocol sent it to `implement-<spec-id>.md` until this version — so the pass below runs once more, over the `Integration` rows, when this version is taken; omitting them left every pre-existing `Integration` row past `todo` with a lawfully written implement anchor this item rejects and a forward-only lifecycle that cannot re-take its RED, so it could neither resume nor be re-verified after the upgrade. **Identify it by a marker, not by its status**: the row carries `Pre-split-evidence: implement` in its `Evidence` cell. **Write it once, from the history**: for **every** `E2E` / `API` / `Integration` row past `todo` — `red` and `green` and `refactor` as much as `done` and `review-fix` — check whether its `Evidence` anchor pointed at `implement-<spec-id>.md` in the commit that last advanced it. A row interrupted mid-cycle by the upgrade has legitimately stored evidence there too, and skipping it left that row unable to finish: unmarked, it is judged by the current rule whatever its status, so the evidence it lawfully wrote was rejected at item 10. Find that commit from the row's **patch history**, not with `git log -S`: the id is on both sides of a status-only change, and `-S` matches a filepair only when one side contains the string, so it walks back to the commit that _added_ the row instead. `git log -p -- <test-list.md>` and take the newest commit whose hunk changes that `TDD-ID`'s line (`git log -L` on the row also works where the line is stable). **On an `Integration` row, read the working tree before the history, because an advance need not be committed at all.** A phase may leave `test-list.md` and its evidence uncommitted — `references/evidence-revision.md` addresses exactly that tree as `working-tree+<content hash>` — and then no commit carries the advance: the patch-history walk finds the row's pre-advance line, or nothing where the ledger is untracked, and the marker is never appended, so an `Integration` row interrupted mid-`red` / `green` / `refactor` by the upgrade is judged by the new rule against the implement anchor it lawfully wrote. So, **for `Integration` only**: compare the row's line with `HEAD` first (`git diff -- <test-list.md>`, and treat an untracked `test-list.md` as wholly uncommitted). Where it differs, the working-tree line **is** the advance — read its anchor from there. Only where the line matches `HEAD` does the patch history decide. **`E2E` and `API` are decided by the history alone.** Their `Layer` moved into the ATDD-owned set in an earlier version, so on a tree that took that version an advance still uncommitted when this one is taken was written under a protocol that already sent them to `atdd-<spec-id>.md`: an implement anchor in the working tree is a row written to the wrong file _now_, not a legacy one. The working-tree read cannot tell those apart — it dates the anchor not at all, which is the very thing the paragraph below says status and anchor alone cannot do — so extending it to those two layers unconditionally would mark exactly the row that must be refused, one that never produced its ATDD handoff, and pass it as complete. **Except on an upgrade that skipped the version which moved them.** An upgrade need not be taken one release at a time: a project may go from a release before the `E2E` / `API` split straight to this one, and a row may have sat uncommitted across the whole jump — its implement anchor is then as lawful as any `Integration` one, and there is no commit anywhere in the history that carries the advance, so "their genuine pre-split advances are therefore committed" holds only for an upgrade that passed through the intervening version and is false here. **So date the protocol rather than the anchor**, which is the one thing about an uncommitted advance the tree still records: `git show HEAD:.qfai/assistant/skills/qfai-implement/SKILL.md` is the Orchestrator Protocol that advance was written under — the copy this upgrade replaces. Where that copy sends an `E2E` / `API` row's phase evidence to `implement-<spec-id>.md`, the split had not been taken on this tree, so the working-tree read applies to those two layers exactly as it does to `Integration` and a lawful implement anchor found there is marked. Where it already names `atdd-<spec-id>.md` for them, the split had been taken, and an uncommitted implement anchor is refused as above. Where `HEAD` carries no such copy at all — an untracked or absent `.qfai/` — nothing establishes a pre-split protocol, so the refusal stands: that is the reportable direction rather than the accepting one. `Integration`, whose split is this version, always has the working-tree read, because the copy at `HEAD` necessarily still sends it to the implement file; `E2E` and `API` have it only where that copy says the same of them. If the anchor so found named the implement file, append the marker. A row advanced after the split has an ATDD anchor there and gets none. Run it as part of taking this version; until it has run, those rows are judged by the current rule, which is the safe direction: they are reported, not silently accepted. Status and anchor alone cannot tell a legacy row from a new `E2E` / `API` / `Integration` row written to the wrong file — which would let a row that never produced its ATDD handoff be accepted as complete. **A row with no marker is judged by the current rule whatever its status**, which for an `E2E` / `API` / `Integration` row means the ATDD file: status and anchor alone cannot tell a legacy row from one written to the wrong file after the split, and accepting the implement anchor without the marker let a row that never produced an ATDD handoff pass the gate as complete. **A row that carries the marker** is the legacy case, and its implement anchor is accepted — it has no ATDD entry to produce, and a `done` row has no legal transition that would let it re-observe a RED, so requiring the new location would make an already-complete row permanently ungateable. Until the marker pass has run, unmarked legacy rows are reported rather than accepted; that is the safe direction, and running it is what clears them. A row advanced after the split writes to the file its `Layer` owns. The cell is a pointer, not the payload (`references/execution-ledger.md#evidence-cell-contract`). `Review pack seal` is recomputed here from the `review-<timestamp>/` directory it names, and a mismatch means the pack was edited after the round closed. Each reviewer verdict's `Audited evidence hash` is **recomputed** here over the entry's phase-authored fields: the revision excludes `.qfai/evidence/**`, so this is the only thing that tells a verdict passed on the evidence as read from one passed on evidence edited afterwards. A verdict carrying a `Record re-attestation` is compared against **that** hash and not the superseded original — a record repair moved the bytes the original read, by design — and the re-attestation's `Record re-attestation pack seal` is recomputed here beside the round's `Review pack seal`, each from the pack it names. The re-attestation is written as a pack of its own for exactly this reason: neither seal is ever edited, so a repaired record stays checkable rather than becoming an untraceable rewrite of a sealed pack (`.qfai/assistant/constitution/drift-protocol.md#the-record-defect-queue`). The item's four sub-agent observations (items 3, 5, 7, 8) all name the **same** revision (`references/evidence-revision.md`) — **except item 3**, which cannot be taken against the final tree on any row: a RED precedes the code that makes it pass, and a `falsifiability` row's mutation run is taken against a tree reverted before the GREEN. It names its own field (`RED revision`, or `Falsifiability revision` in its place); items 5, 7 and 8 share `Revision`. That is the property that RED is worth having, not a defect in it; demanding one revision across all four made an `observed-red` E2E/API row unable to reach `done` however correct its evidence was. Such a row's RED names the revision it was observed at, items 5, 7 and 8 agree among themselves, and the reviewer checks that the handed-over RED names this row's selector and the predicate it owns rather than that it matches the final tree
