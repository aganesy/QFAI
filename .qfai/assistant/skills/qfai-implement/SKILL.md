...
(RED / GREEN / Refactor commands and results) is written **before** items 7-8, because it is what the
reviewers audit — naming only `implement-<spec-id>.md` here split an `E2E` row in two. The verdict fields are appended **after** items 7-8, and a phase-authored evidence file
