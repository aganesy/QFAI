...
- Read spec + contract inputs first whenever the item is UI-affecting (`references/ui-affecting.md`). Read order: `01_Spec.md` → `03_Acceptance-Criteria.md` → `05_Examples.md` → root `DESIGN.md` → `.qfai/contracts/design/DESIGN.md.lock.yaml` → `.qfai/contracts/design/design-system.yaml` (post-loop token mirror) → `.qfai/contracts/design/prototype-handoff.yaml` → the declared UI contracts, resolved exactly as the trigger resolves them (`references/ui-affecting.md#the-two-inputs`: every `.yaml` **and** `.yml` under `<contractsDir>/ui/**` recursively, `<contractsDir>` = `paths.contractsDir`) — a review must not skip the contract whose clause routed it → `.qfai/evidence/prototyping/iter-NN/<screen>.{png,html}` → `.qfai/prototypes/final/index.html`.
