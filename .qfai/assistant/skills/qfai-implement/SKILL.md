...
- Empty evidence entries are rejected: minimum evidence per TDD item must be met — `TDDLIST_EVIDENCE_EMPTY` (warning inside its promotion window, error from the release the finding names; a row already at a terminal status backfills the cell in place — `references/execution-ledger.md`). The cell is a **pointer**, in one legal shape capped at 240 characters — `RED:<fail|falsifiability|n-a> GREEN:pass ORACLE:<proved|equivalent-mutant> [TIER:<T1|T2|T3>] REV:<revision> -> .qfai/evidence/<implement|atdd>-<spec-id>.md#<tdd-nnnn>`, where the anchor's stage, `<spec-id>` and fragment are all checked against the row — the stage its `Layer` assigns, this spec's id, and this row's own `### TDD-NNNN` section — an `implement-` anchor on an ATDD-owned row needs the `Pre-split-evidence: implement` marker item 10 writes on `E2E` / `API` rows only, and that marker is legal nowhere else; `REV:` takes only the two spellings `references/evidence-revision.md` defines; commands and output live in the evidence file the anchor names — `TDDLIST_EVIDENCE_CELL_MALFORMED` (warning, waivable as `TDDLIST-007`) / `TDDLIST_EVIDENCE_CELL_OVERSIZE` (warning inside its promotion window, error from the release the finding names; waivable as `TDDLIST-008`), grammar in `references/evidence-cell-grammar.md` `RED:n-a` on an `E2E` / `API` / `Integration` row is `TDDLIST_EVIDENCE_RED_PROVENANCE` (error, rule `TDDLIST-009`) and **no waiver covers it** — those rows owe an observed RED or a falsifiability argument, and `references/execution-ledger.md#atdd-owned-rows` says of exactly them "There is no waiver here". A row carrying more cells than its table's header declares is `TDDLIST_ROW_EXTRA_CELLS` (warning inside its promotion window, error from the release the finding names; waivable as `TDDLIST-010`): cells are read by header index, so a surplus column is read by no rule at all.
