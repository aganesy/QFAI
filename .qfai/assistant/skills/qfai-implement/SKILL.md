...
the lifecycle is forward-only along `todo` -> `red` -> `green` -> `refactor` -> `done` plus the re-entry edges the reference enumerates, an `exception` requires a DR-ID, and a `blocked` row requires a `Blocked-By` and is never selected. `blocked` is entered from **any active status**, not only from `todo` — a blocker that surfaces at `red`, `green` or `refactor` is filed there, not at `exception`.
