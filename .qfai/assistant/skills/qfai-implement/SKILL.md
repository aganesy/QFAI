...
10. `test-list.md` Status is current and its Evidence cell's anchor resolves to a fresh per-item entry in the evidence file its `Layer` owns, and the seals and hashes that entry carries are recomputed here. The rule is `references/record-contract.md` — layer ownership, the `Pre-split-evidence: implement` compatibility marker and its one-off migration, what is recomputed, and the revision each of the four observations names. A row that satisfies it draws no `TDDLIST_EVIDENCE_*` or `TDDLIST_STALE_STATUS` finding from `npx qfai validate`; this item never fails for a reason that means the software is wrong
