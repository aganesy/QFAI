...
10. `test-list.md` Status is current and its Evidence cell's anchor resolves to a fresh per-item entry in the evidence file its `Layer` owns — `.qfai/evidence/implement-<spec-id>.md`, or `.qfai/evidence/atdd-<spec-id>.md` for an `E2E` / `API` row, whose RED provenance was produced by the stage that authored its test (`references/execution-ledger.md#atdd-owned-rows`). The cell is a pointer, not the payload (`references/execution-ledger.md#evidence-cell-contract`), and the item's four sub-agent observations (items 3, 5, 7, 8) all name the **same** revision (`references/evidence-revision.md`)
