...
1. **The exit criterion is executable, not prose**: the system starts from a declared entrypoint and the surface one declared `US-*` names is **reached** over the **real transport that entrypoint declares** — a socket for a service, stdio for a CLI, the queue for a worker — proven by a committed smoke script that exits non-zero otherwise. **Reached, not satisfied**: the script asserts that the started process answered at all, whatever it answered — a seam this phase newly authors answers with the non-contracted sentinel of item 2 because item 2 leaves it nothing else, while a surface an existing application or a finished row already serves exits the phase on its real response, never regressed to a sentinel to look skeletal — and requiring the `US-*`'s outcome would contradict item 2 on every project whose obligations are authorization, calculation or persistence, and leave the phase no legal exit. `qa-gatekeeper` is **mandatory and blocking** here, never the author's own account: its `PASS` on the recorded run goes in `Skeleton gatekeeper`. An already-passing smoke script satisfies the phase — re-run it and record that run, and continue only while **it** exits 0; a non-zero re-run makes the entrypoint unproven again and puts it back through this phase's cycle budget. That re-run is owed **inside** the invocation as well as at the start of the next one: the spec-level checkpoint re-runs every in-scope `Skeleton command` and blocks completion on a non-zero exit (`references/checkpoint-verification.md`), because the rest of that command set is satisfiable by a tree whose composition root this invocation's own rows have broken. A spec set with no runnable entrypoint (a library) records `Skeleton: not applicable` with the reason; the verdict is written and the phase is never skipped silently. The record is project-level, at `.qfai/evidence/skeleton.md` — git-tracked through the managed `.gitignore` negation, since Bound 2's "same commit" and the next invocation's check both need it out of one working directory — one section per declared entrypoint: the phase runs before the first row is selected, so there is no ledger `Evidence` cell to hang it off, and that file is what makes "already proven" checkable on the next invocation. Because it is tracked, `Skeleton command` and `Skeleton result` go in verbatim **with known secret values replaced by a named placeholder** — a queue URL, a connection string or a token written there unredacted enters the repository's permanent history, where rotation is the only removal; the exit status is never redacted and the failing command and its error stay readable, so the halt is still classifiable.
...
3. **Write the seam debt back in the same commit** — also **blocking**, and **through the ledger's owner**: enumerate every shortcut in `Skeleton debt` and raise the missing rows as a Change Request for `/qfai-sdd` per `constitution/drift-protocol.md#when-drift-is-detected`. Adding a row here would be drift — this skill's carve-out is the `Status` / `DR-ID` / `Evidence` cells of rows that already exist — and that CR's halt is the scoped one, so the rest of `Phase: Red` continues. The skeleton may be shallow; it may not be invisible to the ledger.
