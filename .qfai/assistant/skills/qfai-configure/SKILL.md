...
Follow `.qfai/assistant/constitution/shared-skill-operating-baseline.md#completion-contract-shared`. **Smallest applicable smoke check** (this skill's override): one terminating command drawn from the minimum runnable path you just recorded — its smallest test or check invocation, or a readiness probe against a service the path starts — plus `npx qfai validate --fail-on error` against the `qfai.config.yaml` this run produced, **judged only over the findings this skill owns**: config-load and traceability findings, and findings in the files the allow-list above lets this run write. A profile-less `validate` runs every validator, so an `error` this run neither introduced nor is permitted to repair — an unresolved spec, ATDD or prototyping artifact — is reported by code as a pre-existing blocker owned by that stage's skill rather than read as this bullet's FAIL; a config-owned or traceability `error` still fails it. The long-running parts of that path (dev server, database) are setup for the probe, never the check itself: a command that does not exit cannot return the pass/fail this bullet needs. A path whose terminating command was recorded but never executed is UNRUN, not a pass.
