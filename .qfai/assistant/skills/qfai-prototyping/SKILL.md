...
| U     | orchestrator                                              | **Put the question the loop was built to answer back to the user, against the converged prototype**, through `AskUserQuestion`. Blocking: `H` does not start until they answer. Record the answer in `.qfai/evidence/prototyping/grilling.md` under `## Session`. Under a no-question mode nobody can answer, so the choice is written to `## Escalated` and the run stops there rather than certifying a design nobody picked — convergence is the reviewer's verdict on four axes, not the user's on the question.                                                                                                                                                                                                                                                                                                            | user-settled choice, recorded            |
