...
Under a no-question mode the session cannot ask, so what the prototype is for,
what would count as better and what is out of bounds are recorded open and
nobody answers them. Starting C0 there spends the whole cycle budget building
against nothing: the generator has no constraint to satisfy, and the reviewer
