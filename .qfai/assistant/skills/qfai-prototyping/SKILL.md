...
One final prototype satisfying the spec under a locked brand identity, with no blocking findings, no layout anti-patterns, and no DESIGN.md violations.
...
- `references/reviewer-prompt.md` — reviewer schema, the eight criteria,
...
(`blockingFindings` empty AND `layoutAntiPatternsDetected` empty AND
