...
- Declare each row's `Owning module` from the TC's parent `BR` — one module per row, `-` only when no single module owns it. It is the sole input `delivery-planner` can judge parallel dispatch on, and no later phase can recover it.
- Resolve that parent through the TC's `AC-Refs`: it is the `04_Business-Rules.md` row whose `AC-Refs` names the same `AC`. `EX-Ref` never selects it, so an error / boundary TC carrying `—` inherits the parent of its normal-case sibling on that `AC`. Neither table carries a module path column, so name the repository module that `BR`'s `Rule` will be implemented in; write `-` when the TC's ACs resolve to no `BR`, or to several with different homes.
- Migrate a pre-existing ledger in place: when its header predates `Owning module`, append the column to the header and separator rows and fill it for every row, including rows the delta otherwise leaves untouched. The delta rule preserves cell values, not the column set, and the template is copied only when the file is absent — without this step an upgraded project's ledger never gains the column. Position is free: the validator resolves the column by name.
