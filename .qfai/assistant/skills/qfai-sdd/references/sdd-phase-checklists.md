...
- Retiring a row means **deleting it from the ledger table**, with the removal recorded as `<spec-id>/TDD-NNNN` in the driving `CR-*` (`TDD-ID` is unique only within its spec, so a bare `TDD-0001` is ambiguous in a CR spanning two) — removing a row is an upstream change and takes the Change Request path. There is no `retired` status (`Status = retired` is a `TDDLIST_INVALID_STATUS` error), and moving the row below the ledger does not retire it either: `validateTddList` scores every schema-complete table in the file. The RED/GREEN record survives the deletion in the evidence file the row's `Layer` owns — `.qfai/evidence/implement-<spec-id>.md`, or `.qfai/evidence/atdd-<spec-id>.md` for an `Integration` / `API` / `E2E` row — which is what the `Evidence` cell pointed at. A retired `TDD-ID` is never reused: allocate above the highest the spec has ever issued.
