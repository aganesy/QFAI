...
- Seed four groups of rows, all at `Status = todo`:
  - one row per coverage-target TC from `06_Test-Cases.md` that declares a `Level` the layer vocabulary recognises (`L1` / `L2` / `unit` / `component`), obligation in `TC-Refs`;
...
- **Integration-level means every `Level` whose ATDD annotation routes to the `tests/integration` tree**: `L3`, `integration`, a blank cell, a spelling that names no layer (`smoke`), **and `system` / `acceptance`**. Route by that destination, not by whether you recognise the word: `system` and `acceptance` are in the layer vocabulary and are not unit or component, so a "blank or unrecognised" test puts them in neither TC group while `/qfai-atdd` still writes their tests, leaving a TC whose handoff row nothing seeds. These are ATDD-owned rows (`../../qfai-implement/references/execution-ledger.md`): `/qfai-atdd` authors their tests, this phase is the only thing that creates the rows, and no validator reports them missing. A TC whose `Level` is blank or unrecognised belongs here and not in the coverage-target group, because `QFAI-ATDD-112` routes every `Level` it cannot read to the same tree; seeding it as a coverage-target row has two skills write a test for one TC, and `TDDLIST_UNKNOWN_LEVEL` is a waivable `warning` that does not stop such a TC reaching this phase. Its `Integration` row still satisfies `TDDLIST_TC_NOT_COVERED`, which accepts any row carrying the TC in `TC-Refs`.
- "One row" is a floor in both TC groups: a matrix-shaped TC — several rejection reasons, a status-code matrix, independent state transitions — is seeded one row per independently observable boundary, all carrying that TC in `TC-Refs` (`../../qfai-implement/references/selector-granularity.md`). For an `Integration` TC this phase is the only place the split can happen: `/qfai-atdd` takes its RED per row and never writes the ledger.
- Fill `BR-Ref` — the TC's `EX-Ref`, then that `EX`'s `BR-Ref` in `05_Examples.md` (one `EX` may name several `BR-*`); only a TC with no `EX-Ref` falls back to its `AC-Refs` and the `BR` whose `AC-Refs` names one of them in `04_Business-Rules.md`; the lowest-numbered `BR-*` of the whole union wins, `-` when none does. It is `/qfai-implement`'s T1 review-group key and this is the only phase that can resolve it. Optional to `npx qfai validate`, so an unseeded ledger still passes — and still cannot be batched.
...
- Without the E2E and API rows the ledger cannot hold a `US-*` / `CON-API-*` obligation at all, so an all-`done` ledger reports complete beside a `QFAI-ATDD-111` / `QFAI-ATDD-113` gate at 0%. Without the integration rows an integration-level TC has no row anywhere, and **nothing reports that** — the ledger looks complete and `validate` stays quiet while the whole integration layer sits outside the micro-cycle.
...
- **A `Level` change that crosses between the two TC groups reclassifies the row; it is not an upstream reset.** `L3` -> `L1`/`L2`, or `L1`/`L2` -> `L3`, retires nothing (both layers are seeded here) so only the changed-TC reset fires — and that rule writes `Status` and `DR-ID` and nothing else. The row's group is what decides its `Layer`, which skill authors the test, and which evidence file gate item 10 resolves its anchor against, so a reset alone leaves a now-`L1` TC holding an ATDD-owned `Integration` row waiting on a `/qfai-atdd` handoff that `QFAI-ATDD-112` no longer produces, or a now-`L3` TC holding a coverage-target row while both skills write a test for it. Handle the crossing explicitly:
  - **Row still at `Status = todo`** — retire it and seed a fresh row in the new group. Nothing has been observed yet, so there is no identity or evidence to carry, and re-seeding sets `Layer`, `Test file` and `Selector` correctly by construction.
  - **Row past `todo`** — **stop and raise a `CR-*`**. Its `Evidence` points into the previous owner's file (`implement-<spec-id>.md` vs `atdd-<spec-id>.md`), and the template keeps prior `Evidence` across a reset, so rewriting `Layer` alone leaves the anchor resolving against a file that never held the record. Which evidence survives a change of owner is a decision for the change record, not a mechanical rewrite.
