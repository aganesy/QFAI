...
- Contract-scoped mode, scope closure: re-expand after every contract write this phase makes — recompute the specs referencing the contracts now in scope, reconcile the ones that just entered, and repeat until a pass adds no spec. A shared contract amended here may be referenced only by specs Phase 0 never put in scope.
