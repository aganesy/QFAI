...
- Name the boundary in `Selector` on every split row, at seed time, while `Test file` is still `-`. `TDD-ID` is a serial and `TC-Refs` repeats identically across sibling rows, so `Selector` is the only cell that tells two rows of one `TC-*` apart: it is the row's stable identity across reseeds. Never reuse one boundary's `Selector` on two rows, and never repoint an existing row's `Selector` at a different boundary — that silently re-scopes a row whose `Evidence` was observed against the old one.
...
- Migrate legacy aggregate rows before applying the delta: a ledger seeded under the old "one row per TC" wording holds a single row for a matrix-shaped TC. Such a row is **not** unchanged, and the preserve rule above does not protect it. Re-split it — keep it for the one boundary its `Selector` already names and append a `todo` row per remaining boundary. When its `Selector` names no single boundary, or its `Status` is past `todo`, reset that row to `todo` too and keep the prior `Evidence` under the upstream-reset rule: a single RED stops at the first failing assert, so an aggregate row's observation evidences no boundary in full and must be re-observed per row.
