...
- Once split, each of those rows is identified by the (`TC-Refs`, `Boundary`) pair — sibling rows share one `TC-*` and differ only in `Boundary`, so a reseed matches on that pair, not on `Selector`. A slug is unique within its `TC-*` and nowhere wider, so `Boundary` on its own is not a ledger-wide key: a generic slug recurs across TCs and would match the wrong sibling. `Selector` is the runtime test name and `/qfai-implement` rewrites it when a review-fix handback replaces the test, so it is an execution handle, not an identity. Retire the row of a boundary dropped from a surviving `TC-*`, and re-split a legacy single-row aggregate instead of preserving it as unchanged — that migration is one re-scope gated on an approved `CR-*` enumerating the aggregate rows, appended rows included, and it lands whole or not at all (`sdd-phase-checklists.md#phase-2b-seed-tddtest-listmd`).
- `Boundary` is an optional column and the split row's stable identity: a short slug for the one observable boundary the row owns, written by `/qfai-sdd` Phase 2b while `Test file` is still `-` and never rewritten by `/qfai-implement`. A ledger that holds more than one row for a `TC-*` needs it, and `validateTddList` enforces that: sibling rows with no `Boundary` raise `QFAI-TDD-003` and two siblings claiming one slug raise `QFAI-TDD-004`. **Both ship behind a promotion window**, so neither severity is fixed: each is a `warning` until the release its window names and an `error` from then on, and the finding says which while it is the former. A legacy ledger cannot be migrated before its `CR-*` is approved, which is what the window on `QFAI-TDD-003` is for; a duplicate slug can only be authored by a Phase 2b that already writes the column, which is why its window is short rather than absent. A single-row `TC-*` owes no slug — it is identified by its TC.
