...
- Once split, each of those rows is identified by its `Boundary` cell — sibling rows share one `TC-*` and differ only there, so a reseed matches rows on `Boundary`, not on `Selector` or `TC-Refs`. `Selector` is the runtime test name and `/qfai-implement` rewrites it when a review-fix handback replaces the test, so it is an execution handle, not an identity. Retire the row of a boundary dropped from a surviving `TC-*`, and re-split a legacy single-row aggregate instead of preserving it as unchanged — appending its missing boundary rows is unconditional, while resetting or retiring the aggregate row itself needs an approved `CR-*` that enumerates it (`sdd-phase-checklists.md#phase-2b-seed-tddtest-listmd`).
- `Boundary` is an optional column and the split row's stable identity: a short slug for the one observable boundary the row owns, written by `/qfai-sdd` Phase 2b while `Test file` is still `-` and never rewritten by `/qfai-implement`. A ledger that holds more than one row for a `TC-*` needs it; a legacy ledger without the column gets it at the next reseed.
