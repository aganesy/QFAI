...
- **Ownership split.** `/qfai-sdd` owns the rows — which obligations exist and what each
  ...
  Request path — including the rows `/qfai-implement` used to open itself for a post-RED scope
  gap or a checkpoint regression, which now come back here as a Change Request.
