...
- Phase 2b seeds each target spec's tdd/test-list.md from 06_Test-Cases.md (one row per coverage-target TC, plus at least one Layer = Integration row per integration-level TC — every Level whose ATDD annotation routes to the tests/integration tree: L3, integration, blank, a spelling that names no layer, and system / acceptance — Status = todo; a matrix-shaped TC splits one row per boundary) and is a delta: existing rows keep their TDD-ID, Status, Test file, Selector, DR-ID and Evidence, and a row past todo is never retired by comparing its Selector against the spec.
