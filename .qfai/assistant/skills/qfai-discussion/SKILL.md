...
9. **Emit root `DESIGN.md` draft** for UI-bearing targets per `references/design-dna-intake.md`. Required when UI-bearing; skip otherwise. Run it in two phases: **Phase A** selects `brand.archetype` from `references/design-md-brand-catalog.md` and adopts its `aesthetic_properties` as defaults; **Phase B** then customizes those defaults, routing each to its own home: `color_tendency` / `typography` / `spacing` into the project-specific `visual.*` token tree, and the `interaction` default into `accessibility.motion` (`visual.*` accepts only `colors | typography | radius | shadow | spacing`).
