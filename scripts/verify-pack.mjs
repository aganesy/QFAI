...
// `.github/` may exist under root init assets PROVIDED it only ships `workflows/`
// (spec-0017 REQ-0009 ships .github/workflows/qfai-validate.yml). Other wrapper
// subdirs are still generated by runInit symlinks and must not be packed.
const rootGithubDir = path.join(rootAssetsDir, ".github");
if (existsSync(rootGithubDir)) {
  for (const forbidden of ["instructions", "agents", "skills", "commands", "prompts"]) {
    if (existsSync(path.join(rootGithubDir, forbidden))) {
      throw new Error(`assets/init/root/.github/${forbidden} must not exist.`);
    }
  }
}
