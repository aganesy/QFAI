...
async function main() {
  ...
  const roots = targets.length > 0 ? targets : [REPO_ROOT];
  const files = [];
  ...
  for (const file of unique) {
    ...
    try {
      ...
      console.error(
        `check-mermaid: cannot read ${path.relative(REPO_ROOT, file)}: ${describeError(error)}`,
      );
      return 2;
    }
  ...
  if (listOnly) {
    for (const diagram of diagrams) {
      console.log(`${path.relative(REPO_ROOT, diagram.file)}:${diagram.line}`);
    }
  ...
  if (failures.length > 0) {
    for (const failure of failures) {
      console.error(`${path.relative(REPO_ROOT, failure.file)}:${failure.line}: ${failure.reason}`);
    }
    console.error(
