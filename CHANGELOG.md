...
- **参照グラフの到達判定が、マシン側のパスでトークン走査が止まる場合を取りこぼして
  いた。** `paths.skillsDir` をプロジェクト外に置くと、その配下の文書は隣のファイルを
  **絶対パス**で引用する。引用トークンの正規表現が跨げるのは
  `[\p{L}\p{N}\p{M}._-]` の連なりだけなので、絶対パスの前半 — リポジトリではなく
  マシンが決める部分 — に空白や `~` が入っていると、そこで走査が切れて
  `1/AppData/.../guide.md` のような尾部だけがトークンになり、どの base からも解決
  できない。結果、明白に引用されている reference が `QFAI-SKILLS-013` (未参照) として
  報告されていた。

  `isTokenScannable` はこれまで**プロジェクト相対**の綴りだけを見ていた。外部
  `skillsDir` の場合その綴りは綺麗なままなので、パスを直接照合する第 2 パス
  (`unscannableTargets`) にも載らず、救済されなかった。絶対形の綴りも判定に加え、
  トークン走査が跨げないパスを持つ文書は第 2 パスに回すようにした。

  Windows では `os.tmpdir()` が 8.3 短縮名 (`C:\Users\RUNNER~1\AppData\Local\Temp`) を
  返すため、この不具合はテスト sandbox のほぼ全件に当たっていた。CI matrix は Linux
  専用で、そこでは短縮名が現れないため検出できない。回帰ケースは `~` を明示的に含む
  ディレクトリを作って書いたので、Linux でも赤くなる。
