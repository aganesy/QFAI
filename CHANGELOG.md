...
- `QFAI-REVIEW-001` (root `.gitignore` managed-block check) now also
  ...
  re-run `qfai init` (idempotent; the gitignore writer skips the
  marker when present and only adds the missing entry). The init
  branch also writes the new entry on fresh repos.
